...
// ============================================================================
// ⚫ 검은수염 (암흑 왕좌 전용 보스)
//    지상과 동일한 높이(y=2000) · 맵 최우측(x 28000~33000)
// ============================================================================
const DARK_AREA   = { minX: 28000, maxX: 33000, minY: 600, maxY: 2060 };
const DARK_GROUND = 2000;
const DARK_ENTRY_X = 30500;
const DARK_ENTRY_Y = DARK_GROUND - 45;
// ✅ [추가] 렌더/스폰 격리 구간 (gameState.js 의 DARK_ZONE_X 와 동일)
const DARK_ZONE_MIN = 27400;
const DARK_ZONE_MAX = 33600;

const BB_RADIUS = HINBEOM_RADIUS;                       // 크기는 박힌범과 동일 (94.5)
// ✅ [수정] 초당 회복 제거 + 체력 1.2배 (10000 → 12000)
const BB_MAXHP  = Math.round(HINBEOM_MAXHP * 1.2);      // 12000
const BB_SPEED  = 1.75;
const BB_HOME_X = 30500;
...
const DARKFLOOR_JUMP_NERF = 0.7;

// ⛓️ 크로우즈 (7초마다) + 파공아
const CROWS_INTERVAL   = 7000;
...
// ============================================================================
// 🟪 지저스 바제스 (검은수염 체력 절반 이하에서 소환되는 보라색 거인)
// ============================================================================
const BG_RADIUS      = BB_RADIUS * 0.8;                 // 검은수염의 0.8배 = 75.6
const BG_MAXHP       = Math.round(BB_MAXHP * 0.5);       // 검은수염의 0.5배 = 6000
const BG_SPEED       = 2.2;
const BG_GOLD        = 4000;
const BG_XP          = 800;

// 🌪️ 등장 : 하늘에서 빠르게 낙하 → 착지 시 큰 풍압
const BG_FALL_FROM   = -2600;                           // 낙하 시작 높이
const BG_FALL_SPEED  = 120;                             // 프레임당 낙하 속도
const BG_LAND_DAMAGE = 500;                             // 착지 풍압 피해
const BG_LAND_MULT   = 2.0;                             // 바제스 크기의 2배 범위

// 🦘 점프 공격 : 3초마다 · 0.7초 예고 후 도약
const BG_JUMP_INTERVAL  = 3000;
const BG_JUMP_TELEGRAPH = 700;
const BG_JUMP_TRAVEL    = 320;                          // 실제 도약에 걸리는 시간(ms)
const BG_JUMP_DAMAGE    = 300;
const BG_JUMP_MULT      = 1.5;                          // 바제스 크기의 1.5배 범위
const BG_JUMP_ARC       = 520;                          // 도약 포물선 높이
...
/** ⛓️ 크로우즈 조준선(빨간 표시) 안에 대상이 들어와 있는가 */
function isInCrowsBeam(bb, t) {
    ...
    return s >= 0 && s <= CROWS_RANGE && d <= (CROWS_THICKNESS / 2);
}
...
const BG_DELTA_FIELDS = [
    'x', 'y', 'hp', 'maxHp', 'radius', 'state', 'knockbackForce',
    'fallingUntil', 'jumpTelegraphUntil', 'jumpingUntil', 'jumpTargetX',
    'frozenUntil', 'electrocutedUntil', 'airFreezeUntil', 'raigoPullUntil',
    'burningUntil', 'maguBombUntil', 'justiceBombUntil'
];
...
function makeBlackbeard() {
    return {
        ...
        burgessSummoned: false     // 🟪 바제스를 이미 소환했는가
    };
}

/** 🟪 지저스 바제스 초기 상태 객체 생성 (소환 전에는 hp = 0) */
...
const State = {
    ...
    blackbeardKilledBy: null,          // 🟪 바제스가 남아있을 때를 대비해 처치자를 기억
    turrets: [
...
// ✅ [수정] 우측 오크라 스폰 위치를 암흑 왕좌(27400~) 밖으로 당겨 겹치지 않게 한다
for(let i=0; i<8; i++) spawnOkra(22300 + i*550, 1955);
...
function checkPlayerDeath(targetPlayer, attackerId) {
    if (targetPlayer.hp <= 0 && !targetPlayer.isDead) {
        // ✅ [추가] 암흑 왕좌 안에서 죽었다면, 박힌범을 다시 잡아 포탈이 새로 생길 때까지 재입장 금지
...
function killHinbeom(attackerId) {
    ...
    if (Math.random() < 0.25) {
        ...
        // ✅ [추가] 새 암흑 포탈이 열리면 모든 플레이어의 입장 금지가 해제된다
...
function checkBurgessSummon() {
    ...
    bg.fallingUntil = Date.now() + 8000;   // 안전장치 (최대 8초 안에는 착지)
    bg.jumpNextCast = 0;
...
/** 🟪 바제스 착지 / 점프 착지 시 큰 풍압 */
function burgessShockwave(cx, cy, radius, damage, attackerIdForKb) {
...
function killBlackbeard(attackerId) {
    ...
    setTimeout(() => {
        ...
        State.blackbeard.burgessSummoned = false;
        // 🟪 바제스도 완전히 초기화
        State.burgess = makeBurgess();
        io.emit('burgessDespawn');
        State.blackbeardPortal = null;
...
const serverContext = {
    ...
    recordHinbeomDamage,
    PORTAL_COOLDOWN, PORTAL_DWELL_MS, PORTAL_RADIUS,
    ...
    BG_JUMP_INTERVAL, BG_JUMP_TELEGRAPH, BG_JUMP_TRAVEL, BG_JUMP_DAMAGE, BG_JUMP_MULT, BG_JUMP_ARC
};
