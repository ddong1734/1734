...
const HINBEOM_XP      = 500 * 2;              // ✅ [수정] 처치 보상 경험치 2배 (500 → 1000)
const HINBEOM_RESPAWN = 120000;               // 부활 시간(ms)

// 🏆 박힌범 오크라 드롭 조건
const HINBEOM_DROP_ITEM   = 'hinbeom_okra';   // 드롭 아이템 id
const HINBEOM_DROP_DAMAGE = 2500;             // 이 수치 이상 피해를 준 플레이어만 대상
const HINBEOM_DROP_CHANCE = 0.15;             // 대상자별 15% 확률

// 🐗 할배새끼 오크라 드롭 조건 (소환된 할배새끼 처치 시)
const MINION_DROP_ITEM   = 'halbae_okra';     // 드롭 아이템 id
const MINION_DROP_CHANCE = 0.05;              // 5% 확률

// 🌀 박힌범 처치 시 생성되는 귀환 포탈
const PORTAL_RADIUS   = 110;                  // 포탈 판정/표시 반경
const PORTAL_DURATION = HINBEOM_RESPAWN;      // 박힌범이 부활할 때까지 유지
const PORTAL_COOLDOWN = 1500;                 // 같은 플레이어의 연속 사용 방지(ms)
const PORTAL_DWELL_MS = 3000;                 // ✅ 포탈 위에서 머물러야 하는 시간(3초)
...
// ⚫ 검은수염 (암흑 왕좌 전용 보스)
// ----------------------------------------------------------------------------
//  암흑 왕좌 구조 (data.js Platforms 기준)
//    바닥 : { x: 22000, y: -4000, w: 5000, dark } → 윗면 y = -4000
//    좌벽 : { x: 22000, y: -4900, h: 900, solid, dark }
//    우벽 : { x: 26960, y: -4900, h: 900, solid, dark }
// ============================================================================
const DARK_AREA   = { minX: 22000, maxX: 27000, minY: -5200, maxY: -3940 };
const DARK_GROUND = -4000;                     // 암흑 왕좌 바닥 발판 윗면
const DARK_ENTRY_X = 24500;                    // 포탈로 진입했을 때 소환되는 위치
const DARK_ENTRY_Y = DARK_GROUND - 45;

const BB_RADIUS = HINBEOM_RADIUS;              // ✅ 크기는 박힌범과 동일
const BB_MAXHP  = HINBEOM_MAXHP;               // ✅ 체력도 박힌범과 동일 (10000)
const BB_SPEED  = 1.75;
const BB_HOME_X = 24500;                       // 암흑 왕좌 정중앙
const BB_HOME_Y = DARK_GROUND - BB_RADIUS;
const BB_REGEN  = 100;                         // 1초마다 회복량
const BB_GOLD   = 8000;                        // 처치 보상 골드
const BB_XP     = 1500;                        // 처치 보상 경험치
const BB_RESPAWN = 120000;                     // 부활 시간(ms)

// 🌊 블랙홀 — 암흑물질 장판 (1초마다 7%)
const DARKFLOOR_CHANCE   = 0.07;
const DARKFLOOR_ROLL_MS  = 1000;
const DARKFLOOR_DURATION = 4000;               // ✅ [수정] 시전시간 3초 → 4초
const DARKFLOOR_TICK_MS  = 500;                // 0.5초마다
const DARKFLOOR_TICK_DMG = 40;                 // 0.5초당 40 피해
const DARKFLOOR_JUMP_NERF = 0.7;               // 점프력 70% 감소 (클라이언트 물리에서 적용)

// ⛓️ 크로우즈 (7초마다) + 파공아
const CROWS_INTERVAL   = 7000;                 // 7초마다 시전
const CROWS_TELEGRAPH  = 1000;                 // 시전 1초 전 경직 + 빨간 조준 표시
const CROWS_RANGE      = 1680 * 3;             // ✅ 각성 엘 토르와 동일한 사거리
const CROWS_THICKNESS  = 90 * 3;               // ✅ 각성 엘 토르 두께 (270)
const CROWS_PULL_MS    = 420;                  // ✅ [수정] 900 → 420 : 훨씬 빠르게 끌려온다
const GURA_DAMAGE      = 500;                  // 파공아 피해량
const GURA_RADIUS_MULT = 3.0;                  // ✅ [수정] 1.2 → 3.0 : 공격 판정 대폭 상향

// 🌑 공중 강림 (1초마다 3% · 5초 지속)
const DESCENT_CHANCE    = 0.03;
const DESCENT_ROLL_MS   = 1000;
const DESCENT_DURATION  = 5000;
const DESCENT_TICK_MS   = 1000;                // 1초마다
const DESCENT_TICK_DMG  = 150;                 // 1초당 150 피해 + 기절
const DESCENT_RISE      = 420;                 // ✅ [수정] 900 → 420 : 플레이어 화면에 보이는 높이
const DESCENT_ASCEND_MS = 2000;                // ✅ [추가] 어둠 소용돌이로 솟구쳐 오르는 시간(2초)
...
const PLAYER_DELTA_FIELDS = [
    ...
    'hasGoro', 'hasArkMaxim', 'hasGodEnel', 'elThorActive', 'yataActive', 'crowsPullUntil'
];
...
const compressors = {
    ...
    blackbeardDelta: new DeltaCompressor(BB_DELTA_FIELDS)     // ⚫ 추가
};
...
function makeHinbeom() {
    return {
        ...
        hakiCount: 0,             
        lastRegenTick: 0,
        damageBy: {}              // 🏆 플레이어별 누적 피해량 (박힌범 오크라 드롭 판정용)
    };
}
...
function makeBlackbeard() {
    return {
        ...
        // 🔒 스킬 중복 사용 방지 (이 시각까지는 다른 스킬을 쓸 수 없다)
        castingUntil: 0,
        ...
        // ⛓️ 크로우즈
        crowsNextCast: 0,
        telegraphUntil: 0,          // 경직 + 빨간 조준 표시 구간
        crowsPendingTarget: null,
        crowsAimX: 0, crowsAimY: 0,
        crowsActiveTarget: null,
        crowsHitAt: 0,              // 이 시각에 파공아가 터진다

        // 🌑 공중 강림 (어둠 소용돌이로 2초간 솟구친 뒤 5초간 시전)
        descentNextRoll: 0,
        risingUntil: 0,             // ✅ 어둠 소용돌이로 상승 중인 구간
        riseFromY: 0, riseToY: 0,   // ✅ 상승 보간용
        descentUntil: 0,
        descentNextTick: 0,
        descentActive: false,
        descentGroundY: 0           // 강림 종료 후 되돌아갈 지면 Y
    };
}
...
const State = {
    ...
    hinbeomPortal: null,                       // 🌀 박힌범 처치 시 생성되는 귀환 포탈
    darkPortal: null,                          // 🟣 박힌범 처치 시 25% 확률로 생성되는 암흑 왕좌 포탈
    blackbeard: makeBlackbeard(),              // ⚫ 검은수염
    blackbeardPortal: null,                    // 🌀 검은수염 처치 시 생성되는 귀환 포탈
    turrets: [
...
/** 🏆 박힌범 처치 시 '2500 이상 피해를 준 모든 플레이어'에게 15% 확률로 박힌범 오크라 지급 */
...
function processBurns(now) {
    for (let [key, stacks] of State.burnMap) {
        ...
        else if (key === 'blackbeard') { entity = State.blackbeard; kind = 'blackbeard'; } // ⚫ 추가
        ...
        if (kind === 'player') {
            ...
            if (State.hinbeomMinions.length > 0) {
                ...
                recordHinbeomDamage(lastOwner, dmg);                                      // 🏆 피해량 기록
                if (State.hinbeom.hp <= 0) killHinbeom(lastOwner);
            }
        } else if (kind === 'blackbeard') {                                               // ⚫ 추가
...
function resetGame() {
    ...
    State.hinbeomPortal = null;                                                           // 🌀 추가
    State.darkPortal = null;                                                              // 🟣 추가
    State.blackbeard = makeBlackbeard();                                                  // ⚫ 추가
    State.blackbeardPortal = null;                                                        // 🌀 추가
    io.emit('syncHinbeomPortal', null);                                                   // 🌀 추가
    io.emit('syncDarkPortal', null);                                                      // 🟣 추가
    io.emit('syncBlackbeardPortal', null);                                                // 🌀 추가
    ...
    compressors.blackbeardDelta.snapshots.clear();                                        // ⚫ 추가
}
...
function checkPlayerDeath(targetPlayer, attackerId) {
    if (targetPlayer.hp <= 0 && !targetPlayer.isDead) {
        ...
        targetPlayer.portalDwellUntil = 0; targetPlayer.portalDwellStart = 0;      // 🌀 사망 시 포탈 대기 초기화
        targetPlayer.darkDwellUntil = 0; targetPlayer.darkDwellStart = 0;          // 🟣 암흑 포탈 대기 초기화
        targetPlayer.crowsPullUntil = 0;                                           // ⚫ 크로우즈 해제
...
function killHinbeom(attackerId) {
    ...
    // 🏆 2500 이상 피해를 준 모든 플레이어에게 15% 확률로 박힌범 오크라 지급
    tryHinbeomOkraDrop();
    State.hinbeom.damageBy = {};
    ...
    // 🟣 25% 확률로 기지 귀환 포탈 '옆'에 암흑 왕좌 포탈 생성
    if (Math.random() < 0.25) {
        let px = State.hinbeom.x + 320;
        // 바구니 밖으로 나가면 반대편에 만든다
        if (px > HINBEOM_AREA.maxX - 160) px = State.hinbeom.x - 320;
    ...
    setTimeout(() => {
        ...
        State.hinbeom.damageBy = {};                       // 🏆 피해 기록 초기화
        State.hinbeomPortal = null;                        // 🌀 부활과 함께 포탈 소멸
        State.darkPortal = null;                           // 🟣 암흑 포탈도 함께 소멸
...
function killBlackbeard(attackerId) {
    if (State.blackbeard.state === 'dead') return;
    if (attackerId && State.players[attackerId]) {
    ...
    State.blackbeard.targetId = null;
    State.blackbeard.state = 'dead';
    clearBlackbeardSkills();
    clearBurns('blackbeard', State.blackbeard);

    setTimeout(() => {
        State.blackbeard.hp = State.blackbeard.maxHp;
        State.blackbeard.x = State.blackbeard.homeX; State.blackbeard.y = State.blackbeard.homeY;
        State.blackbeard.state = 'idle';
        State.blackbeard.knockbackForce = 0;
        State.blackbeard.frozenUntil = 0; State.blackbeard.electrocutedUntil = 0;
        State.blackbeard.airFreezeUntil = 0; State.blackbeard.raigoPullUntil = 0;
        State.blackbeard.burningUntil = 0; State.blackbeard.maguBombUntil = 0; State.blackbeard.justiceBombUntil = 0;
        State.blackbeard.lastRegenTick = 0;
        State.blackbeard.darkFloorNextRoll = 0;
        State.blackbeard.descentNextRoll = 0;
        State.blackbeard.crowsNextCast = 0;
...
function applyAoEDamage(attacker, cx, cy, radius, damage, kb) {
    ...
    if (State.hinbeom.hp > 0 && Math.hypot(cx - State.hinbeom.x, cy - State.hinbeom.y) < radius + State.hinbeom.radius) {
        ...
        if (State.hinbeomMinions.length > 0) {
            ...
            recordHinbeomDamage(attacker.id, damage);                                      // 🏆 피해량 기록
...
/** ⚫ 검은수염 어그로 — 공격자가 암흑 왕좌 안에 있을 때만 추적한다 */
...
const serverContext = {
    ...
    getBlackbeard: () => State.blackbeard,                             // ⚫ 추가
    ...
    recordHinbeomDamage,                                               // 🏆 추가
    PORTAL_COOLDOWN, PORTAL_DWELL_MS, PORTAL_RADIUS,                   // 🌀 추가
...
function tryReconnect(socket, sessionId) {
    ...
    p.portalDwellUntil = 0; p.portalDwellStart = 0;                                        // 🌀 재접속 시 대기 초기화
    p.darkDwellUntil = 0; p.darkDwellStart = 0;                                            // 🟣 재접속 시 대기 초기화
    p.crowsPullUntil = 0;                                                                  // ⚫ 크로우즈 해제
    ...
    if (State.blackbeard.targetId === existingOldId) State.blackbeard.targetId = null;     // ⚫ 추가
    ...
    socket.emit('reconnectSuccess', {
        ...
        hinbeomPortal: State.hinbeomPortal,                                                 // 🌀 추가
        darkPortal: State.darkPortal,                                                       // 🟣 추가
        blackbeard: State.blackbeard,                                                       // ⚫ 추가
        blackbeardPortal: State.blackbeardPortal                                            // 🌀 추가
    });
...
io.on('connection', (socket) => {
    ...
    socket.on('joinLobby', (data) => {
        ...
        State.players[socket.id] = {
            ...
            lastDarkPortalUse: 0, darkDwellStart: 0, darkDwellUntil: 0,
    ...
    socket.on('startGame', () => { 
        ...
        io.emit('syncHinbeomPortal', State.hinbeomPortal);                                  // 🌀 추가
        io.emit('syncDarkPortal', State.darkPortal);                                        // 🟣 추가
        io.emit('syncBlackbeardPortal', State.blackbeardPortal);                            // 🌀 추가
    });
    ...
    socket.on('playerMove', (data) => {
        ...
        if (p && !p.isDead) { 
            ...
            if (Date.now() - (p.lastPortalUse || 0) < 500) return;      // 🌀 포탈 순간이동 직후 좌표 되돌림 방지
            if (Date.now() - (p.lastDarkPortalUse || 0) < 500) return;  // 🟣 암흑 포탈 진입 직후 좌표 되돌림 방지
    ...
    socket.on('borsLightDash', (data) => {
        ...
        if (Date.now() < (p.crowsPullUntil || 0)) return;               // ⚫ 크로우즈 중 봉인
    ...
    socket.on('useSkill', (data) => { 
        ...
        if (Date.now() < (p.crowsPullUntil || 0)) return;               // ⚫ 크로우즈 중 스킬 봉인
    ...
    socket.on('action', (actionData) => {
        ...
        if (Date.now() < (attacker.crowsPullUntil || 0)) return;        // ⚫ 크로우즈 중 평타 봉인
        ...
        if (State.hinbeom.hp > 0 && Math.hypot(actionData.x - State.hinbeom.x, actionData.y - State.hinbeom.y) < pveHitRadius + State.hinbeom.radius) {
            ...
            if (State.hinbeomMinions.length > 0) {
                ...
                recordHinbeomDamage(socket.id, myDamage);                                  // 🏆 피해량 기록
