...
const P_YAMI_BIND_MS   = 2000;                          // 끌어온 뒤 경직 시간
const P_YAMI_TICK_MS   = 500;
const P_YAMI_TICK_DMG  = 25;
const P_YAMI_FX_MS     = 420;                           // 이펙트 표시 시간
...
/** ⛓️ 특정 플레이어와 얽힌 어둠어둠 속박을 모두 정리한다 */
...
function scheduleGuraCharge(attacker, isSuper, delayMs) {
    ...
    // 일반 발동일 때만 0.5초 경직을 건다 (시너지는 어둠어둠 경직이 이미 걸려 있다)
    if (!isSuper) {
...
//    반환값 : 적중 여부 (true 면 흔들흔들 시너지 발동 가능)
// ============================================================================
function firePlayerYami(attacker, dir) {
    let now = Date.now();
...
/** ⛓️ 어둠어둠 속박 진행 처리 (게임 루프에서 매 프레임 호출) */
...
function triggerFruitOnAttack(attacker, dir) {
    ...
    if (yamiReady) {
        let hit = firePlayerYami(attacker, dir);
        if (hit && guraReady) {
