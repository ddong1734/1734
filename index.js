...
//      charLogic/kashimo.js : ⚡ 카시모 반격 · 전하 · 주력 방출 · 환수호박 · 순간이동
//
// 🗣️ [신규] NPC '티치' 대화 · 퀘스트 시스템
...
// ============================================================================
// 🧱 [신규] 세로벽(solid) 목록 — 환수호박 순간이동이 이걸 통과하지 못한다.
//    data.js 의 Platforms 중 solid:true 인 항목과 반드시 동일해야 한다.
//    (가로 발판은 여기 없으므로 순간이동으로 자유롭게 넘을 수 있다)
// ============================================================================
const SOLID_WALLS = [
    // 🧱 중앙 정글 왼쪽 끝 벽
...
const LOCK_FIELDS_RESET = {
    ...
    // ⚡🌋 주력 방출(고정 포함) · ⚡🔮 음파 경직 · 순간이동 쿨타임도 함께 정리한다
...
// ============================================================================
// 🗡️ 평타 처리
//    ⚡🔮 [신규] 환수호박 중인 카시모는 평타 대신 전격 순간이동이 나간다.
// ============================================================================
...
io.on('connection', (socket) => {
    ...
    socket.on('action', (actionData) => {
        ...
        if (attacker.characterType === 'KASHIMO' && attacker.amberActive) {
            Kashimo.tryAmberBlink(attacker, safeData, serverContext);
            return;
        }
