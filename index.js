...
// ============================================================================
// 💥 흔들흔들열매 — 평타를 누르면 0.5초 경직 후 파공아가 터진다
//    · 범위 / 이펙트는 검은수염의 파공아와 완전히 동일 (BB_RADIUS * 3 = 283.5)
//    · ✨ 어둠어둠 크로우즈 적중 시에는 '강화 파공아'로 승격된다
// ============================================================================
const P_GURA_COOLDOWN   = 10000;
const P_GURA_CHARGE_MS  = 500;                          // 시전 경직
const P_GURA_DAMAGE     = 150;
const P_GURA_STUN       = 1000;
const P_GURA_RADIUS     = BB_RADIUS * GURA_RADIUS_MULT; // 283.5

// ✨ 시너지(강화) 파공아
const P_GURA_SUPER_MULT   = 1.3;                           // 범위 1.3배
...
// ============================================================================
// ⛓️ 어둠어둠열매 — 평타를 누르는 즉시 전방으로 크로우즈가 뻗는다
//    · 길이  : 각성 엘 토르 사거리(1680) × 0.3 = 504
//    · 두께  : 각성 엘 토르 두께(90 × 3 = 270) × 0.7 = 189
//    · 이동 시간 없이 즉시 판정 → 범위 내 가장 가까운 적을 곧바로 끌어온다
//    · 끌어온 뒤 2초간 '어둠 흡수' : 대상과 시전자 모두 평타/스킬 사용 불가
//    · ✨ 흔들흔들 시너지로 발동할 때는 도트 피해를 주지 않는다
// ============================================================================
const P_YAMI_COOLDOWN  = 7000;
const P_YAMI_RANGE     = Math.round(1680 * 0.3);        // 504
const P_YAMI_THICKNESS = Math.round(90 * 3 * 0.7);      // 189
const P_YAMI_BIND_MS   = 2000;                          // 끌어온 뒤 흡수(경직) 시간
const P_YAMI_TICK_MS   = 500;
const P_YAMI_TICK_DMG  = 25;
const P_YAMI_FX_MS     = 420;                           // 촉수 이펙트 표시 시간
...
const State = {
    ...
    giantPartisanQueue: [],
    mantleBolts: [], 
    // ⛓️ 어둠어둠열매 크로우즈 흡수 진행 목록
    yamiBinds: [],
    // 💥 흔들흔들열매 파공아 예약 목록 (0.5초 경직 후 발동)
    guraCharges: [],
...
/** 🍈 검은수염 처치 보상 — 흔들흔들 20% · 어둠어둠 20% (각각 독립 판정) */
function tryBlackbeardFruitDrop(attackerId) {
    if (!attackerId) return;
    let p = State.players[attackerId];
    if (!p) return;

    if (Math.random() < BB_GURA_DROP_CHANCE) {
        if (p.inventory.length >= 20) {
            io.to(attackerId).emit('goldenDrop', { msg: '인벤토리가 가득 차 흔들흔들열매를 놓쳤습니다!', fail: true });
        } else {
            p.inventory.push({ uid: Math.random().toString(36).substr(2, 9), id: BB_GURA_DROP_ITEM });
            io.to(attackerId).emit('goldenDrop', { msg: '💥 흔들흔들열매 획득! (평타에 파공아 발동)', inventory: p.inventory });
        }
    }

    if (Math.random() < BB_YAMI_DROP_CHANCE) {
        if (p.inventory.length >= 20) {
            io.to(attackerId).emit('goldenDrop', { msg: '인벤토리가 가득 차 어둠어둠열매를 놓쳤습니다!', fail: true });
        } else {
            p.inventory.push({ uid: Math.random().toString(36).substr(2, 9), id: BB_YAMI_DROP_ITEM });
            io.to(attackerId).emit('goldenDrop', { msg: '⛓️ 어둠어둠열매 획득! (평타에 크로우즈 발동)', inventory: p.inventory });
        }
    }
}

/** 🏆 바제스 처치 보상 — 챔피언 벨트 30% */
function tryBurgessBeltDrop(attackerId) {
    if (!attackerId) return;
    let p = State.players[attackerId];
    if (!p) return;
    if (Math.random() >= BG_BELT_DROP_CHANCE) return;

    if (p.inventory.length >= 20) {
        io.to(attackerId).emit('goldenDrop', { msg: '인벤토리가 가득 차 챔피언 벨트를 놓쳤습니다!', fail: true });
        return;
    }
    p.inventory.push({ uid: Math.random().toString(36).substr(2, 9), id: BG_BELT_DROP_ITEM });
    io.to(attackerId).emit('goldenDrop', { msg: '🏆 챔피언 벨트 획득! (방어력 +20%)', inventory: p.inventory });
}
...
function recalcStats(p) {
    ...
    p.equippedUids.forEach(uid => {
        ...
        if (stats.hasGura) p.hasGura = true; if (stats.hasYami) p.hasYami = true;
        if (item.id === 'seolgonnyak') p.seolgonnyakCount = 1;
    });

    if (p.defense > 0.9) p.defense = 0.9;   // 방어력 상한 (챔피언 벨트 중복 장착 대비)
...
function checkPlayerDeath(targetPlayer, attackerId) {
    if (targetPlayer.hp <= 0 && !targetPlayer.isDead) {
        ...
        targetPlayer.yamiLockUntil = 0; targetPlayer.yamiBindUntil = 0; targetPlayer.guraChargeUntil = 0;
        ...
        clearYamiBindsFor(targetPlayer.id);
        clearGuraChargesFor(targetPlayer.id);
...
/** ⛓️ 특정 플레이어와 얽힌 어둠어둠 흡수를 모두 정리한다 */
function clearYamiBindsFor(pid) {
    for (let i = State.yamiBinds.length - 1; i >= 0; i--) {
        ...
        let tp = State.players[b.targetId];
        if (tp) { tp.yamiBindUntil = 0; }
        let op = State.players[b.ownerId];
        if (op) { op.yamiLockUntil = 0; }
        io.emit('yamiBindEnd', { id: b.targetId });
        io.emit('crowsEnd', { id: b.targetId });
        State.yamiBinds.splice(i, 1);
    }
}

/** 💥 특정 플레이어의 파공아 예약을 취소한다 */
...
//    🍈 [추가] 흔들흔들 / 어둠어둠 열매를 각각 20% 확률로 드롭한다
function killBlackbeard(attackerId) {
    ...
    tryBlackbeardFruitDrop(attackerId);
...
// ============================================================================
// 💥 흔들흔들열매 — 실제 파공아 폭발 처리
//    isSuper 가 true 면 강화 파공아 (범위 1.3배 · 피해 300 · 기절 2초)
// ============================================================================
function explodePlayerGura(attacker, cx, cy, isSuper) {
    let now = Date.now();
    if (!attacker) return;

    const RAD  = isSuper ? P_GURA_SUPER_RADIUS : P_GURA_RADIUS;
    const DMG  = isSuper ? P_GURA_SUPER_DAMAGE : P_GURA_DAMAGE;
    const STUN = isSuper ? P_GURA_SUPER_STUN   : P_GURA_STUN;
    ...
    for (let tid in State.players) {
        ...
        if (Math.hypot(cx - t.x, cy - t.y) > RAD) continue;
        let actual = DMG * (1 - (t.defense || 0));
        t.hp -= actual;
        emitDamageText(t.x, t.y, actual);
        t.frozenUntil = Math.max(t.frozenUntil || 0, now + STUN);
        t.electrocutedUntil = Math.max(t.electrocutedUntil || 0, now + STUN);
        if (t.hp <= 0) checkPlayerDeath(t, attacker.id);
        else { io.to(tid).emit('takeDamage', actual); io.emit('syncPlayerFull', t); }
    }

    // 할배새끼 보스
    if (State.monster.hp > 0 && Math.hypot(cx - State.monster.x, cy - State.monster.y) < RAD + State.monster.radius) {
        State.monster.hp -= DMG; State.monster.targetId = attacker.id; State.monster.state = 'chase';
        State.monster.frozenUntil = Math.max(State.monster.frozenUntil || 0, now + STUN);
        emitDamageText(State.monster.x, State.monster.y, DMG);
        if (State.monster.hp <= 0) killMonster(attacker.id);
    }

    // 박힌범 (할배새끼 생존 시 무적)
    if (State.hinbeom.hp > 0 && State.hinbeom.state !== 'dead' && Math.hypot(cx - State.hinbeom.x, cy - State.hinbeom.y) < RAD + State.hinbeom.radius) {
        if (State.hinbeomMinions.length === 0) {
            State.hinbeom.hp -= DMG;
            recordHinbeomDamage(attacker.id, DMG);
            State.hinbeom.frozenUntil = Math.max(State.hinbeom.frozenUntil || 0, now + STUN);
            emitDamageText(State.hinbeom.x, State.hinbeom.y, DMG);
            aggroHinbeom(attacker.id);
            if (State.hinbeom.hp <= 0) killHinbeom(attacker.id);
        }
    }
    ...
    // 검은수염
    if (State.blackbeard.hp > 0 && State.blackbeard.state !== 'dead' && Math.hypot(cx - State.blackbeard.x, cy - State.blackbeard.y) < RAD + State.blackbeard.radius) {
        State.blackbeard.hp -= DMG;
        State.blackbeard.frozenUntil = Math.max(State.blackbeard.frozenUntil || 0, now + STUN);
        emitDamageText(State.blackbeard.x, State.blackbeard.y, DMG);
        aggroBlackbeard(attacker.id);
        checkBurgessSummon();
        if (State.blackbeard.hp <= 0) killBlackbeard(attacker.id);
    }
    ...
    // 바제스
    if (burgessAlive() && Math.hypot(cx - State.burgess.x, cy - State.burgess.y) < RAD + State.burgess.radius) {
        State.burgess.hp -= DMG;
        State.burgess.frozenUntil = Math.max(State.burgess.frozenUntil || 0, now + STUN);
        emitDamageText(State.burgess.x, State.burgess.y, DMG);
        aggroBurgess(attacker.id);
        if (State.burgess.hp <= 0) killBurgess(attacker.id);
    }
    ...
    for (let i = State.hinbeomMinions.length - 1; i >= 0; i--) {
        let mn = State.hinbeomMinions[i];
        if (mn.hp > 0 && Math.hypot(cx - mn.x, cy - mn.y) < RAD + mn.radius) {
            mn.hp -= DMG; mn.targetId = attacker.id; mn.state = 'chase';
            mn.frozenUntil = Math.max(mn.frozenUntil || 0, now + STUN);
    ...
    // 오크라
    State.okras.forEach(ok => {
        if (ok.hp > 0 && Math.hypot(cx - ok.x, cy - ok.y) < RAD + ok.radius) {
            ok.hp -= DMG; ok.targetId = attacker.id; ok.state = 'chase';
            ok.frozenUntil = Math.max(ok.frozenUntil || 0, now + STUN);
...
function scheduleGuraCharge(attacker, isSuper, delayMs) {
    ...
    attacker.guraCdEnd = now + P_GURA_COOLDOWN;
    io.to(attacker.id).emit('guraCd', { until: attacker.guraCdEnd });

    let delay = (delayMs !== undefined) ? delayMs : P_GURA_CHARGE_MS;

    // ✅ 단독 발동 : 0.5초 경직을 걸고 그 뒤에 터진다
    //    (시너지 발동일 때는 어둠어둠 흡수 경직이 이미 걸려 있으므로 중복으로 걸지 않는다)
    if (!isSuper) {
        attacker.guraChargeUntil = now + delay;
        io.emit('guraCharge', { id: attacker.id, until: attacker.guraChargeUntil });
        io.emit('syncPlayerFull', attacker);
    }

    State.guraCharges.push({
        ownerId: attacker.id,
        fireAt: now + delay,
        isSuper: !!isSuper
    });
...
//    · 시전자 몸 중앙(p.x, p.y)에서 전방으로 뻗는 직사각형 판정
//    · 이동 시간 없이 즉시 가장 가까운 대상을 시전자 옆으로 끌어온다
//    · 끌어온 뒤 2초간 '어둠 흡수' : 대상과 시전자 모두 행동 불가
//    · 대상이 없으면 어둠 잔상 이펙트만 남긴다
//    noDamage 가 true 면 흡수 도트 피해를 주지 않는다 (흔들흔들 시너지용)
//    반환값 : 적중 여부
// ============================================================================
function firePlayerYami(attacker, dir, noDamage) {
    ...
    let d = (dir === -1) ? -1 : 1;
    let half = P_YAMI_THICKNESS / 2;
    let minX = (d === 1) ? attacker.x : attacker.x - P_YAMI_RANGE;
    let maxX = (d === 1) ? attacker.x + P_YAMI_RANGE : attacker.x;
    let minY = attacker.y - half;
    let maxY = attacker.y + half;

    // 쿨타임은 발동 시점에 즉시 소모한다
    attacker.yamiCdEnd = now + P_YAMI_COOLDOWN;
    io.to(attacker.id).emit('yamiCd', { until: attacker.yamiCdEnd });

    // ── 판정 : 범위 안에서 가장 가까운 대상 하나 ─────────────────────
    ...
    const consider = (obj, kind, id, r) => {
        ...
        if (dist < bestDist) { bestDist = dist; best = { obj: obj, kind: kind, id: id }; }
    };
    ...
    if (!best) {
        io.emit('yamiSlash', {
            ...
            x2: attacker.x + d * P_YAMI_RANGE, y2: attacker.y,
            half: half,
            duration: P_YAMI_FX_MS
        });
        return false;
    }

    // ── 적중 : 즉시 시전자 옆으로 끌어온다 ────────────────────────────
    let destX = attacker.x + d * 100;
    let destY = attacker.y;
    let tgt = best.obj;
    ...
    let bindEnd = now + P_YAMI_BIND_MS;
    attacker.yamiLockUntil = bindEnd;
    io.emit('yamiSelfLock', { id: attacker.id, until: bindEnd });

    if (best.kind === 'player') {
        tgt.x = destX; tgt.y = destY;
        tgt.vy = 0; tgt.knockbackForce = 0;
        tgt.isCasting = false;
        ...
        if (best.kind === 'monster') { tgt.targetId = attacker.id; tgt.state = 'chase'; }
        ...
        else if (best.kind === 'minion' || best.kind === 'okra') { tgt.targetId = attacker.id; tgt.state = 'chase'; }
    }
...
/** ⛓️ 어둠어둠 흡수 진행 처리 (게임 루프에서 매 프레임 호출) */
function processYamiBinds(now) {
    for (let i = State.yamiBinds.length - 1; i >= 0; i--) {
        ...
        // 대상 실체를 종류별로 찾아온다
        let tgt = null;
        if (b.targetKind === 'player') tgt = State.players[b.targetId];
        else if (b.targetKind === 'monster') tgt = (State.monster.hp > 0 && State.monster.state !== 'dead') ? State.monster : null;
        else if (b.targetKind === 'hinbeom') tgt = (State.hinbeom.hp > 0 && State.hinbeom.state !== 'dead') ? State.hinbeom : null;
        else if (b.targetKind === 'blackbeard') tgt = (State.blackbeard.hp > 0 && State.blackbeard.state !== 'dead') ? State.blackbeard : null;
        else if (b.targetKind === 'burgess') tgt = burgessAlive() ? State.burgess : null;
        else if (b.targetKind === 'minion') tgt = getMinion(b.targetId);
        else if (b.targetKind === 'okra') tgt = State.okras.find(o => o.id === b.targetId);

        let dead = !tgt || (tgt.hp !== undefined && tgt.hp <= 0) || (b.targetKind === 'player' && tgt.isDead);

        if (dead || !owner || owner.isDead) {
            if (b.targetKind === 'player' && tgt) tgt.yamiBindUntil = 0;
            if (owner) owner.yamiLockUntil = 0;
            io.emit('yamiBindEnd', { id: b.targetId });
            State.yamiBinds.splice(i, 1);
            continue;
        }

        // ✅ 흔들흔들 시너지로 발동한 흡수는 도트 피해를 주지 않는다
        if (!b.noDamage) {
            while (now >= b.nextTick && now < b.endAt) {
                b.nextTick += P_YAMI_TICK_MS;

                if (b.targetKind === 'player') {
                    let actual = P_YAMI_TICK_DMG * (1 - (tgt.defense || 0));
                    tgt.hp -= actual;
                    emitDamageText(tgt.x, tgt.y, actual);
                    if (tgt.hp <= 0) { checkPlayerDeath(tgt, b.ownerId); break; }
                    else io.to(b.targetId).emit('takeDamage', actual);
                } else {
                    tgt.hp -= P_YAMI_TICK_DMG;
                    emitDamageText(tgt.x, tgt.y, P_YAMI_TICK_DMG);
                    if (b.targetKind === 'hinbeom') recordHinbeomDamage(b.ownerId, P_YAMI_TICK_DMG);
                    if (tgt.hp <= 0) {
        ...
        if (now >= b.endAt) {
            if (b.targetKind === 'player' && tgt) { tgt.yamiBindUntil = 0; io.emit('syncPlayerFull', tgt); }
...
/**
 * 🍈 평타 시 열매 능력 통합 처리
 *    · 두 열매가 모두 준비되어 있고 크로우즈가 적중하면 → 시너지 강화 파공아
 *      (이때 어둠어둠 흡수 도트 피해는 발생하지 않는다)
 *    · 그 외에는 각각 독립 발동 (흔들흔들은 0.5초 경직 후)
 */
function triggerFruitOnAttack(attacker, dir) {
    ...
    if (yamiReady) {
        // 시너지 조건이 갖춰져 있으면 흡수 도트 피해를 끄고 발동한다
        let hit = firePlayerYami(attacker, dir, guraReady);
        if (hit && guraReady) {
            // ✨ 시너지 : 끌어온 직후 강화 파공아가 터진다
            scheduleGuraCharge(attacker, true, P_YAMI_FX_MS);
            return;
        }
        if (guraReady) scheduleGuraCharge(attacker, false, P_GURA_CHARGE_MS);
        return;
    }
...
const serverContext = {
    ...
    // 🍈 열매 평타 능력
    triggerFruitOnAttack, firePlayerYami, explodePlayerGura, scheduleGuraCharge,
    processYamiBinds, processGuraCharges, clearYamiBindsFor, clearGuraChargesFor,
    P_GURA_COOLDOWN, P_GURA_CHARGE_MS, P_GURA_DAMAGE, P_GURA_STUN, P_GURA_RADIUS,
    P_GURA_SUPER_RADIUS, P_GURA_SUPER_DAMAGE, P_GURA_SUPER_STUN,
...
function tryReconnect(socket, sessionId) {
    ...
    for (let i = State.yamiBinds.length - 1; i >= 0; i--) {
        let b = State.yamiBinds[i];
        if (b.ownerId === existingOldId || b.targetId === existingOldId) State.yamiBinds.splice(i, 1);
    }
    for (let i = State.guraCharges.length - 1; i >= 0; i--) {
        if (State.guraCharges[i].ownerId === existingOldId) State.guraCharges.splice(i, 1);
    }
...
io.on('connection', (socket) => {
    ...
    socket.on('joinLobby', (data) => {
        ...
        State.players[socket.id] = {
            ...
            hasGura: false, hasYami: false, guraCdEnd: 0, yamiCdEnd: 0, yamiLockUntil: 0, yamiBindUntil: 0, guraChargeUntil: 0,
    ...
    socket.on('borsLightDash', (data) => {
        ...
        if (Date.now() < (p.yamiLockUntil || 0) || Date.now() < (p.yamiBindUntil || 0) || Date.now() < (p.guraChargeUntil || 0)) return;
    ...
    socket.on('useSkill', (data) => { 
        ...
        if (Date.now() < (p.crowsPullUntil || 0)) return;
        // ⛓️ 어둠 흡수 중(시전자·대상 모두) · 💥 파공아 시전 경직 중에는 스킬 사용 불가
        if (Date.now() < (p.yamiLockUntil || 0) || Date.now() < (p.yamiBindUntil || 0) || Date.now() < (p.guraChargeUntil || 0)) return;
    ...
    socket.on('action', (actionData) => {
        ...
        // ⛓️ 어둠 흡수 중(시전자·대상 모두) · 💥 파공아 시전 경직 중에는 평타 사용 불가
        if (Date.now() < (attacker.yamiLockUntil || 0) || Date.now() < (attacker.yamiBindUntil || 0) || Date.now() < (attacker.guraChargeUntil || 0)) return;
        ...
        if (actionData.type === 'thunder_bolt') {
            ...
            // 🍈 열매 평타 능력 (에넬 평타에도 동일 적용)
            triggerFruitOnAttack(attacker, dir);
            return;
        }
        ...
        triggerFruitOnAttack(attacker, actionData.dir || attacker.lastFacing || 1);
    });
    ...
    socket.on('disconnect', () => { 
        ...
        clearYamiBindsFor(socket.id);
        clearGuraChargesFor(socket.id);
        if (State.blackbeard) {
