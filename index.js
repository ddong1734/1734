...
//      charLogic/kashimo.js : ⚡ 카시모 반격 전류 · 전하 스택 · 주력 방출
// ============================================================================
...
// ⚡ 카시모 고유 특성 모듈 (반격 전류 · 전하 스택 · 주력 방출)
...
//
// ⚡ 카시모 하지메 연동 두 가지
//    ① 반격 전류 : 공격자가 '카시모를 평타로 때렸을 때' 공격자가 50 피해를 되받는다.
//                  (공격자가 에넬 · 카시모면 면제)
//    ② 전하 스택 : 공격자가 '카시모일 때' 맞은 대상 모두에게 전하가 1칸씩 쌓인다.
// ============================================================================
function handleBasicAttack(socket, attacker, actionData) {
    ...
    let isKashimo = charType === 'KASHIMO';          // ⚡ 공격자가 카시모인가
    ...
    //    같은 평타로 카시모 여러 명을 때려도 반격은 1회만 발동한다.
    ...
    for (let tid in State.players) {
        ...
        // 🔌 맞은 쪽이 카시모라면 공격자에게 반격 전류
        tryCounter(t);
    }

    // 🔌 [보강] 사거리 판정이 서로 달라 위 루프에서 놓친 경우를 대비해,
    //    가까이 있는 카시모에게도 반격 기회를 한 번 더 준다.
    if (!counterFired) {
        for (let tid in State.players) {
            if (tid === socket.id) continue;
            let t = State.players[tid];
