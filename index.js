...
//     (플레이어는 바닥 위 y = -1445 에 서고, 2단 점프로도 벽을 못 넘는다)
//
//  ⚙️ 조정 가능한 값들
// ============================================================================
...
//    기존에는 객체의 모든 키를 순회하며 비교했는데, inventory / equippedUids 같은 배열은
//    structuredClone 으로 복사될 때마다 '새 참조'가 되어 !== 비교가 항상 true 였다.
//    → 매 프레임 인벤토리 배열 전체가 델타에 실려 전 클라이언트로 브로드캐스트되던 문제를 제거한다.
...
class DeltaCompressor {
    ...
    getDelta(id, currentData) {
        ...
        // 최초 1회는 기존과 동일하게 전체 객체를 보낸다 (닉네임/팀/구체 수 등 초기 정보 포함)
        if (!last) {
            last = {};
...
function makeHinbeom() {
    return {
        ...
        hakiNextRoll: 0,          // 다음 7% 판정 시각
        hakiBursts: [],           // 동시에 여러 개가 겹칠 수 있다 (중복 방출 허용)
        hakiActiveUntil: 0        // 클라이언트 표시용 (가장 늦게 끝나는 패기)
    };
}
...
const serverContext = {
    ...
    HINBEOM_AREA, HAKI_CHANCE, HAKI_ROLL_MS, HAKI_DURATION, HAKI_TICK_MS, HAKI_TICK_DMG, HAKI_TICKS
};
