...
function recalcStats(p) {
    ...
    if (p.defense > 0.9) p.defense = 0.9;   // 방어력 상한 (챔피언 벨트 중복 대비)
...
io.on('connection', (socket) => {
    ...
    socket.on('useSkill', (data) => {
        ...
        if (logic && logic.useSkill) logic.useSkill(p, data, serverContext);
    });
    ...
    socket.on('landSkill1', (data) => {
        ...
        if (!p || p.isDead) return;
        let logic = CharLogic[p.characterType];
        if (logic && logic.landSkill1) logic.landSkill1(p, data, serverContext);
    });
    ...
    socket.on('action', (actionData) => {
        ...
        io.emit('actionEffect', {
            id: socket.id, type: actionData.type, x: actionData.x, y: actionData.y,
            dir: actionData.dir, life: actionData.lifeFrames, maxLife: actionData.lifeFrames
        });
        ...
        handleBasicAttack(socket, attacker, actionData);
        // 🍈 열매 평타 능력 — 준비된 열매가 있으면 자동 발동
        Fruits.triggerFruitOnAttack(attacker, actionData.dir || attacker.lastFacing || 1);
    });
