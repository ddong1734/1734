...
// 🔥 [마르코] 재생 게이지 자동 집계
//
//   받은 피해를 게이지에 쌓아야 하는데, 피해를 주는 코드가 22곳에 흩어져 있다.
//   전부 고치면 빠뜨리기 쉬우므로, 모든 경로가 반드시 거치는
//   'syncPlayerFull' 방송을 가로채 체력이 줄어든 만큼만 자동으로 쌓는다.
//   (회복은 무시한다)
// ============================================================================
const _marcoHpSeen = new Map();
const _rawEmit = io.emit.bind(io);
io.emit = function (event, data) {
    try {
        if (event === 'syncPlayerFull' && data && data.id && data.characterType === 'MARCO') {
            const prev = _marcoHpSeen.get(data.id);
            if (prev !== undefined && data.hp < prev) {
                const lost = prev - data.hp;
                const M = CharLogic && CharLogic.MARCO;
                if (M && typeof M.addGauge === 'function' && lost > 0) {
                    M.addGaugeQuiet(data, lost);
                }
            }
            _marcoHpSeen.set(data.id, data.hp);
        }
    } catch (e) { /* 집계 실패가 방송을 막아선 안 된다 */ }
    return _rawEmit(event, data);
};
...
const { addBeopjinKill, recalcStats, applyBaseDamage, checkPlayerDeath, resetGame, tryReconnect, clampSpecialArea } = PlayerCore;
...
const deferred = {
    ...
    // 🔥 헤이안 스쿠나
    killSukuna:   (...a) => Bosses.killSukuna(...a),
    aggroSukuna:  (...a) => Bosses.aggroSukuna(...a),
...
const serverContext = Object.assign({
    ...
    get tryDomainDarkSkill() { return Combat.tryDomainDarkSkill; },
    get processDomainDark() { return Combat.processDomainDark; },
    get tryDomainKickSkill() { return Combat.tryDomainKickSkill; },
    get processDomainKick() { return Combat.processDomainKick; },
    ...
    // ⬛ 다부라 고유 특성
    Dabura,
    // 🕊️ 쿠루스 하나 — 신성력 · 축복 회복 · 야곱의 사다리
    kurusuProcessAll: (now) => CharLogic.KURUSU.processAll(now, serverContext),
    // 🔥 마르코 — 재생 · 불꽃 덩어리 · 불길 · 보호막
    ...
    marcoBlocked: (targetObj, attackerTeam) => {
        try {
            ...
            for (let sid in State.players) {
                ...
                if (attackerTeam !== undefined && sp.team === attackerTeam) continue;
                if (Math.hypot(targetObj.x - sp.marcoShieldX, targetObj.y - sp.marcoShieldY) <= 210) {
                    io.emit('marcoShieldHit', { x: targetObj.x, y: targetObj.y, sx: sp.marcoShieldX, sy: sp.marcoShieldY });
                    return true;
                }
            }
        } catch (e) { /* 판정 실패가 게임을 막아선 안 된다 */ }
        return false;
    }
...
const wireBag = {
    ...
    makeMonster, makeBases, makeTurrets, makePlayer, makeNpcs,
    makeHinbeom, makeBlackbeard, makeBurgess, makeSukuna,
    // 모듈 간 상호 참조
    ...
    isDaidoLocked: Combat.isDaidoLocked
};
