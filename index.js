...
const P_YAMI_PULL_MS   = 300;                           // 끌어오는 데 걸리는 시간
const P_YAMI_BIND_MS   = 2000;                          // 끌려온 뒤 경직 시간
...
const PLAYER_DELTA_FIELDS = [
    ...
    'elThorActive', 'yataActive', 'crowsPullUntil', 'yamiLockUntil', 'yamiBindUntil', 'darkBanned'
];
...
function checkPlayerDeath(targetPlayer, attackerId) {
    if (targetPlayer.hp <= 0 && !targetPlayer.isDead) {
        ...
        targetPlayer.yamiLockUntil = 0; targetPlayer.yamiBindUntil = 0;
...
// ============================================================================
// 💥 흔들흔들열매 — 평타에 실리는 파공아
//    검은수염의 파공아와 동일한 반경 / 이펙트를 사용한다.
// ============================================================================
function firePlayerGura(attacker, cx, cy) {
    let now = Date.now();
    if (!attacker || !attacker.hasGura) return;
    if (now < (attacker.guraCdEnd || 0)) return;

    attacker.guraCdEnd = now + P_GURA_COOLDOWN;
    io.to(attacker.id).emit('guraCd', { until: attacker.guraCdEnd });

    io.emit('playerGura', { x: cx, y: cy, radius: P_GURA_RADIUS });
    ...
    for (let tid in State.players) {
        ...
        if (Math.hypot(cx - t.x, cy - t.y) > P_GURA_RADIUS) continue;
        let actual = P_GURA_DAMAGE * (1 - (t.defense || 0));
        t.hp -= actual;
        emitDamageText(t.x, t.y, actual);
        t.frozenUntil = Math.max(t.frozenUntil || 0, now + P_GURA_STUN);
        t.electrocutedUntil = Math.max(t.electrocutedUntil || 0, now + P_GURA_STUN);
        if (t.hp <= 0) checkPlayerDeath(t, attacker.id);
        else { io.to(tid).emit('takeDamage', actual); io.emit('syncPlayerFull', t); }
    }

    // 할배새끼 보스
    if (State.monster.hp > 0 && Math.hypot(cx - State.monster.x, cy - State.monster.y) < P_GURA_RADIUS + State.monster.radius) {
        State.monster.hp -= P_GURA_DAMAGE; State.monster.targetId = attacker.id; State.monster.state = 'chase';
        State.monster.frozenUntil = Math.max(State.monster.frozenUntil || 0, now + P_GURA_STUN);
        emitDamageText(State.monster.x, State.monster.y, P_GURA_DAMAGE);
        if (State.monster.hp <= 0) killMonster(attacker.id);
    }

    // 박힌범 (할배새끼 생존 시 무적)
    if (State.hinbeom.hp > 0 && State.hinbeom.state !== 'dead' && Math.hypot(cx - State.hinbeom.x, cy - State.hinbeom.y) < P_GURA_RADIUS + State.hinbeom.radius) {
        if (State.hinbeomMinions.length === 0) {
            State.hinbeom.hp -= P_GURA_DAMAGE;
            recordHinbeomDamage(attacker.id, P_GURA_DAMAGE);
            State.hinbeom.frozenUntil = Math.max(State.hinbeom.frozenUntil || 0, now + P_GURA_STUN);
            emitDamageText(State.hinbeom.x, State.hinbeom.y, P_GURA_DAMAGE);
            aggroHinbeom(attacker.id);
            if (State.hinbeom.hp <= 0) killHinbeom(attacker.id);
        }
    }
    ...
    // 검은수염
    if (State.blackbeard.hp > 0 && State.blackbeard.state !== 'dead' && Math.hypot(cx - State.blackbeard.x, cy - State.blackbeard.y) < P_GURA_RADIUS + State.blackbeard.radius) {
        State.blackbeard.hp -= P_GURA_DAMAGE;
        State.blackbeard.frozenUntil = Math.max(State.blackbeard.frozenUntil || 0, now + P_GURA_STUN);
        emitDamageText(State.blackbeard.x, State.blackbeard.y, P_GURA_DAMAGE);
        aggroBlackbeard(attacker.id);
        checkBurgessSummon();
        if (State.blackbeard.hp <= 0) killBlackbeard(attacker.id);
    }
    ...
    // 바제스
    if (burgessAlive() && Math.hypot(cx - State.burgess.x, cy - State.burgess.y) < P_GURA_RADIUS + State.burgess.radius) {
        State.burgess.hp -= P_GURA_DAMAGE;
        State.burgess.frozenUntil = Math.max(State.burgess.frozenUntil || 0, now + P_GURA_STUN);
        emitDamageText(State.burgess.x, State.burgess.y, P_GURA_DAMAGE);
        aggroBurgess(attacker.id);
        if (State.burgess.hp <= 0) killBurgess(attacker.id);
    }
    ...
    for (let i = State.hinbeomMinions.length - 1; i >= 0; i--) {
        let mn = State.hinbeomMinions[i];
        if (mn.hp > 0 && Math.hypot(cx - mn.x, cy - mn.y) < P_GURA_RADIUS + mn.radius) {
            mn.hp -= P_GURA_DAMAGE; mn.targetId = attacker.id; mn.state = 'chase';
            mn.frozenUntil = Math.max(mn.frozenUntil || 0, now + P_GURA_STUN);
            emitDamageText(mn.x, mn.y, P_GURA_DAMAGE);
            if (mn.hp <= 0) killMinion(mn, attacker.id);
        }
    }
    ...
    // 오크라
    State.okras.forEach(ok => {
        if (ok.hp > 0 && Math.hypot(cx - ok.x, cy - ok.y) < P_GURA_RADIUS + ok.radius) {
            ok.hp -= P_GURA_DAMAGE; ok.targetId = attacker.id; ok.state = 'chase';
            ok.frozenUntil = Math.max(ok.frozenUntil || 0, now + P_GURA_STUN);
            emitDamageText(ok.x, ok.y, P_GURA_DAMAGE);
            if (ok.hp <= 0) killOkra(ok, attacker.id);
        }
    });
}
...
// ============================================================================
// ⛓️ 어둠어둠열매 — 평타에 실리는 전방 크로우즈
//    전방 직사각형(길이 504 · 두께 189) 안의 가장 가까운 적 하나를 끌어온다.
// ============================================================================
function firePlayerYami(attacker, dir) {
    let now = Date.now();
    if (!attacker || !attacker.hasYami) return;
    if (now < (attacker.yamiCdEnd || 0)) return;
    if (now < (attacker.yamiLockUntil || 0)) return;
    ...
    // 조준선 안의 가장 가까운 적 플레이어를 찾는다
    let tgt = null, tgtId = null, closest = Infinity;
    for (let tid in State.players) {
        if (tid === attacker.id) continue;
        let t = State.players[tid];
        if (!t || t.isDead || t.team === attacker.team) continue;
        if (t.x < minX || t.x > maxX || t.y < minY || t.y > maxY) continue;
        let dist = Math.hypot(t.x - attacker.x, t.y - attacker.y);
        if (dist < closest) { closest = dist; tgt = t; tgtId = tid; }
    }

    // 빔 이펙트는 대상 유무와 상관없이 항상 보여준다
    let endX = attacker.x + d * P_YAMI_RANGE;
    let endY = attacker.y;
    io.emit('playerYamiBeam', {
        x: attacker.x, y: attacker.y,
        x2: tgt ? tgt.x : endX, y2: tgt ? tgt.y : endY,
        targetId: tgtId,
        duration: P_YAMI_PULL_MS
    });
    ...
    // 대상을 시전자 바로 옆으로 끌어온다
    let destX = attacker.x + d * 90;
    let destY = attacker.y;

    tgt.crowsPullUntil = now + P_YAMI_PULL_MS;
    ...
    // 끌려온 뒤 2초 속박 + 0.5초마다 25 피해 / 시전자도 같은 시간 경직
    let bindEnd = now + P_YAMI_PULL_MS + P_YAMI_BIND_MS;
    ...
    State.yamiBinds.push({
        ownerId: attacker.id,
        targetId: tgtId,
        startAt: now + P_YAMI_PULL_MS,
        endAt: bindEnd,
        nextTick: now + P_YAMI_PULL_MS + P_YAMI_TICK_MS
    });

    io.emit('yamiSelfLock', { id: attacker.id, until: bindEnd });
    io.emit('yamiBind', { id: tgtId, until: bindEnd });
...
function processYamiBinds(now) {
    for (let i = State.yamiBinds.length - 1; i >= 0; i--) {
        ...
        let tgt = State.players[b.targetId];

        if (!tgt || tgt.isDead || !owner || owner.isDead) {
            if (tgt) tgt.yamiBindUntil = 0;
        ...
        // 속박 중에는 대상을 시전자 옆에 붙잡아 둔다
        if (now < b.endAt && !tgt.isDead) {
            tgt.crowsPullUntil = 0;
        }
...
const serverContext = {
    ...
    // 🍈 열매 평타 능력
    firePlayerGura, firePlayerYami, processYamiBinds, clearYamiBindsFor,
    P_GURA_COOLDOWN, P_GURA_DAMAGE, P_GURA_STUN, P_GURA_RADIUS,
    P_YAMI_COOLDOWN, P_YAMI_RANGE, P_YAMI_THICKNESS, P_YAMI_PULL_MS, P_YAMI_BIND_MS, P_YAMI_TICK_MS, P_YAMI_TICK_DMG,
    // 🟪 지저스 바제스
...
function tryReconnect(socket, sessionId) {
    ...
    p.crowsPullUntil = 0;
    p.yamiLockUntil = 0; p.yamiBindUntil = 0;
...
io.on('connection', (socket) => {
    ...
    socket.on('joinLobby', (data) => {
        ...
        State.players[socket.id] = {
            ...
            hasGura: false, hasYami: false, guraCdEnd: 0, yamiCdEnd: 0, yamiLockUntil: 0, yamiBindUntil: 0,
    ...
    socket.on('borsLightDash', (data) => {
        ...
        if (Date.now() < (p.yamiLockUntil || 0) || Date.now() < (p.yamiBindUntil || 0)) return;
    ...
    socket.on('action', (actionData) => {
        ...
        if (Date.now() < (attacker.yamiLockUntil || 0) || Date.now() < (attacker.yamiBindUntil || 0)) return;
        ...
        if (actionData.type === 'thunder_bolt') {
            ...
            firePlayerGura(attacker, actionData.x, actionData.y);
            firePlayerYami(attacker, dir);
            return;
        }
        ...
        // 🍈 열매 평타 능력 — 쿨타임이 돌아왔다면 자동 발동
        firePlayerGura(attacker, actionData.x, actionData.y);
        firePlayerYami(attacker, actionData.dir || attacker.lastFacing || 1);
    });
