...
//      charLogic/kashimo.js : ⚡ 카시모 반격 · 전하 · 주력 방출 · 환수호박
// ============================================================================
...
function recalcStats(p) {
    ...
    // ⚡🔮 환수호박 발동 중이라면 이동속도 1.7배를 다시 곱해 준다.
    //    (아이템 장착 등으로 recalcStats 가 다시 불려도 배율이 유지되어야 한다)
    if (p.amberActive) {
...
function processBurns(now) {
    for (let [key, stacks] of State.burnMap) {
        ...
        for (let i = stacks.length - 1; i >= 0; i--) {
            let b = stacks[i];
            // 🛟 틱 타임스탬프가 어긋나도 루프가 폭주하지 않도록 상한을 둔다
            let guard = 0;
...
const LOCK_FIELDS_RESET = {
    ...
    // ⚡🌋 주력 방출 · ⚡🔮 음파 경직도 함께 정리한다
    surgeActive: false, surgeEnd: 0, surgeNextTick: 0,
...
function checkPlayerDeath(targetPlayer, attackerId) {
    ...
    setTimeout(() => {
        try {
            ...
            targetPlayer.kashimoCharge = 0; targetPlayer.kashimoChargeUntil = 0;
            // ⚡🔮 부활 시 환수호박은 완전히 해제된 상태 (3번 스킬 다시 사용 가능)
...
function resetGame() {
    ...
    // ⚡🔮 환수호박 잔상 · 연쇄 폭발 초기화
    State.amberTrails = []; State.waveChains = [];
...
function tryReconnect(socket, sessionId) {
    ...
    // ⚡🔮 환수호박은 재접속해도 유지된다 (죽어야만 풀린다)
    //    다만 잔상 소유자 id 를 새 소켓 id 로 옮긴다
...
function handleBasicAttack(socket, attacker, actionData) {
    ...
    // 🔌 카시모에게 평타를 맞혔다면, 공격자가 전류 반격을 되받는다.
...
io.on('connection', (socket) => {
    ...
    socket.on('playerMove', (data) => {
        ...
        // ⚡🔮 음파 경직 중에는 서버 좌표를 갱신하지 않는다 (완전 고정)
    ...
    socket.on('useSkill', (data) => {
        ...
        if (Fruits.isActionLocked(p)) return;
        // ⚡🔮 음파 경직 중에는 스킬 사용 불가
        if (Date.now() < (p.sonicChargeUntil || 0)) return;
    ...
    socket.on('action', (actionData) => {
        ...
        // ⚡🔮 음파 경직 중에는 평타 사용 불가
        if (Date.now() < (attacker.sonicChargeUntil || 0)) return;
    ...
    socket.on('disconnect', () => {
        // ⚡🌋 방출 중이었다면 클라이언트 이펙트를 끈다
        let dp = State.players[socket.id];
        if (dp && dp.surgeActive) {
            dp.surgeActive = false; dp.surgeEnd = 0; dp.surgeNextTick = 0;
            io.emit('kashimoSurgeEnd', { id: socket.id });
        }
        // ⚡🔮 접속이 끊기면 잔상 · 연쇄 폭발은 정리한다 (환수호박 상태 자체는 유지)
        for (let i = State.amberTrails.length - 1; i >= 0; i--) {
