...
const HINBEOM_XP      = 500 * 2;              // 처치 보상 경험치 2배 (1000)
...
// ✅ [수정] 쿨다운이 길어 재진입 시 카운트다운이 계속 초기화되던 문제 → 300ms 로 축소
...
// ⚫ 검은수염 (암흑 왕좌 전용 보스)
// ----------------------------------------------------------------------------
//  ✅ [수정] 상공(-4000) → 지상과 동일한 높이(y=2000)로 내리고 맵 최우측으로 이동.
//     유성화산(spawnY -2900) · 만뢰 · 뇌영 등 하늘에서 떨어지는 스킬이 정상 표시된다.
//    바닥 : { x: 28000, y: 2000, w: 5000, dark }
//    좌벽 : { x: 28000, y: 1100, h: 900, solid, dark }
//    우벽 : { x: 32960, y: 1100, h: 900, solid, dark }
// ============================================================================
const DARK_AREA   = { minX: 28000, maxX: 33000, minY: 600, maxY: 2060 };
const DARK_GROUND = 2000;                      // 암흑 왕좌 바닥 (지상과 동일 높이)
const DARK_ENTRY_X = 30500;                    // 포탈로 진입했을 때 소환되는 위치
...
const BB_SPEED  = 1.75;
const BB_HOME_X = 30500;                       // 암흑 왕좌 정중앙
...
const DARKFLOOR_DURATION = 4000;               // 시전시간 4초
...
const DESCENT_RISE      = 420;                 // 플레이어 화면에 보이는 높이
const DESCENT_ASCEND_MS = 2000;                // 어둠 소용돌이로 솟구쳐 오르는 시간(2초)
...
/**
 * ⛓️ [신규] 크로우즈 조준선(빨간 표시) 안에 대상이 들어와 있는가
 *    시전 시점의 조준 방향을 기준으로 한 '띠(band)' 판정.
 *    → 빨간 표시에서 벗어나면 절대 맞지 않는다.
 */
function isInCrowsBeam(bb, t) {
    ...
    let s = rx * ux + ry * uy;                      // 진행 방향 거리
    let d = Math.abs(rx * (-uy) + ry * ux);         // 중심선으로부터의 수직 거리
...
function makeBlackbeard() {
    return {
        ...
        crowsAimUX: 1, crowsAimUY: 0,   // ✅ 조준 방향 단위벡터 (빨간 표시 판정용)
...
const serverContext = {
    ...
    MINION_EVERY,
    // ⚫ 검은수염
    killBlackbeard, aggroBlackbeard, isInDarkArea, clearBlackbeardSkills, isInCrowsBeam,
