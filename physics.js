...
// 🗺️ [수정] 암흑 왕좌가 맵 최우측(x 28000~33000)으로 이동함에 따라
//    WORLD_WIDTH 를 최소 34000 으로 자동 확장한다 (data.js 값이 작아도 안전)

window.initPhysicsTerrain = () => {
    if (!window.GameData || !window.GameData.Map) return;
    
    // 🗺️ 암흑 왕좌(최대 x=33000)까지 담을 수 있도록 월드 폭을 보정한다
    let baseWidth = window.GameData.Map.WORLD_WIDTH || 32000;
    window.WORLD_WIDTH = Math.max(baseWidth, 34000);
    ...
    blueData.forEach(pos => {
        ...
        // 정글 발판 미러링은 '원래 맵 폭' 기준으로 유지한다 (확장된 폭을 쓰면 위치가 어긋난다)
        window.PLATFORMS.push({ x: baseWidth - pos.x - 1000, y: pos.y, w: 1000, h: 30, type: 'jungle' }); 
    });
};
...
window.updatePlayerPhysics = (p, slowUntil, loopNow, visualFX) => {
    ...
    if (p.characterType === 'BORSALINO' && loopNow < (p.lightDashUntil || 0)) {
        ...
        let wWidthDash = window.WORLD_WIDTH || 34000;
    ...
    let wWidth = window.WORLD_WIDTH || 34000;
