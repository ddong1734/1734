...
//
// 🛟 [핵심 수정 — 게임 멈춤 현상]
//   원인 : 잠금 상태(isCasting / elThorLockUntil / skill1Dashing / yataActive 등)가
//          한 번 굳으면 아래 조기 return 들에 영원히 걸려 캐릭터가 못 움직였다.
//          카메라는 내 좌표를 따라가므로 화면까지 멈춘 것처럼 보인다.
//   해결 : ① 모든 조기 return 구간에 '만료 시각' 조건을 함께 검사한다.
//             근거가 사라지면 그 자리에서 잠금을 풀고 정상 물리로 넘어간다.
//          ② 좌표가 NaN 으로 오염되면 즉시 복구한다 (한 번 오염되면 모든 비교가
//             false 가 되어 캐릭터가 영영 화면에서 사라진다).
//          ③ 야타 경로가 비었거나 시전 시간이 지나면 강제로 종료한다.
...
window.updatePlayerPhysics = (p, slowUntil, loopNow, visualFX) => {
    ...
    if (p.yataActive) {
        ...
        if (!p.yataPath || p.yataPath.length < 2 || elapsed > dur + 2000) {
            ...
            // ⚫ [추가] 암흑 왕좌 안이라면 절대 구역 밖/바닥 아래로 나가지 않는다
    ...
    //    castLockUntil 이 지났는데도 isCasting 이 true 로 남아 있으면
    //    (= 해제 setTimeout 이 유실된 경우) 그 자리에서 풀고 정상 물리로 넘어간다.
