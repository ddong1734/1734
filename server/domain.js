...
function openDomain(p) {
    ...
    // ⚡ 전개되는 동안 범위 안의 모든 대상을 경직시킨다.
    //    시전자 본인도 함께 묶인다 (감전 표시).
    //    ⚠️ 경직은 3초(YUM_STUN_MS)로, 전개 연출 4초보다 짧다.
    dm.stunEnd = now + C.YUM_STUN_MS;
    freezeInside(dm, dm.stunEnd);
}
...
function processDomains(now) {
    ...
    for (let i = State.domains.length - 1; i >= 0; i--) {
        ...
        if (dm.phase === 'expand') {
            // ② 전개 중 — 경직이 끝나기 전에 들어온 대상도 함께 묶는다
            if (now < (dm.stunEnd || 0)) freezeInside(dm, dm.stunEnd);
            if (now >= dm.expandEnd) {
                dm.phase = 'active';
                changed = true;
            }
            ...
            if (now >= dm.endAt) {
                // 🌑 [수정] 주인의 영역 스킬이 아직 시전 중이라면 끝날 때까지 유지한다.
                //    (지속시간 15초가 끝나도 스킬이 잘리지 않게 한다)
                let busyUntil = ownerSkillBusyUntil(owner);
                if (busyUntil > now) {
                    dm.endAt = busyUntil + 200;
                    dm.collapseEnd = dm.endAt + C.YUM_COLLAPSE_MS;
                    changed = true;
                } else {
                    dm.phase = 'collapse';
...
/**
 * 🌑 주인의 영역 전용 스킬이 언제까지 진행 중인가.
 *    지속시간 15초가 끝나도 시전 중인 스킬이 있으면 그때까지 영역을 유지한다.
 *    @return 진행 중이면 종료 시각(ms), 아니면 0
 */
function ownerSkillBusyUntil(p) {
    if (!p) return 0;
    let until = 0;
    const bump = (v) => { if (v && v > until) until = v; };

    bump(p.domLightEnd);        // ☀️ 영역 [빛] — 수렴 + 3초 폭격
    bump(p.domDarkStrikeAt);    // 🌑 영역 [어둠] — 2초 변환 후 타격
    bump(p.domKickEnd);         // 💫 영역 [아광속 발차기] — 경직 + 5초 비행
    bump(p.domKickChargeEnd);
