...
function resolveBurnKey(key) {
    ...
    if (key === 'sukuna') return { e: State.sukuna, kind: 'sukuna' };   // 🔥 헤이안 스쿠나
...
function processBurns(now) {
    for (let [key, stacks] of State.burnMap) {
        ...
        if (kind === 'sukuna' && (!State.sukuna || State.sukuna.state === 'dead')) continue;   // 🔥
        ...
        if (entity.hp <= 0) {
            ...
            else if (kind === 'sukuna' && Bosses.killSukuna) Bosses.killSukuna(lastOwner);   // 🔥
...
function handleBasicAttack(socket, attacker, actionData) {
    ...
    let now = Date.now();
    let myDamage = attacker.baseDamage + attacker.bonusDamage;
    // 🗿 [키드] 골렘 평타는 1.5배 (배율은 아래 hitRadius 계산과 짝을 이룬다)
    if (attacker.kidGolemEnd && Date.now() < attacker.kidGolemEnd) myDamage = Math.round(myDamage * 1.5);
    ...
    // 🗿 [키드] 고철 골렘 상태 — 평타 피해 1.5배 · 범위 2.6배
    //    (팔을 크게 휘두르므로 판정도 함께 커진다)
    const kidGolem = !!(attacker.kidGolemEnd && Date.now() < attacker.kidGolemEnd);
    // 🗿 골렘은 보이는 크기만큼 히트박스도 커진다 (radius 는 physics 가 쓴다)
    if (kidGolem) {
        hitRadius = Math.round(hitRadius * 2.0);
        pveHitRadius = Math.round(pveHitRadius * 2.0);
    }
...
/** ⚔️ 다이도가 스킬 시전 중이라 다른 행동을 할 수 없는가 */
function isDaidoLocked(p, now) {
    if (!p) return false;
    if (p.daidoRush && now < (p.daidoRushEnd || 0)) return true;
    if (p.daidoFury && now < (p.daidoFuryEnd || 0)) return true;
    if (p.daidoIaiAt && now < p.daidoIaiAt) return true;
    return false;
}
...
module.exports = {
    ...
    handleBasicAttack, sanitizeActionData, isDaburaLocked, isDaidoLocked
};
