...
function shieldAt(x, y, team) {
    ...
    for (const sid in State.players) {
        ...
        const dx = x - sp.marcoShieldX, dy = y - sp.marcoShieldY;
...
function pushOutOfShield(o, team) {
    ...
    if (!sp) return false;
    const RX = 120, RY = (sp.marcoShieldRY || 260);
    let dx = o.x - sp.marcoShieldX, dy = o.y - sp.marcoShieldY;
    if (dx === 0 && dy === 0) { dx = 1; dy = 0; }
    const n = (dx * dx) / (RX * RX) + (dy * dy) / (RY * RY);
    const k = 1 / Math.sqrt(Math.max(n, 0.0001));
    o.x = sp.marcoShieldX + dx * k * 1.06;
    o.y = sp.marcoShieldY + dy * k * 1.06;
    o.knockbackForce = 0;
    return true;
}
