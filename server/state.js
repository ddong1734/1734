...
/** 🗣️ [신규] NPC 목록 생성 (각 팀 정글 상단 발판 중앙에 티치 한 명씩) */
function makeNpcs() {
    return C.NPC_TICH.map(n => ({
...
function makePlayer(opts) {
    ...
    return Object.assign({
        ...
        // ⚡ 카시모 하지메 전용 상태
        kashimoBoltCdEnd: 0,
        // ⚡🌋 주력 방출 (2번 스킬) — surgeLockUntil 동안 완전히 고정된다
        ...
        amberLastTrailX: 0, amberLastTrailY: 0,
        // ⚡🔮 [신규] 환수호박 평타 = 전격 순간이동
        blinkCdEnd: 0,
        // ⚡🔮 환수호박 전용 스킬 쿨타임 / 경직
        waveCdEnd: 0,
...
const State = {
    ...
    // ⚡🔮 환수호박 전기 잔상 · 전자파 연쇄 폭발 예약
    amberTrails: [], waveChains: [],
    // ⚡🌩️ [신규] 뇌신 전자파 재폭발(에코) 예약
    waveEchoes: [],
    bases: makeBases(),
    monster: makeMonster(),
    hinbeom: makeHinbeom(),
