...
module.exports = (deps) => {
    ...
    function killBurgess(attackerId) {
        ...
        if (attackerId && State.players[attackerId] && Math.random() < 0.35) {
            giveItem(attackerId, 'golden_belt',
                '🟡 황금 벨트 획득!',
                '인벤토리가 가득 차 황금 벨트를 놓쳤습니다!');
        }
    ...
    function rerollOkraGrade(ok) {
        ok.isHaeru = false;
        if (ok.isEliteGolden) { ok.isGolden = true; ok.maxHp = 2000; ok.hp = ok.maxHp; }
        else {
    ...
    function tryGoldenDrop(ok, attackerId) {
        ...
        if (ok.isHaeru) {
            if (Math.random() < 0.25) {
                giveItem(attackerId, 'haeru',
                    '💎 해루석 획득!',
                    '인벤토리가 가득 차 해루석을 놓쳤습니다!');
            }
            return;
        }
    ...
    function spawnOkra(x, y, opts) {
        let ok = Object.assign({
            ...
            isGolden: false, isHaeru: false, hp: 700, maxHp: 700, atk: 30, speed: 1,
    ...
    function killSukuna(attackerId) {
        ...
        if (attackerId && State.players[attackerId] && Math.random() < 0.50) {
            giveItem(attackerId, 'sukuna_finger',
                '🔥 스쿠나의 손가락 획득!',
                '인벤토리가 가득 차 스쿠나의 손가락을 놓쳤습니다!');
        }
        // 😈 30% 확률로 '타천' 을 준다 (손가락과 별개 판정)
        if (attackerId && State.players[attackerId] && Math.random() < 0.30) {
            giveItem(attackerId, 'tacheon',
                '😈 타천 획득!',
                '인벤토리가 가득 차 타천을 놓쳤습니다!');
        }
