...
module.exports = (deps) => {
    const { io, emitDamageText, checkPlayerDeath, applyShockBlast,
            ...
            aggroHinbeom, aggroBlackbeard, aggroBurgess, recordHinbeomDamage,
            killSukuna, aggroSukuna } = deps;   // 🔥 헤이안 스쿠나
    ...
    function resolveTarget(kind, id) {
        ...
        if (kind === 'sukuna') return (State.sukuna && State.sukuna.hp > 0 && State.sukuna.state !== 'dead') ? State.sukuna : null;   // 🔥
    ...
    function firePlayerYami(attacker, dir, noDamage) {
        ...
        if (State.sukuna && State.sukuna.hp > 0 && State.sukuna.state !== 'dead') consider(State.sukuna, 'sukuna', 'sukuna', State.sukuna.radius);   // 🔥
        ...
        if (best.kind === 'player') {
            ...
            else if (best.kind === 'sukuna' && typeof aggroSukuna === 'function') aggroSukuna(attacker.id);   // 🔥
                ...
                while (now >= b.nextTick && now < b.endAt) {
                    ...
                    if (b.targetKind === 'player') {
                        ...
                        if (tgt.hp <= 0) {
                            ...
                            else if (b.targetKind === 'sukuna' && typeof killSukuna === 'function') killSukuna(b.ownerId);   // 🔥
    ...
    function clearStuckStates(now) {
        for (let pid in State.players) {
            ...
            // ⚔️ 다이도 하가네 — 무자비 · 질풍참 · 일섬 · 3연타 마무리
            if (p.daidoFury && (!p.daidoFuryEnd || now >= p.daidoFuryEnd + STUCK_PAD)) {
                p.daidoFury = false; p.daidoFuryEnd = 0; p.isCasting = false;
                daburaFixed = true;
            }
            if (p.daidoRush && (!p.daidoRushEnd || now >= p.daidoRushEnd + STUCK_PAD)) {
                p.daidoRush = false; p.daidoRushEnd = 0;
                daburaFixed = true;
            }
            if (p.daidoIaiAt && now >= p.daidoIaiAt + STUCK_PAD) {
                p.daidoIaiAt = 0; p.isCasting = false;
                daburaFixed = true;
            }
            if (p.daidoSpinEnd && now >= p.daidoSpinEnd + STUCK_PAD) {
