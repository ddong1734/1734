...
io.on('connection', (socket) => {
    ...
    try {
        ...
        socket.emit('blackMarketSync', { x: BM.BM_X, y: BM.BM_Y,
            discounts: State.blackMarket.discounts, nextRollAt: State.blackMarket.nextRollAt });
    } catch (e) {}
    ...
    socket.on('playerMove', (data) => {
        ...
        // 🕊️ 야곱의 사다리 — 시전자·피격자 모두 이 동안 못 움직인다 (이펙트 없음)
        if (p.ladderLockUntil && now < p.ladderLockUntil) return;
        // 🔥 마르코 : 응축 · 보호막 중에는 좌표를 서버가 고정한다
        if (p.marcoCastEnd && now < p.marcoCastEnd) return;
        if (p.marcoShieldEnd && now < p.marcoShieldEnd) return;
        // 🧲 키드 : 차징 · 발사 · 변신 · 고철 고정 중에는 움직일 수 없다
        ...
        for (const sid in State.players) {
            ...
            if (sp.team === p.team) continue;
            const RX = 120, RY = (sp.marcoShieldRY || 260);
            const ddx = data.x - sp.marcoShieldX, ddy = data.y - sp.marcoShieldY;
            const n = (ddx * ddx) / (RX * RX) + (ddy * ddy) / (RY * RY);
            if (n < 1) return;   // 보호막 안쪽 좌표는 아예 받지 않는다
        }
    ...
    // 🕊️ [쿠루스 하나] 활공 상태 — 다른 사람 화면에도 보이도록 서버가 들고 있는다
    socket.on('kurusuGlide', (d) => {
        let p = State.players[socket.id];
        if (!p || p.isDead) return;
        if (p.characterType !== 'KURUSU') return;
        p.kurusuGliding = !!(d && d.up);
        io.emit('syncPlayerFull', p);
    });

    // 🧲 [키드] 레이저 조준 — 발사 중 이동키로 방향을 천천히 돌린다
    socket.on('kidAim', (d) => {
        let p = State.players[socket.id];
        if (!p || p.isDead) return;
        if (p.characterType !== 'KID') return;
        if (!p.kidLaserFireEnd || Date.now() >= p.kidLaserFireEnd) return;
        const a = Number(d && d.angle);
        if (!Number.isFinite(a)) return;
        const dt = Math.max(0.008, Math.min(0.2, Number(d && d.dt) || 0.016));
        let logic = CharLogic['KID'];
        if (logic && logic.aimLaser) logic.aimLaser(p, a, dt, serverContext);
    });

    // 🕶️ [암매상] 아이템 구매
    socket.on('blackMarketBuy', (itemId) => {
        let p = State.players[socket.id];
        if (!p || p.isDead) return;
        if (p.npcTalking) return;
        const BM = require('./blackMarket.js');
        if (!BM.isNear(p)) { socket.emit('buyFail', '암매상에게 더 가까이 가야 합니다.'); return; }
        const price = BM.priceOf(itemId);
        if (price === null) { socket.emit('buyFail', '암매상이 팔지 않는 물건입니다.'); return; }
        if ((p.gold || 0) < price) { socket.emit('buyFail', '골드가 부족합니다.'); return; }
        if (p.inventory.length >= 30) { socket.emit('buyFail', '인벤토리가 가득 찼습니다.'); return; }
