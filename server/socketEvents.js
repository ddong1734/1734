...
let Damage, Fruits, Kashimo, ShopManager, serverContext;
let isNum, getNpc, makePlayer;
let recalcStats, resetGame, tryReconnect;
let clearBurns, handleBasicAttack, sanitizeActionData, isDaburaLocked;
...
function sendNpcLine(p, npc) {
    ...
    io.to(p.id).emit('npcDialog', {
        npcId: npc.id, name: npc.name, kind: npc.kind,
...
io.on('connection', (socket) => {
    ...
    socket.on('useWorldCleave', (data) => {
        ...
        let ux = safeData.dirX, uy = safeData.dirY;
        if (Math.hypot(ux, uy) < 0.05) { ux = safeData.dir; uy = 0; }
    ...
    socket.on('npcInteract', (npcId) => {
        ...
        if (npc.kind === 'mahera') {
            // ── 🗡️ 마허라 ────────────────────────────────────────────
            // ✅ 퀘스트가 이미 완료됐으면 다시 상호작용할 수 없다
            if ((p.maheraStage || 0) >= 2) return;

            if ((p.maheraStage || 0) === 0) {
                mode = 'intro';
            } else {
