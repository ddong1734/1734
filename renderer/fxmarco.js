...
registerVisualFX('marco_regen', (ctx, fx, alpha, state) => {
    ...
    ctx.globalCompositeOperation = "screen";
    ctx.globalAlpha = alpha * 0.9;
    flameBlob(ctx, cx, cy, 108, tt, alpha * 0.85, false);
    ctx.globalAlpha = alpha * 0.8;
    flameBlob(ctx, cx, cy, 72, tt * 1.5 + 2.1, alpha * 0.9, true);
    // 몸을 휘감아 도는 불꽃 고리
    ctx.globalAlpha = alpha * 0.85;
    for (let k = 0; k < 3; k++) {
        const rr = 118 + k * 14;
        ctx.strokeStyle = (k % 2 ? F_WARM : F_CORE) + "0.9)";
        ctx.lineWidth = 4 - k;
        ctx.setLineDash([20, 12]);
        ctx.lineDashOffset = (k % 2 ? 1 : -1) * state.mathNow / (10 + k * 4);
        ctx.beginPath();
        ctx.ellipse(cx, cy, rr, rr * (0.42 + k * 0.1), Math.sin(tt + k) * 0.3, 0, Math.PI * 2);
        ctx.stroke();
    }
    ctx.setLineDash([]);
...
registerVisualFX('marco_ball', (ctx, fx, alpha, state) => {
    ...
    // ⚠️ dirX 가 0(위/아래) 이면 || 1 이 걸려 옆으로 날아가 버린다.
    const fdx = (fx.dirX === undefined) ? 1 : fx.dirX;
    const fdy = (fx.dirY === undefined) ? 0 : fx.dirY;
    const cx = fx.x + fdx * travel;
    const cy = fx.y + fdy * travel;
    const tt = state.mathNow / 1000;
    ...
    for (let k = 1; k <= 5; k++) {
        const bx = cx - fdx * k * R * 0.42;
        const by = cy - fdy * k * R * 0.42;
        flameBlob(ctx, bx, by, R * (0.62 - k * 0.09), tt + k, alpha * (0.5 - k * 0.08), false);
    }

    // 본체 — 3겹을 겹쳐 심이 또렷하게 빛난다
    ctx.globalAlpha = alpha;
    flameBlob(ctx, cx, cy, R, tt, alpha, true);
    ctx.globalAlpha = alpha * 0.85;
    flameBlob(ctx, cx, cy, R * 0.66, tt * 1.4 + 1.7, alpha, false);
    ctx.globalAlpha = alpha;
    const coreG = ctx.createRadialGradient(cx, cy, 2, cx, cy, R * 0.34);
    coreG.addColorStop(0, "rgba(255,255,255,1)");
    coreG.addColorStop(0.45, F_WARM + "0.95)");
    coreG.addColorStop(1, F_FADE);
    ctx.fillStyle = coreG;
...
registerVisualFX('marco_ball_blast', (ctx, fx, alpha, state) => {
    ...
    ctx.lineCap = "round";
    for (let k = 0; k < 14; k++) {
        const a = (k / 14) * Math.PI * 2 + t * 0.6;
        const inR = rr * 0.5, outR = rr * (1.05 + (k % 3) * 0.16);
        const lg2 = ctx.createLinearGradient(
            fx.x + Math.cos(a) * inR, fx.y + Math.sin(a) * inR,
            fx.x + Math.cos(a) * outR, fx.y + Math.sin(a) * outR);
        lg2.addColorStop(0, F_CORE + "0.95)");
        lg2.addColorStop(0.4, (k % 2 ? F_WARM : F_MAIN) + "0.9)");
        lg2.addColorStop(1, F_FADE);
        ctx.strokeStyle = lg2;
        ctx.lineWidth = (7 - (k % 3) * 2) * (1 - t * 0.5);
        ctx.beginPath();
        ctx.moveTo(fx.x + Math.cos(a) * inR, fx.y + Math.sin(a) * inR);
        ctx.lineTo(fx.x + Math.cos(a) * outR, fx.y + Math.sin(a) * outR);
        ctx.stroke();
    }
...
registerVisualFX('marco_cast', (ctx, fx, alpha, state) => {
    ...
    ctx.lineCap = "round";
    for (let k = 0; k < 20; k++) {
        const a = (k / 20) * Math.PI * 2 + state.mathNow / 400;
...
registerVisualFX('marco_field', (ctx, fx, alpha, state) => {
    ...
    for (let k = 0; k < 9; k++) {
        const a = (k / 9) * Math.PI * 2 + tt * 0.5;
        const px = fx.x + Math.cos(a) * R * 0.55;
        const py = fx.y + Math.sin(a) * R * 0.32;
        const h = 150 * (0.55 + Math.abs(Math.sin(tt * 3.5 + k)) * 0.65);
        const pg = ctx.createLinearGradient(px, py, px, py - h);
        pg.addColorStop(0, F_CORE + "0.95)");
        pg.addColorStop(0.22, F_WARM + "0.9)");
        pg.addColorStop(0.6, F_MAIN + "0.6)");
        pg.addColorStop(1, F_FADE);
        ctx.fillStyle = pg;
        ctx.beginPath();
        ctx.moveTo(px - 26, py);
        ctx.quadraticCurveTo(px - 8, py - h * 0.6, px, py - h);
        ctx.quadraticCurveTo(px + 8, py - h * 0.6, px + 26, py);
...
registerVisualFX('marco_shield', (ctx, fx, alpha, state) => {
    ...
    const o = state.players[fx.id] || (fx.id === state.myId ? state.myPlayer : null);
    // ⚠️ dirX 가 0(위/아래) 이면 || 1 이 걸려 보호막이 옆에 그려진다.
    //    실제 판정 위치와 어긋나 '스킬이 안 나간 것처럼' 보였다.
    const sdx = (fx.dirX === undefined) ? 1 : fx.dirX;
    const sdy = (fx.dirY === undefined) ? 0 : fx.dirY;
    const bx = (o ? o.x : fx.x) + sdx * off;
    const by = (o ? o.y : fx.y) + sdy * off;
    const t = 1 - alpha;
    const tt = state.mathNow / 1000;
