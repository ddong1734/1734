...
import './fxkashimo.js';       // ⚡ 카시모 하지메 (평타 · 반격 · 번개 · 주력 방출 · 환수호박 · 전자파 · 음파)
...
class RenderEngine {
    ...
    renderFrame(ctx, state) {
        ...
        //    ⚡🔮 환수호박 각성 중에는 1번이 '전자파'이므로 이 판정을 하지 않는다.
     ...
     *    gameState.js 의 refreshKashimoSkillLabel 이 이 값을 읽어
     *    1번 스킬 버튼 이름을 바꾼다.
     *
     * ⚡🔮 환수호박 각성 중에는 1번이 '전자파'로 고정되므로 판정을 끈다.
     */
    updateKashimoSkillLabel(state) {
        const my = state.myPlayer;

        // 카시모가 아니거나 환수호박 각성 중이면 판정을 끈다
        if (!my || my.characterType !== 'KASHIMO' || my.amberActive) {
