...
import './fxcleave.js';          // 🗡️ [신규] 세계를 가르는 참격
import './fxdomain.js';          // 🌑 [신규] 유명이경 역월 (시전 · 전개 이펙트)
import './fxsukuna.js';          // 🔥 [신규] 헤이안 스쿠나 (참격 · 화염 화살)
import './fxdaido.js';           // ⚔️ [신규] 다이도 하가네 (검 이펙트)
...
import './fxwarship.js';         // ⚔️🚢 [신규] 칠무해 · 세라핌 · 버스터 콜 군함
...
window.renderGameFrame = (
    ...
    const WORLD_W = window.WORLD_WIDTH || 50000;
    ...
    // 📳 [쿠루스 하나] 야곱의 사다리 — 작은 화면 흔들림.
    //    시전자와 실제로 맞은 대상에게만 걸린다 (netKurusu 가 판정).
    if (window.kurusuShakeUntil && Date.now() < window.kurusuShakeUntil) {
        const pw = window.kurusuShakePower || 6;
        camX += (Math.random() - 0.5) * pw * 2;
        camY += (Math.random() - 0.5) * pw * 2;
    }

    // 📷 [수정] 바닥에서 점프해도 돌발판에서와 똑같이 카메라가 1:1 로 따라오게 한다.
    //
    //    (기존) maxCamY = (GROUND_Y + CAM_GROUND_MARGIN) - viewH
    //      → 바닥에 서 있으면 목표 camY 가 이 값보다 커서 클램프에 걸린다.
    //        그 결과 점프 초반 약 94px 구간에서는 카메라가 아예 멈춰 있다가,
    //        그 구간을 벗어나는 순간 갑자기 1:1 로 따라붙어 화면이 튀었다.
    //        (실측: 바닥에서 플레이어가 화면 73.9% 지점 → 공중에서 62.0% 지점으로 점프)
    //        돌발판 위는 이미 클램프 바깥이라 처음부터 부드럽게 따라왔다.
    //
    //    (수정) 하한을 '바닥에 서 있을 때의 camY' 와 정확히 일치시킨다.
    //      → 사각지대가 0 이 되어 바닥이든 발판이든 항상 동일하게 움직인다.
    //        플레이어는 언제나 화면의 CAM_Y_RATIO 지점(62%)에 고정된다.
    let minCamY = -3600;
    let maxCamY = (GROUND_Y - 45) - viewH * CAM_Y_RATIO;
    ...
    const state = {
        ...
        // 🔥 저주의 왕 (헤이안 스쿠나)
        cursePortal: window.serverCursePortal,
        sukuna: window.serverSukuna,
        sukunaPortal: window.serverSukunaPortal,
        sukunaFires: window.serverSukunaFires,
    ...
    // 🌑 [유명이경 역월] 영역 — 맵 위, 캐릭터 아래에 그린다.
    //    · 내가 안에 있는 영역 : 화면 전체가 영역 배경으로 덮인다
    //    · 밖에서 보는 영역   : 커다란 검은 원으로만 보인다
    try { drawDomainLayer(ctx, state, camX, camY, viewW, viewH); }
    catch (e) { logLayerError('domain', e); }

    // 🔥 [저주의 왕] 불길 장판 — 지속 상태라 매 프레임 직접 그린다.
    //    맵 위, 캐릭터 아래에 깔린다.
    try {
        if (state.inCurseZone && window.serverSukunaFires && window.serverSukunaFires.length) {
            drawSukunaFires(ctx, window.serverSukunaFires, Date.now(), state.mathNow);
        }
    } catch (e) { logLayerError('sukunaFire', e); }
    ...
    // 🌗 영역 전개/붕괴 중 화면 밝기 — 화면 좌표계(변환 없음)에서 덮는다.
    //    앞 1초 어두워지고, 뒤 3초에 걸쳐 밝아진다.
    try {
        const myDm = findMyDomain(state);
        if (myDm) {
            const fade = domainFadeAlpha(myDm, Date.now());
            if (fade > 0.001) {
                ctx.setTransform(1, 0, 0, 1, 0, 0);
    ...
    } catch (e) { logLayerError('domainFade', e); }
};

/** 🌑 내가 지금 들어가 있는 영역을 찾는다 (없으면 null) */
function findMyDomain(state) {
    const list = window.serverDomains || [];
    ...
    for (let i = 0; i < list.length; i++) {
        const dm = list[i];
        if (!dm) continue;
        const dx = me.x - dm.x, dy = me.y - dm.y;
        if (Math.hypot(dx, dy) <= dm.radius) return dm;
    }
    return null;
}

/** 🌑 모든 영역을 그린다 (내가 안에 있는 영역은 배경까지) */
function drawDomainLayer(ctx, state, camX, camY, viewW, viewH) {
    const list = window.serverDomains || [];
    if (!list.length) return;
    const now = Date.now();
    const mathNow = state.mathNow;
    const inside = findMyDomain(state);

    // 밖에서 보는 영역(검은 원)을 먼저, 내가 든 영역을 나중에 그린다
    for (let i = 0; i < list.length; i++) {
        const dm = list[i];
        if (!dm || dm === inside) continue;
        drawDomain(ctx, dm, now, mathNow, false, camX, camY, viewW, viewH);
    }
    if (inside) {
        drawDomain(ctx, inside, now, mathNow, true, camX, camY, viewW, viewH);
    }
}
