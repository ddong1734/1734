...
function drawSwingBlade(ctx, cx, cy, angle, len, alpha, t) {
    ...
    const L = len * (0.45 + 0.55 * reach);
    const H = len * 0.30;

    // 뒤로 끌리는 잔광
    ctx.globalAlpha = alpha * 0.45;
    let tg = ctx.createLinearGradient(0, 0, L, 0);
    tg.addColorStop(0, FADE);
    tg.addColorStop(0.6, DEEP + "0.5)");
    tg.addColorStop(1, MID + "0.8)");
    ctx.fillStyle = tg;
    ctx.fillRect(0, -H * 0.26, L, H * 0.52);

    // 초승달 칼자국 (끝이 뾰족)
    ctx.globalAlpha = alpha;
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.quadraticCurveTo(L * 0.5, -H, L, -H * 0.10);
    ctx.lineTo(L, H * 0.10);
    ctx.quadraticCurveTo(L * 0.5, H, 0, 0);
    ctx.closePath();
    let bg = ctx.createLinearGradient(0, 0, L, 0);
    bg.addColorStop(0, EDGE + "1)");
    bg.addColorStop(0.4, MID + "0.95)");
    bg.addColorStop(1, DEEP + "0.35)");
    ctx.fillStyle = bg;
    ctx.fill();
    ...
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.moveTo(0, 0); ctx.quadraticCurveTo(L * 0.5, -H, L, -H * 0.10);
    ctx.moveTo(0, 0); ctx.quadraticCurveTo(L * 0.5,  H, L,  H * 0.10);
    ctx.stroke();
...
registerVisualFX('daido_swing', (ctx, fx, alpha, state) => {
    const t = 1 - alpha;
    drawSwingBlade(ctx, fx.x, fx.y, fx.angle || 0, (fx.radius || 200) * 1.25, alpha, t);
});
