...
//    사진처럼, 시전자 아래쪽으로 두 갈래의 굵은 빛기둥이 뻗어 나간다.
// ============================================================================
registerVisualFX('dabura_light_beams', (ctx, fx, alpha, state) => {
    ...
    // ── ④ 폭발 예정 지점(아래쪽)에 원형 예고선 ──────────────────────
    ctx.strokeStyle = D_GOLD + (0.5 + Math.abs(Math.sin(now / 130)) * 0.4) + ')';
    ctx.lineWidth = 6;
    ctx.setLineDash([22, 16]);
    ctx.lineDashOffset = -now / 22;
    ctx.beginPath();
    ctx.ellipse(0, DOWN, R * 0.98, R * 0.42, 0, 0, Math.PI * 2);
    ctx.stroke();
    ctx.setLineDash([]);
