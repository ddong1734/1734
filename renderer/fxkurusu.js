...
// ────────────────────────────────────────────────────────────────────────────
// ✨ 이동 잔상 — 지나간 자리에 1.5초간 황금빛이 머문다
// ────────────────────────────────────────────────────────────────────────────
registerVisualFX('kurusu_trail', (ctx, fx, alpha, state) => {
    const t = 1 - alpha;
    // 서서히 옅어지며 위로 살짝 떠오른다
    const y = fx.y - t * 26;
    const r = 26 * (1 - t * 0.45);
    ...
    let g = ctx.createRadialGradient(fx.x, y, 2, fx.x, y, r * 2.2);
    g.addColorStop(0, HOLY_CORE + "0.9)");
    g.addColorStop(0.4, HOLY_MID + "0.55)");
    g.addColorStop(1, HOLY_FADE);
    ctx.fillStyle = g;
    ctx.beginPath(); ctx.arc(fx.x, y, r * 2.2, 0, Math.PI * 2); ctx.fill();

    // 가운데 심
    ctx.globalAlpha = alpha * 0.9;
    ctx.fillStyle = HOLY_CORE + "0.95)";
    ctx.beginPath(); ctx.arc(fx.x, y, r * 0.42, 0, Math.PI * 2); ctx.fill();

    // 흩어지는 빛가루
    ctx.globalAlpha = alpha * 0.6;
    for (let k = 0; k < 3; k++) {
        const a = (k / 3) * Math.PI * 2 + fx.seed;
        const d = r * (0.9 + t * 1.5);
        ctx.fillStyle = HOLY_MID + "0.85)";
        ctx.beginPath();
        ctx.arc(fx.x + Math.cos(a) * d, y + Math.sin(a) * d * 0.6, 3.5 * (1 - t), 0, Math.PI * 2);
        ctx.fill();
    }
...
// ────────────────────────────────────────────────────────────────────────────
// ✨ 신성력 획득 — 10초마다 주변에 잠깐 퍼지는 황금 아우라
// ────────────────────────────────────────────────────────────────────────────
registerVisualFX('kurusu_holy_tick', (ctx, fx, alpha, state) => {
    ...
    // 몸을 감싸는 아우라
    ctx.globalAlpha = alpha * 0.75;
    let g = ctx.createRadialGradient(cx, cy, R * 0.15, cx, cy, R);
    g.addColorStop(0, HOLY_CORE + "0.85)");
    g.addColorStop(0.45, HOLY_MID + "0.5)");
    g.addColorStop(1, HOLY_FADE);
    ctx.fillStyle = g;
    ...
    for (let k = 0; k < 6; k++) {
        const a = (k / 6) * Math.PI * 2 + fx.seed;
        const d = R * 0.55;
        const px = cx + Math.cos(a) * d;
        const py = cy + Math.sin(a) * d * 0.55 - t * 55;
        ctx.fillStyle = HOLY_MID + "0.9)";
        ctx.beginPath(); ctx.arc(px, py, 4.5 * (1 - t * 0.6), 0, Math.PI * 2); ctx.fill();
    }
...
registerVisualFX('kurusu_gather', (ctx, fx, alpha, state) => {
    ...
    for (let i = 0; i < pts.length; i++) {
        const P = pts[i];
        // ⏱️ delay 가 있는 구체(타천의 두 번째)는 시간차를 두고 날아온다
        const dly = P.delay || 0;
        if (t < dly) continue;
        const f = Math.min(1, (t - dly) / 0.8);
