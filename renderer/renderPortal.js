...
function drawPortal(ctx, camX, camY, viewW, viewH, mathNow, pt, kind, label, showLifetime) {
    ...
    if (isDark) {
        ...
        // 🔥 검붉은 저주의 기운
        aura.addColorStop(0, "rgba(255, 150, 120, 0.85)");
        aura.addColorStop(0.45, "rgba(190, 24, 20, 0.55)");
        aura.addColorStop(1, "rgba(70, 0, 0, 0)");
    } else {
    ...
    if (isDark || isCurse) {
        ctx.fillStyle = isCurse ? "rgba(14, 1, 2, 0.98)" : "rgba(3, 0, 8, 0.98)";
        ...
        if (isCurse) {
            core.addColorStop(0, "rgba(255, 226, 210, 0.95)");
            core.addColorStop(0.3, "rgba(238, 62, 40, 0.62)");
            core.addColorStop(0.72, "rgba(140, 8, 8, 0.35)");
        } else {
        ...
        core.addColorStop(1, isCurse ? "rgba(30, 0, 0, 0)" : "rgba(10, 0, 25, 0)");
        ctx.fillStyle = core;
        ctx.beginPath(); ctx.ellipse(0, 0, R * pulse, R * 1.25 * pulse, 0, 0, Math.PI * 2); ctx.fill();
        ctx.globalCompositeOperation = "source-over";
    } else {
    ...
    for (let a = 0; a < 4; a++) {
        ...
        ctx.strokeStyle = isCurse ? "rgba(255, 120, 90, 0.95)" : (isDark ? "rgba(190, 110, 255, 0.95)" : "rgba(255, 245, 190, 0.9)");
        ctx.lineWidth = (isDark || isCurse) ? 5 : 7;
        ctx.beginPath();
        for (let t = 0; t <= 1.0001; t += 0.08) {
            let ang = base + t * Math.PI * 1.5;
            let rr = t * R * 1.05 * pulse;
    ...
    for (let s = 0; s < 10; s++) {
        ...
        let rr = (1 - t) * R * 1.9;
        ctx.globalAlpha = t;
        ctx.fillStyle = isCurse ? "rgba(255, 160, 130, 0.95)" : (isDark ? "rgba(205, 150, 255, 0.95)" : "rgba(255, 250, 200, 0.95)");
        ctx.beginPath();
    ...
    ctx.strokeStyle = isCurse ? "rgba(40, 0, 0, 0.95)" : (isDark ? "rgba(15, 0, 30, 0.95)" : "rgba(70, 45, 0, 0.95)"); ctx.lineWidth = 6;
    ctx.strokeText(label, 0, -R * 1.55);
    ctx.fillStyle = isCurse ? "#ffb4a0" : (isDark ? "#d9b3ff" : "#ffe680");
