...
export class RenderEntity {
    ...
    render(ctx, state) {
        ...
        // 🌀 어둠 소용돌이로 솟구치는 동안에는 본체를 그리지 않는다 (dark_rise 이펙트가 대신 표시된다)
        ...
        if (bb && bb.hp > 0 && bb.state !== 'dead' && !bbRising
            ...
            const inSky = (bb.descentUntil && mathNow < bb.descentUntil);         // 공중 강림 시전 중
            ...
            // 🌑 공중 강림 중이면 발밑에 어둠이 흘러내린다 (하늘에 떠 있음을 강조)
            if (inSky) {
                ctx.globalCompositeOperation = "source-over";
                for (let d = 0; d < 7; d++) {
        ...
        // 🚀 [최적화⑦] 인원수 표시는 값이 실제로 바뀐 프레임에만 갱신
