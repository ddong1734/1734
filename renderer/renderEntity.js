...
//
//    ✅ [수정①] 한눈에 보이도록 칸 크기를 키웠다 (9×5 → 15×9, 타이머 3.5 → 6)
//    ✅ [수정②] 전하 바 · 타이머 바 전체를 감싸는 굵은 검은 테두리를 씌웠다
//    ✅ [수정③] 감쇠 타이머가 '위에서 아래로' 줄어들도록 방향을 반전했다
//               (남은 시간만큼 아래쪽이 차 있고, 시간이 흐르면 위에서부터 비워진다)
//
//    hpX / hpW / hpY / hpH : 체력바의 좌상단 X, 폭, Y, 높이
...
function drawKashimoCharge(ctx, obj, hpX, hpW, hpY, hpH, mathNow, scale) {
    ...
    const s = scale || 1;
    const GAP       = 6 * s;      // 체력바와의 간격
    const CELL_W    = 15 * s;     // ✅ 전하 칸 너비 (9 → 15)
    const CELL_H    = 9 * s;      // ✅ 전하 칸 하나의 높이 (5 → 9)
    const CELL_GAP  = 2.5 * s;    // 칸 사이 간격
    const TIMER_W   = 6 * s;      // ✅ 감쇠 타이머 바 너비 (3.5 → 6)
    const TIMER_GAP = 3.5 * s;    // 전하 바와 타이머 바 사이 간격
    const PAD       = 3 * s;      // ✅ 검은 테두리 안쪽 여백
    ...
    if (hasTimer) {
        ...
        // 남은 비율만큼 아래쪽을 채운다 (= 위에서부터 줄어드는 모양)
        let fillH = totalH * ratio;
        let fillY = baseY + (totalH - fillH);          // ✅ 아래에서 시작
...
function drawPortal(ctx, camX, camY, viewW, viewH, mathNow, pt, kind, label, showLifetime) {
    ...
    // ⏳ 소멸까지 남은 시간을 원형 게이지로도 표시 (암흑 포탈 전용)
    ...
    render(ctx, state) {
        ...
        if (!inDarkZone) {
            for (let ok of okras) {
                ...
                // ⚡ 전하 게이지 (작은 몹이므로 축소 — 그래도 확대된 기준이라 잘 보인다)
