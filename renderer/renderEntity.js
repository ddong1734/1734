...
// ============================================================================
// 🗣️ [신규] NPC 렌더링 — 각 팀 정글 상단 발판 중앙의 '티치'
//    · 같은 팀 NPC 는 밝게, 상대팀 NPC 는 어둡게 (상호작용 불가 표시)
//    · 퀘스트 완료 후에도 어둡게 (재상호작용 불가 표시)
//    · 사거리 안에 들어오면 머리 위에 [!] 말풍선이 뜬다
// ============================================================================
function drawNpc(ctx, npc, mathNow, myPlayer, questDone) {
    ...
    ctx.strokeStyle = active ? "#ffd95e" : "#6b6b74";
    ...
    if (active) {
        ctx.globalCompositeOperation = "screen";
        ctx.strokeStyle = `rgba(255, 230, 130, ${0.35 + Math.abs(Math.sin(mathNow / 320)) * 0.45})`;
        ctx.lineWidth = 3;
    ...
    render(ctx, state) {
        ...
        if (!inDarkZone) {
            ...
            for (let ni = 0; ni < npcs.length; ni++) {
                ...
                drawNpc(ctx, n, mathNow, myPlayer, questDone);
            }
        }
        ...
        for (let id in players) {
            ...
            if (talkingOn) {
                ...
                ctx.strokeText("💬 대화 중", p.x, p.y - (amberOn ? 124 : 102));
                ctx.fillStyle = "#ffe680";
                ctx.fillText("💬 대화 중", p.x, p.y - (amberOn ? 124 : 102));
                ctx.restore();
            }
