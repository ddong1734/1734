...
export class RenderEntity {
    ...
    render(ctx, state) {
        const { camX, camY, viewW, viewH, monster, hinbeom, okras, players, myId, myPlayer, mathNow } = state;
        ...
        if (hinbeom && hinbeom.hp > 0 && hinbeom.state !== 'dead'
            ...
            if (hakiOn) {
                ...
                hAura.addColorStop(0, "rgba(255, 90, 130, 0.75)");
                hAura.addColorStop(0.45, "rgba(180, 10, 60, 0.42)");
                hAura.addColorStop(1, "rgba(60, 0, 40, 0)");
                ctx.fillStyle = hAura;
