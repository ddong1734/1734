...
// ============================================================================
function drawPortal(ctx, camX, camY, viewW, viewH, mathNow, pt, kind, label) {
    if (!pt) return;
...
export class RenderEntity {
    ...
    render(ctx, state) {
        ...
        drawPortal(ctx, camX, camY, viewW, viewH, mathNow, state.hinbeomPortal, 'base', '기지 귀환 포탈 (3초 대기)');
        drawPortal(ctx, camX, camY, viewW, viewH, mathNow, state.blackbeardPortal, 'base', '기지 귀환 포탈 (3초 대기)');
        drawPortal(ctx, camX, camY, viewW, viewH, mathNow, state.darkPortal, 'dark', '암흑 왕좌 (3초 대기)');
        ...
        if (bb && bb.hp > 0 && bb.state !== 'dead'
            ...
            const stunned = (bb.telegraphUntil && mathNow < bb.telegraphUntil);   // 크로우즈 시전 1초 전 경직
        ...
        for (let id in players) {
            ...
            // 🌀 포탈 대기 카운트다운 (야타/빛돌진 상태에서도 보이도록 가장 먼저 계산해 둔다)
            ...
            if (p.yataActive) {
                ...
                drawPortalCountdown(ctx, p.x, p.y, portalUntil, mathNow);              // 🌀 카운트다운
                drawDarkPortalCountdown(ctx, p.x, p.y, darkUntil, mathNow);            // 🟣 암흑 포탈 카운트다운
                continue;
            }
