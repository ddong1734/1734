...
// ============================================================================
// ⚡ [신규] 카시모 전하(電荷) 게이지
//    체력바 오른쪽에 세로 4칸으로 나뉜 바를 그린다.
//    · 평타 1회당 1칸씩 보라색으로 채워진다 (아래에서 위로)
//    · 그 바로 오른쪽에 8초 감쇠 타이머를 얇은 세로 바로 표시한다
//      (위에서 아래로 줄어든다)
//
//    hpX / hpY / hpH : 체력바의 좌상단 X, Y 와 높이
//    scale           : 대상 크기에 맞춘 배율 (플레이어 1.0 기준)
// ============================================================================
const CHARGE_MAX = 4;
...
function drawKashimoCharge(ctx, obj, hpX, hpW, hpY, hpH, mathNow, scale) {
    ...
    const s = scale || 1;
    const GAP      = 4 * s;            // 체력바와의 간격
    const CELL_W   = 9 * s;            // 전하 칸 너비
    const CELL_H   = 5 * s;            // 전하 칸 하나의 높이
    const CELL_GAP = 1.5 * s;          // 칸 사이 간격
    const TIMER_W  = 3.5 * s;          // 감쇠 타이머 바 너비
    const TIMER_GAP = 2.5 * s;         // 전하 바와 타이머 바 사이 간격
    ...
    for (let i = 0; i < CHARGE_MAX; i++) {
        ...
        if (filled) {
            ...
            ctx.fillStyle = g;
            ctx.fillRect(baseX + 1 * s, cy + 0.8 * s, CELL_W - 2 * s, CELL_H - 1.6 * s);

            // 가득 찼을 때(4칸) 맥동하는 발광
            if (charge >= CHARGE_MAX) {
                let pulse = 0.45 + Math.abs(Math.sin(mathNow / 130)) * 0.55;
                ctx.globalCompositeOperation = "screen";
                ctx.fillStyle = `rgba(216, 180, 254, ${pulse * 0.8})`;
                ctx.fillRect(baseX + 1 * s, cy + 0.8 * s, CELL_W - 2 * s, CELL_H - 1.6 * s);
        ...
        ctx.strokeStyle = (filled && charge >= CHARGE_MAX)
            ? `rgba(233, 213, 255, ${0.7 + Math.abs(Math.sin(mathNow / 130)) * 0.3})`
            : "rgba(0, 0, 0, 0.9)";
        ctx.lineWidth = Math.max(1, 1 * s);
        ctx.strokeRect(baseX, cy, CELL_W, CELL_H);
    }

    // ── ⏳ 8초 감쇠 타이머 (위에서 아래로 줄어든다) ───────────────────
    ...
    if (until > mathNow) {
        ...
        // 남은 시간만큼 위쪽부터 채워 내려온다 (= 아래로 줄어드는 모양)
        let fillH = totalH * ratio;
        let g2 = ctx.createLinearGradient(tx, baseY, tx, baseY + totalH);
        g2.addColorStop(0, "#d8b4fe");
        g2.addColorStop(1, "#7e22ce");
        ctx.fillStyle = g2;
        ctx.fillRect(tx + 0.5 * s, baseY + 0.5 * s, TIMER_W - 1 * s, Math.max(0, fillH - 1 * s));

        // 마지막 2초는 붉게 점멸해 곧 사라짐을 알린다
        if (remain <= 2000) {
            let blink = 0.4 + Math.abs(Math.sin(mathNow / 100)) * 0.6;
            ctx.fillStyle = `rgba(255, 80, 120, ${blink * 0.75})`;
            ctx.fillRect(tx + 0.5 * s, baseY + 0.5 * s, TIMER_W - 1 * s, Math.max(0, fillH - 1 * s));
        }
...
// ============================================================================
// ⏳ [신규] 포탈 소멸까지 남은 시간 표시
//    포탈 자체의 머리 위에 "남은 시간 N초"를 크게 띄운다.
// ============================================================================
...
// ============================================================================
// 🌀 포탈 그리기 공용 함수
//    kind: 'base'(노란 기지 귀환) | 'dark'(보라+검정 암흑 왕좌)
// ============================================================================
function drawPortal(ctx, camX, camY, viewW, viewH, mathNow, pt, kind, label, showLifetime) {
    ...
    // ⏳ [추가] 소멸까지 남은 시간을 원형 게이지로도 표시 (암흑 포탈 전용)
    ...
    render(ctx, state) {
        ...
        if (!inDarkZone) {
            for (let ok of okras) {
                ...
                // ⚡ 전하 게이지 (작은 몹이므로 축소)
                drawKashimoCharge(ctx, ok, ok.x - 29, 58, ok.y - 34, 8, mathNow, 0.75);
