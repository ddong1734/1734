...
registerVisualFX('sukuna_slash_fire', (ctx, fx, alpha, state) => {
    ...
    for (let k = -2; k <= 2; k++) {
        const off = k * h * 0.19;
        const len = w * reach;
        ctx.globalAlpha = alpha * (k === 0 ? 1 : (Math.abs(k) === 1 ? 0.8 : 0.55));

        let g = ctx.createLinearGradient(-w / 2, 0, w / 2, 0);
        g.addColorStop(0, "rgba(255, 30, 50, 0)");
        g.addColorStop(0.35, "rgba(255, 70, 70, 0.85)");
        g.addColorStop(0.7, "rgba(255, 140, 130, 0.95)");
        g.addColorStop(1, "rgba(255, 255, 255, 1)");
        ctx.strokeStyle = g;
        ctx.lineWidth = (k === 0 ? 26 : (Math.abs(k) === 1 ? 16 : 9));
        ctx.lineCap = "round";
        ctx.beginPath();
        ctx.moveTo(-w / 2, off + h * 0.24);
        ctx.lineTo(-w / 2 + len, off - h * 0.24);
        ctx.stroke();
    }

    // 베인 자리에 남는 붉은 균열
    ctx.globalAlpha = alpha * 0.7;
    ctx.strokeStyle = "rgba(120, 0, 10, 0.9)";
    ctx.lineWidth = 5;
    for (let k = -1; k <= 1; k++) {
        ctx.beginPath();
        ctx.moveTo(-w / 2, k * h * 0.3 + h * 0.2);
        ctx.lineTo(w / 2 * reach, k * h * 0.3 - h * 0.2);
        ctx.stroke();
    }

    // 착탄 섬광 (크게)
    ctx.globalAlpha = alpha * 0.9;
    let fg = ctx.createRadialGradient(0, 0, 6, 0, 0, w * 0.55);
    fg.addColorStop(0, "rgba(255,255,255,1)");
    fg.addColorStop(0.28, "rgba(255, 120, 100, 0.85)");
    fg.addColorStop(0.62, "rgba(220, 30, 30, 0.45)");
    fg.addColorStop(1, "rgba(140, 0, 10, 0)");
    ctx.fillStyle = fg;
    ctx.beginPath(); ctx.arc(0, 0, w * 0.55, 0, Math.PI * 2); ctx.fill();

    // 사방으로 튀는 파편
    ctx.globalAlpha = alpha * 0.8;
    ctx.strokeStyle = "rgba(255, 200, 190, 0.95)";
    ctx.lineWidth = 4; ctx.lineCap = "round";
    for (let k = 0; k < 10; k++) {
        const a = (k / 10) * Math.PI * 2 + t * 2;
        const r0 = w * 0.22, r1 = w * (0.42 + t * 0.3);
        ctx.beginPath();
