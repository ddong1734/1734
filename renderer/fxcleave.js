...
registerVisualFX('world_cleave', (ctx, fx, alpha, state) => {
    ...
    // 🚀 [수정] 속도를 아주 조금만 줄였다 (앞 22% → 34% 구간에 사거리 끝 도달).
    //    급감속을 완화(지수 2.2 → 1.7)해 날아가는 모습이 눈에 더 잘 남는다.
    const flyT = Math.min(1, t / 0.34);
    const ease = 1 - Math.pow(1 - flyT, 1.7);
    const head = CLEAVE_RANGE * ease;           // 반달의 현재 위치

    const R = CLEAVE_HALF;
    const thick = R * 0.40;

    // 도달한 뒤에는 잔상만 서서히 사라진다 (잔상이 확실히 보이도록 천천히)
    const tailFade = (flyT < 1) ? 1 : Math.max(0, 1 - (t - 0.34) / 0.66);

    RenderUtils.withRotation(ctx, ox, oy, ang, () => {

        // ── ① 회색 잔상 띠 ─────────────────────────────────────────
        ctx.globalAlpha = 0.62 * tailFade;
        let trail = ctx.createLinearGradient(0, 0, Math.max(1, head), 0);
        trail.addColorStop(0, "rgba(105,107,114,0)");
        trail.addColorStop(0.3, "rgba(140,142,150,0.45)");
        trail.addColorStop(1, "rgba(210,213,220,0.9)");
        ctx.fillStyle = trail;
        ctx.fillRect(0, -R * 0.86, head, R * 1.72);

        // ── ② 잔상 반달 여러 장 ────────────────────────────────────
        //    [수정] 12장으로 늘리고 더 진하게 — 지나온 길이 확실히 남는다
        const GHOSTS = 12;
        for (let k = GHOSTS; k >= 1; k--) {
            let back = head - (k / GHOSTS) * R * 4.2;
            if (back < 0) continue;
            let ga = (1 - k / (GHOSTS + 1)) * 0.62 * tailFade;
            let gs = 0.70 + (1 - k / GHOSTS) * 0.30;
            let g = Math.round(130 + (1 - k / GHOSTS) * 95);
