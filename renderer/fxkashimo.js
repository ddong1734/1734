...
//                          ✅ [수정] 대상을 '관통'해 뒤쪽까지 뻗는다 (endX/endY)
//  · kashimo_surge       : 주력 방출 (✅ 좌우 범위 확대 · 시전 중 고정)
...
// ⚡✨ 대기를 가르는 번개
//    ✅ [수정] 대상을 '관통'한다.
//       시전자 몸속에서 터져 나와 대상을 꿰뚫고 그 너머(endX/endY)까지 뻗는다.
//       경로 위 두께(fx.radius) 안이 모두 판정 범위이므로 그만큼 굵게 그린다.
// ============================================================================
registerVisualFX('kashimo_sky_bolt', (ctx, fx, alpha, state) => {
    ...
    if (reach > 4) {
        ...
        // ⚡ 굵은 번개 4가닥 (관통이므로 기존보다 한 가닥 더 두껍게)
        for (let b = 0; b < 4; b++) {
...
registerVisualFX('kashimo_surge', (ctx, fx, alpha, state) => {
    ...
    const HALF_W = (fx.radius !== undefined) ? (fx.radius / 2) : 360;   // ✅ 720 / 2
    ...
    // ── ③ 기둥을 감아 도는 보라 번개 줄기 (범위 확대에 맞춰 9가닥) ──
    const STRANDS = 9;
    ctx.lineCap = 'round'; ctx.lineJoin = 'round';
