...
//  · kashimo_counter    : 반격 전류 — 카시모를 때린 상대의 몸에 작은 전류가 흐른다
//  · kashimo_bolt_cast  : 1번 스킬 시전 순간 총구 섬광
//  · kashimo_bolt_hit   : 번개가 대상에 적중한 자리의 전격
//  · kashimo_sky_bolt   : 대기를 가르는 번개 — 하늘에서 내리꽂히는 거대한 보라 번개
...
// ============================================================================
// 🔌 반격 전류 — 카시모를 평타로 때린 상대의 몸에 작은 전류가 흐른다
//    fx.targetId 로 대상을 매 프레임 추적한다.
// ============================================================================
...
// ============================================================================
// ⚡✨ 대기를 가르는 번개 — 하늘에서 대상에게 내리꽂히는 거대한 보라 번개
...
registerVisualFX('kashimo_sky_bolt', (ctx, fx, alpha, state) => {
    ...
    const FALL = 2200;                   // 하늘에서 내려오는 높이
    const topY = ty - FALL;

    // 강림 연출 : 앞 20% 구간에서 번개가 내리꽂힌다
    const strikeT = Math.min(1, prog / 0.2);
    ...
    let colH = FALL * strikeT;
    if (colH > 8) {
        let col = ctx.createLinearGradient(tx, topY, tx, topY + colH);
        col.addColorStop(0, 'rgba(168, 85, 247, 0)');
        col.addColorStop(0.35, K_GLOW + (0.30 * alpha) + ')');
        col.addColorStop(1, K_GLOW + (0.55 * alpha) + ')');
        ctx.fillStyle = col;
        ctx.beginPath();
        ctx.moveTo(tx - 110, topY);
        ctx.lineTo(tx + 110, topY);
        ctx.lineTo(tx + 190, topY + colH);
        ctx.lineTo(tx - 190, topY + colH);
        ctx.closePath();
        ctx.fill();
        ...
        // ⚡ 기둥 안의 굵은 번개 3가닥
        ctx.save();
        ctx.translate(tx, topY);
        ctx.rotate(Math.PI / 2);          // 아래 방향으로 회전
        for (let b = 0; b < 3; b++) {
            let amp = 46 - b * 12;
            drawBoltLine(
                ctx, colH, amp, b * 3.1, now,
                [26 - b * 6, 11 - b * 3, 4],
                [
                    K_GLOW + (0.55 * alpha) + ')',
    ...
    if (strikeT >= 1) {
        ...
        ctx.lineCap = 'round';
        for (let s = 0; s < 14; s++) {
            let ang = (Math.PI * 2 / 14) * s + prog * 0.7;
            let r1 = R * 0.18;
            ...
            ctx.beginPath();
            ctx.moveTo(Math.cos(ang) * r1, Math.sin(ang) * r1);
            ctx.lineTo(Math.cos(ang) * r2 * 1.14, Math.sin(ang) * r2 * 1.14);
            ctx.stroke();
        }
