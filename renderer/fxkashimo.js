...
//  · kashimo_surge        : 주력 방출 (🏵️ 여의 범위 확대)
//  · kashimo_amber_aura   : 환수호박 오라
//  · kashimo_amber_trail  : 환수호박 전기 잔상 (2초)
//  · kashimo_blink        : ✅ [신규] 전격 순간이동 — 출발↔도착 전류
//  · kashimo_blink_blast  : ✅ [신규] 순간이동 도착지 전기폭발
//  · kashimo_wave_blast   : 전자파 연쇄 폭발
//  · kashimo_wave_echo    : ✅ [신규] 뇌신 재폭발
//  · kashimo_sonic_charge / kashimo_sonic : 음파
//  · (투사체) kashimo_bolt : 🏵️ 여의 장착 시 더 굵고 빠르게 보인다
// ============================================================================
...
registerVisualFX('kashimo_surge', (ctx, fx, alpha, state) => {
    ...
    let coreW = (yeoui ? 55 : 40) * corePulse;
    ...
    // 🏵️ 여의 : 범위가 넓어진 만큼 줄기 수도 늘린다
    const STRANDS = yeoui ? 13 : 9;
    ctx.lineCap = 'round'; ctx.lineJoin = 'round';
    for (let s = 0; s < STRANDS; s++) {
        let phase = now / (95 + s * 22) + s * 1.37;
        let side  = (s % 2 === 0) ? 1 : -1;
        let amp   = HALF_W * (0.32 + (s % 3) * 0.17);
    ...
    let auraR = (yeoui ? 135 : 110) * auraPulse;
    ...
    for (let s = 0; s < (yeoui ? 46 : 34); s++) {
        let sp = ((now / 520) + s * 0.0294) % 1;
        let sx = (Math.sin(s * 2.7 + now / 300) * HALF_W * 0.88) * (0.3 + sp * 0.8);
        let sy = 40 - sp * H * 0.95;
    ...
    let topGlow = ctx.createRadialGradient(0, -H * 0.92, 8, 0, -H * 0.92, HALF_W * 1.15);
    ...
    ctx.beginPath(); ctx.arc(0, -H * 0.92, HALF_W * 1.15, 0, Math.PI * 2); ctx.fill();

    // ✅ 시전 중 고정 표시 (발밑 링)
    ctx.globalCompositeOperation = 'source-over';
    ctx.globalAlpha = hold * 0.9;
    ctx.strokeStyle = `rgba(233, 213, 255, ${0.55 + Math.abs(Math.sin(now / 110)) * 0.4})`;
    ctx.lineWidth = 5;
    ctx.setLineDash([16, 12]);
    ctx.lineDashOffset = -now / 22;
    ctx.beginPath();
    ctx.ellipse(0, 44, 78, 24, 0, 0, Math.PI * 2);
    ctx.stroke();
    ctx.setLineDash([]);
...
//    두 점을 잇는 굵은 번개 다발 + 양 끝 섬광으로 '전기가 되어 이동했다'를 표현
// ============================================================================
registerVisualFX('kashimo_blink', (ctx, fx, alpha, state) => {
    const now = state.mathNow;
    const prog = 1 - alpha;
    ...
    let path = ctx.createLinearGradient(0, -HALF, 0, HALF);
    path.addColorStop(0, 'rgba(168, 85, 247, 0)');
    path.addColorStop(0.5, K_GLOW + (0.5 * alpha) + ')');
    path.addColorStop(1, 'rgba(168, 85, 247, 0)');
    ctx.fillStyle = path;
    ...
    // ✨ 경로를 따라 흐르는 전하 입자
    for (let s = 0; s < 10; s++) {
        let sp = ((now / 180) + s * 0.1) % 1;
        ctx.globalAlpha = alpha * (1 - Math.abs(sp - 0.5) * 1.5) * 0.95;
        ctx.fillStyle = (s % 2 === 0) ? '#ffffff' : K_MAIN;
        ctx.beginPath();
        ctx.arc(dist * sp, Math.sin(s * 2.1 + now / 80) * 14, 6, 0, Math.PI * 2);
        ctx.fill();
    }
    ctx.globalAlpha = alpha;
    ctx.restore();
    ...
    ctx.save();
    ctx.translate(fromX, fromY);
    drawSparkBurst(ctx, sR * 0.85, 7, alpha * 0.8, now, 1.1);
    ctx.restore();

    // ── ③ 도착지 섬광 (나타나는 쪽) ──────────────────────────────────
    let eR = 96 * (0.7 + prog * 0.5);
    let eg = ctx.createRadialGradient(toX, toY, 5, toX, toY, eR);
    eg.addColorStop(0, 'rgba(255, 255, 255, 1)');
    eg.addColorStop(0.3, `rgba(240, 220, 255, ${0.85 * alpha})`);
    eg.addColorStop(0.65, K_GLOW + (0.5 * alpha) + ')');
    ...
    drawSparkBurst(ctx, eR * 0.9, 10, alpha, now, 2.3);
    // 도착 링
    ctx.strokeStyle = `rgba(255, 255, 255, ${alpha * 0.9})`;
    ctx.lineWidth = 7 * alpha + 2;
    ctx.beginPath(); ctx.arc(0, 0, eR * (0.5 + prog * 0.7), 0, Math.PI * 2); ctx.stroke();
    ctx.restore();
...
// ============================================================================
// ⚡🔮 [신규] 순간이동 도착지 전기폭발 (150 · 기절 1초)
// ============================================================================
registerVisualFX('kashimo_blink_blast', (ctx, fx, alpha, state) => {
    const now = state.mathNow;
    const prog = 1 - alpha;
    const R = (fx.radius !== undefined) ? fx.radius : 190;
    ...
    let rr = R * (0.45 + prog * 0.8);
    let g = ctx.createRadialGradient(0, 0, 6, 0, 0, rr);
    g.addColorStop(0, 'rgba(255, 255, 255, 1)');
    g.addColorStop(0.24, 'rgba(240, 220, 255, 0.9)');
    g.addColorStop(0.58, K_GLOW + (0.55 * alpha) + ')');
    g.addColorStop(1, 'rgba(70, 10, 130, 0)');
    ctx.fillStyle = g;
    ctx.beginPath(); ctx.arc(0, 0, rr, 0, Math.PI * 2); ctx.fill();

    drawSparkBurst(ctx, rr * 0.95, 12, alpha, now, 1.7);

    // 이중 충격 링
    ctx.strokeStyle = `rgba(255, 255, 255, ${alpha * 0.95})`;
    ctx.lineWidth = 10 * alpha + 2;
    ctx.beginPath(); ctx.arc(0, 0, rr * (0.5 + prog * 0.65), 0, Math.PI * 2); ctx.stroke();
    ctx.strokeStyle = K_GLOW + (alpha * 0.8) + ')';
    ctx.lineWidth = 5 * alpha + 1;
    ctx.beginPath(); ctx.arc(0, 0, rr * (0.72 + prog * 0.72), 0, Math.PI * 2); ctx.stroke();

    // 방사형 섬광
    ctx.lineCap = 'round';
    for (let s = 0; s < 11; s++) {
        let a2 = (Math.PI * 2 / 11) * s + prog * 0.8;
        let r1 = rr * 0.2;
        let r2 = rr * (0.85 + prog * 0.45);
        ctx.strokeStyle = `rgba(255, 255, 255, ${alpha * 0.85})`;
        ctx.lineWidth = 8 * alpha + 1.5;
        ctx.beginPath();
        ctx.moveTo(Math.cos(a2) * r1, Math.sin(a2) * r1);
        ctx.lineTo(Math.cos(a2) * r2, Math.sin(a2) * r2);
        ctx.stroke();
    ...
    // 튀는 파편
    for (let s = 0; s < 12; s++) {
        let sa = s * 2.25 + prog * 3;
        let sr = rr * (0.32 + prog * 0.95);
        ctx.globalAlpha = alpha * (1 - prog * 0.6);
        ctx.fillStyle = (s % 2 === 0) ? '#ffffff' : K_MAIN;
        ctx.beginPath();
        ctx.arc(Math.cos(sa) * sr, Math.sin(sa) * sr * 0.88, 7 - prog * 4, 0, Math.PI * 2);
        ctx.fill();
    }
...
// ============================================================================
// ⚡🌩️ [신규] 뇌신 재폭발 — 0.3초 뒤 전체 자리에서 동시에 터진다
...
registerVisualFX('kashimo_wave_echo', (ctx, fx, alpha, state) => {
    ...
    if (colH > 6) {
        ...
        drawBoltLine(ctx, colH, 20, (fx.val || 0) * 2.1, now,
            [16, 7, 2.6],
            [K_GLOW + (0.5 * alpha) + ')',
...
registerProjectile('kashimo_bolt', (ctx, proj, state) => {
    ...
    RenderUtils.withRotation(ctx, proj.x, proj.y, ang, () => {
        ...
        const TAIL = yeoui ? 420 : 260;
        const HALF = yeoui ? 56 : 34;

        let tail = ctx.createLinearGradient(0, -HALF, 0, HALF);
        tail.addColorStop(0, 'rgba(168, 85, 247, 0)');
        tail.addColorStop(0.5, K_GLOW + (yeoui ? '0.7)' : '0.55)'));
        ...
        drawBoltLine(ctx, TAIL + 90, yeoui ? 28 : 17, 1.7, now,
            yeoui ? [34, 16, 6] : [22, 10, 4],
            [K_GLOW + '0.5)', 'rgba(196, 132, 252, 0.95)', 'rgba(255, 255, 255, 1)']);
        ctx.restore();

        let headR = yeoui ? 78 : 52;
        let head = ctx.createRadialGradient(70, 0, 3, 70, 0, headR);
        head.addColorStop(0, 'rgba(255, 255, 255, 1)');
        head.addColorStop(0.35, yeoui ? 'rgba(255, 245, 200, 0.92)' : 'rgba(233, 213, 255, 0.9)');
        ...
        ctx.save();
        ctx.translate(70, 0);
        drawSparkBurst(ctx, yeoui ? 62 : 40, yeoui ? 9 : 6, 0.9, now, 2.1);
        ctx.restore();
