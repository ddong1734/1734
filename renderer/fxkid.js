...
registerVisualFX('kid_golem_break', (ctx, fx, alpha, state) => {
    ...
    // ── ① 무너지는 골렘 실루엣 (점점 아래로 주저앉는다) ────────
    if (t < 0.45) {
        const collapse = t / 0.45;
        ctx.save();
        ctx.translate(cx, cy);
        ctx.scale(1 + collapse * 0.12, 1 - collapse * 0.55);   // 납작하게 눌린다
        ctx.globalAlpha = alpha * (1 - collapse * 0.5);
        drawGolemBody(ctx, 0, 0, 1, alpha * (1 - collapse), state.mathNow, false);
        ctx.restore();
    }

    // ── ② 사방으로 튀는 고철 파편 26개 ─────────────────────────
    for (let k = 0; k < 26; k++) {
        const a = (k / 26) * Math.PI * 2 + (k % 3) * 0.4;
        const spd = 190 + ((k * 41) % 100) * 2.6;
        const px = cx + Math.cos(a) * spd * ease;
        // 위로 튀었다가 중력으로 떨어진다
        const py = cy + Math.sin(a) * spd * 0.55 * ease + Math.pow(ease, 2) * 240;
        const rr = 10 + ((k * 29) % 10) * 2.4;
