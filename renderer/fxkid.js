...
registerVisualFX('kid_golem_cast', (ctx, fx, alpha, state) => {
    ...
    // ── ② 점점 골렘 실루엣이 잡힌다 ────────────────────────────
    if (t > 0.28) {
        // 아래에서 위로 차오르듯 몸이 조립된다
        const f = Math.min(1, (t - 0.28) / 0.62);
 ...
 *    grow 0 → 1 로 커지며 완성된다.
 */
export function drawGolemBody(ctx, cx, cy, grow, alpha, mathNow, ghost, swing, facing) {
    // ⚔️ swing 0~1 : 평타를 칠 때 팔이 위 → 아래로 크게 내려온다
    const SW = (swing === undefined) ? 0 : Math.max(0, Math.min(1, swing));
    const FC = (facing === -1) ? -1 : 1;

    const S = Math.max(0.05, grow);
    const W = 175 * S, H = 220 * S;
    const top = cy - H;
    const bob = Math.sin(mathNow / 620) * 4 * S;

    // 🎨 영상 배색 — 거의 검은 남색 장갑 + 크림빛 흰 판 + 금색 링
    const D0 = "#0b0f1c";     // 가장 어두운 외곽
    const D1 = "#161c33";     // 장갑 어두운 면
    const D2 = "#27304f";     // 장갑 중간
    const D3 = "#3b4670";     // 장갑 밝은 면
    const PL = "#f2f2e6";     // 흰 판
    const PS = "#c9cabb";     // 흰 판 그림자
    const GD = "#c9982a";     // 금색
    const GL = "#f7dd86";
    ...
    const spike = (x, y, len, ang, w) => {
        ...
        ctx.beginPath();
        ctx.moveTo(0, -w);
        ctx.quadraticCurveTo(len * 0.55, -w * 0.42, len, 0);
        ctx.quadraticCurveTo(len * 0.55, w * 0.42, 0, w);
        ctx.closePath();
        const g = ctx.createLinearGradient(0, -w, len, w);
        g.addColorStop(0, PS); g.addColorStop(0.35, PL); g.addColorStop(1, "#ffffff");
        ctx.fillStyle = g; ctx.fill();
        ctx.strokeStyle = D0; ctx.lineWidth = 2.6 * S; ctx.stroke();
        ctx.restore();
    };

    /** ⬜ 장갑 위에 얹히는 흰 줄무늬 판 */
    const plate = (x, y, w2, h2, rot) => {
        ctx.save(); ctx.translate(x, y); ctx.rotate(rot || 0);
        ctx.fillStyle = PL;
        ctx.fillRect(-w2 / 2, -h2 / 2, w2, h2);
        ctx.strokeStyle = D0; ctx.lineWidth = 3 * S;
        ctx.strokeRect(-w2 / 2, -h2 / 2, w2, h2);
        ctx.fillStyle = PS;
        ctx.fillRect(-w2 / 2, h2 * 0.18, w2, h2 * 0.32);
        ctx.restore();
    };
    ...
    for (const sd of [-1, 1]) {
        ctx.save();
        ctx.translate(cx + sd * W * 0.26, cy - H * 0.13);
        ctx.fillStyle = D1;
        ctx.beginPath();
        ctx.moveTo(-W * 0.18, 0); ctx.lineTo(W * 0.18, 0);
        ctx.lineTo(W * 0.21, H * 0.14); ctx.lineTo(-W * 0.21, H * 0.14);
        ctx.closePath(); ctx.fill();
        ctx.strokeStyle = D0; ctx.lineWidth = 4.5 * S; ctx.stroke();
        plate(0, H * 0.06, W * 0.28, H * 0.05, 0);
        ctx.restore();
    }

    // ── ② 몸통 (넓적한 드럼 · 흰 줄무늬) ───────────────────────
    ctx.beginPath();
    ctx.moveTo(cx - W * 0.50, top + H * 0.34);
    ctx.quadraticCurveTo(cx - W * 0.60, top + H * 0.62, cx - W * 0.42, cy - H * 0.11);
    ctx.lineTo(cx + W * 0.42, cy - H * 0.11);
    ctx.quadraticCurveTo(cx + W * 0.60, top + H * 0.62, cx + W * 0.50, top + H * 0.34);
    ctx.closePath();
    const bg2 = ctx.createLinearGradient(cx - W * 0.6, top, cx + W * 0.5, cy);
    bg2.addColorStop(0, D3); bg2.addColorStop(0.4, D2); bg2.addColorStop(1, D1);
    ctx.fillStyle = bg2; ctx.fill();
    ctx.strokeStyle = D0; ctx.lineWidth = 6 * S; ctx.stroke();

    // 몸통 흰 줄무늬 3줄 (세로)
    for (let k = -1; k <= 1; k++) {
        plate(cx + k * W * 0.26, top + H * 0.56, W * 0.13, H * 0.34, 0);
    }
    // 허리 금색 링
    ctx.strokeStyle = GD; ctx.lineWidth = 6 * S;
    ctx.beginPath(); ctx.ellipse(cx, top + H * 0.72, W * 0.46, H * 0.07, 0, 0, Math.PI * 2); ctx.stroke();
    ctx.strokeStyle = GL; ctx.lineWidth = 2.4 * S;
    ctx.beginPath(); ctx.ellipse(cx, top + H * 0.71, W * 0.46, H * 0.07, 0, -2.3, -0.4); ctx.stroke();

    // ── ③ 어깨 (거대한 둥근 덩어리) ────────────────────────────
    for (const sd of [-1, 1]) {
        const sx = cx + sd * W * 0.52, sy = top + H * 0.34;
        ctx.beginPath(); ctx.arc(sx, sy, W * 0.30, 0, Math.PI * 2);
        const sg = ctx.createRadialGradient(sx - sd * W * 0.1, sy - W * 0.1, W * 0.04, sx, sy, W * 0.30);
        sg.addColorStop(0, D3); sg.addColorStop(0.55, D2); sg.addColorStop(1, D0);
        ctx.fillStyle = sg; ctx.fill();
        ctx.strokeStyle = D0; ctx.lineWidth = 5 * S; ctx.stroke();
        // 어깨 흰 판
        plate(sx, sy + W * 0.05, W * 0.30, H * 0.07, sd * 0.2);
    }

    // ── ④ 🔺 가시 왕관 — 머리·어깨 위를 빙 두른다 ──────────────
    const crownY = top + H * 0.20;
    const crown = [];
    for (let k = 0; k < 11; k++) {
        const f = k / 10;
        const a = -Math.PI * 0.94 + f * Math.PI * 0.88;   // 왼쪽 위 → 오른쪽 위
        const bx2 = cx + Math.cos(a) * W * 0.56;
        const by2 = crownY + Math.sin(a) * H * 0.30;
        const len = W * (0.30 + Math.sin(f * Math.PI) * 0.18);
        spike(bx2, by2, len, a, W * 0.055);
        crown.push([bx2 + Math.cos(a) * len, by2 + Math.sin(a) * len]);
    }
    ...
    ctx.fillStyle = D0;
    ctx.beginPath();
    ctx.moveTo(cx - W * 0.24, hy + H * 0.10);
    ctx.lineTo(cx - W * 0.19, hy - H * 0.08);
    ctx.lineTo(cx + W * 0.19, hy - H * 0.08);
    ctx.lineTo(cx + W * 0.24, hy + H * 0.10);
    ctx.closePath(); ctx.fill();
    ctx.strokeStyle = D0; ctx.lineWidth = 5 * S; ctx.stroke();
    plate(cx, hy + H * 0.10, W * 0.34, H * 0.045, 0);

    for (const sd of [-1, 1]) {
        const ex = cx + sd * W * 0.105, ey = hy;
        const ew = W * 0.12, eh = H * 0.065;
        ctx.fillStyle = PL;
        ctx.fillRect(ex - ew / 2 - 3 * S, ey - eh / 2 - 3 * S, ew + 6 * S, eh + 6 * S);
        ctx.strokeStyle = D0; ctx.lineWidth = 2.6 * S;
        ctx.strokeRect(ex - ew / 2 - 3 * S, ey - eh / 2 - 3 * S, ew + 6 * S, eh + 6 * S);
        ctx.fillStyle = D0;
        ctx.fillRect(ex - ew / 2, ey - eh / 2, ew, eh);
        ctx.globalCompositeOperation = "screen";
        ctx.globalAlpha = alpha * (0.82 + Math.sin(mathNow / 190) * 0.18);
    ...
    // ── ⑥ 팔 (평타 시 크게 휘두른다) ───────────────────────────
    for (const sd of [-1, 1]) {
        const sx = cx + sd * W * 0.52, sy = top + H * 0.40;
        ctx.save();
        ctx.translate(sx, sy);
        const isSwingArm = (SW > 0 && sd === FC);
        const swingAng = isSwingArm ? (-1.35 + (1 - Math.pow(1 - SW, 2.2)) * 2.15) : 0;
        ctx.rotate(sd * (0.20 + Math.sin(mathNow / 760 + sd) * 0.06) + swingAng * sd);
        // 팔뚝
        ctx.fillStyle = D2;
        ctx.beginPath();
        ctx.moveTo(-W * 0.19, 0); ctx.lineTo(W * 0.19, 0);
        ctx.lineTo(W * 0.16, H * 0.46); ctx.lineTo(-W * 0.22, H * 0.46);
        ctx.closePath(); ctx.fill();
        ctx.strokeStyle = D0; ctx.lineWidth = 5 * S; ctx.stroke();
        plate(0, H * 0.14, W * 0.30, H * 0.06, 0);
        plate(0, H * 0.30, W * 0.28, H * 0.06, 0);
        // 주먹 + 발톱 4개
        const fy = H * 0.50;
        ctx.fillStyle = D1;
        ctx.beginPath(); ctx.arc(0, fy, W * 0.17, 0, Math.PI * 2); ctx.fill();
        ctx.strokeStyle = D0; ctx.lineWidth = 4 * S; ctx.stroke();
        for (let k = 0; k < 4; k++) {
            const a = 0.42 + k * 0.72;
            spike(Math.cos(a) * W * 0.14, fy + Math.sin(a) * W * 0.14, W * 0.21, a, W * 0.033);
        }
        ctx.restore();
    }

    // ── ⑦ ⚡ 가시 사이를 잇는 자홍 방전 (영상의 핵심) ──────────
    ctx.globalCompositeOperation = "screen";
    ctx.globalAlpha = alpha * 0.95;
    for (let k = 0; k < crown.length - 1; k++) {
        if ((k + Math.floor(mathNow / 90)) % 2) continue;   // 지지직 끊긴다
        magBolt(ctx, crown[k][0], crown[k][1], crown[k + 1][0], crown[k + 1][1],
                0.95, 2.8 * S, k + Math.floor(mathNow / 100), k % 3 === 0);
    }
    // 왕관 중앙에 뭉치는 빛
    const gy2 = crownY - H * 0.16;
    const cg = ctx.createRadialGradient(cx, gy2, 3, cx, gy2, W * 0.5);
    cg.addColorStop(0, MAG_CORE + (0.8 * alpha) + ")");
    cg.addColorStop(0.4, MAG_MAIN + (0.45 * alpha) + ")");
    cg.addColorStop(1, MAG_FADE);
    ctx.fillStyle = cg;
    ctx.beginPath(); ctx.arc(cx, gy2, W * 0.5, 0, Math.PI * 2); ctx.fill();

    // 몸통을 훑는 방전
    ctx.globalAlpha = alpha * 0.7;
    for (let k = 0; k < 3; k++) {
        const yy = top + H * (0.44 + k * 0.16);
        magBolt(ctx, cx - W * 0.46, yy, cx + W * 0.46, yy, 0.8, 2.4 * S,
                k + Math.floor(mathNow / 150), k === 1);
    }
...
// ────────────────────────────────────────────────────────────────────────────
// 💥 골렘 해제 — 고철이 통째로 부서져 사방으로 흩어진다
// ────────────────────────────────────────────────────────────────────────────
registerVisualFX('kid_golem_break', (ctx, fx, alpha, state) => {
    const t = 1 - alpha;
    const cx = fx.x, cy = fx.y;
    const ease = 1 - Math.pow(1 - Math.min(1, t / 0.5), 2);

    ctx.save();

    // ── ① 몸이 '뚝뚝 끊기며' 부서진다 (투명해지지 않는다) ──────
    //    · 알파를 낮추는 대신, 조각을 잘라내며 사라지게 한다
    //    · 부서지는 동안 몸이 덜컹덜컹 흔들린다
    if (t < 0.42) {
        const collapse = t / 0.42;
        ctx.save();
        // 기계가 고장 난 것처럼 덜컹거린다
        const jolt = (collapse < 0.85) ? Math.sin(state.mathNow / 22) * 9 * collapse : 0;
        ctx.translate(cx + jolt, cy + collapse * 26);

        // 아래에서부터 잘려 나간다 (남은 부분만 보인다)
        const keep = 1 - collapse;
        ctx.beginPath();
        ctx.rect(-260, -420, 520, 420 * keep + 40);
        ctx.clip();

        ctx.scale(1 + collapse * 0.06, 1);
        drawGolemBody(ctx, 0, 0, 1, alpha, state.mathNow, false);   // 알파 유지
        ctx.restore();
        ...
        ctx.globalCompositeOperation = "screen";
        ctx.globalAlpha = alpha * 0.9;
        const cutY = cy + collapse * 26 - 420 + 420 * keep + 40;
        for (let k = 0; k < 6; k++) {
            const px = cx - 90 + k * 36 + Math.sin(state.mathNow / 60 + k) * 8;
            magBolt(ctx, px, cutY, px + (k % 2 ? 30 : -30), cutY - 34,
                    0.9, 3, k + Math.floor(state.mathNow / 70), k % 2 === 0);
        }
        ctx.restore();
    }
    ...
    // ── ② 사방으로 튀는 고철 파편 26개 ─────────────────────────
    for (let k = 0; k < 40; k++) {
        const a = (k / 40) * Math.PI * 2 + (k % 3) * 0.4;
        const spd = 190 + ((k * 41) % 100) * 2.6;
        const px = cx + Math.cos(a) * spd * ease;
        // 위로 튀었다가 중력으로 떨어진다
        const py = cy + Math.sin(a) * spd * 0.55 * ease + Math.pow(ease, 2) * 240;
        const rr = 9 + ((k * 29) % 10) * 3.1;
        ctx.globalAlpha = alpha * (1 - ease * 0.35);
        scrapChunk(ctx, px, py, rr, k * 17 + 5, a + ease * 7);
    }
    ...
    ctx.globalAlpha = alpha * 0.9;
    const R = 260 * ease;
    const g = ctx.createRadialGradient(cx, cy - 40, 6, cx, cy - 40, R);
    g.addColorStop(0, "rgba(255,255,255,0.95)");
    g.addColorStop(0.25, MAG_CORE + "0.85)");
    g.addColorStop(0.6, MAG_MAIN + "0.5)");
    g.addColorStop(1, MAG_FADE);
    ctx.fillStyle = g;
    ctx.beginPath(); ctx.arc(cx, cy - 40, R, 0, Math.PI * 2); ctx.fill();

    for (let k = 0; k < 8; k++) {
        const a = (k / 8) * Math.PI * 2;
        magBolt(ctx, cx, cy - 40,
                cx + Math.cos(a) * R * 1.15, cy - 40 + Math.sin(a) * R * 0.8,
                alpha * 0.85, 3.2, k * 5 + 3, k % 3 === 0);
    }

    // ── ④ 바닥에 이는 먼지 ─────────────────────────────────────
    ctx.globalCompositeOperation = "source-over";
    ctx.globalAlpha = alpha * 0.4 * (1 - ease * 0.6);
    ctx.fillStyle = "#6d7480";
    for (let k = 0; k < 7; k++) {
        const dx = (k - 3) * 52 * ease;
        ctx.beginPath();
        ctx.ellipse(cx + dx, cy + 8, 40 * ease + 12, 16 * ease + 6, 0, 0, Math.PI * 2);
        ctx.fill();
    }
    ctx.globalAlpha = 1;
    ctx.restore();
...
registerVisualFX('kid_golem_aura', (ctx, fx, alpha, state) => {
    ...
    //    ⚔️ 평타를 치면 그 순간부터 0.4초 동안 팔을 크게 휘두른다
    let swing = 0;
    if (o.kidSwingAt && state.mathNow - o.kidSwingAt < 400) {
        swing = (state.mathNow - o.kidSwingAt) / 400;
    }
    drawGolemBody(ctx, cx, cy, 1, alpha, state.mathNow, false, swing, o.lastFacing);
    ...
    // 🔩 주위를 떠도는 고철 파편 (영상처럼 사방에 흩날린다)
    ctx.globalCompositeOperation = "source-over";
    for (let k = 0; k < 22; k++) {
        // 각자 다른 속도·궤도로 천천히 떠다닌다
        const spd = 380 + ((k * 47) % 100) * 6;
        const a = state.mathNow / spd + (k / 22) * Math.PI * 2;
        const rr = 130 + ((k * 31) % 100) * 1.9 + Math.sin(state.mathNow / 520 + k) * 18;
        const px = cx + Math.cos(a) * rr;
        const py = cy - 70 + Math.sin(a) * rr * 0.5 + Math.sin(state.mathNow / 700 + k * 2) * 14;
        ctx.globalAlpha = alpha * (0.55 + ((k * 17) % 10) / 22);
        scrapChunk(ctx, px, py, 7 + ((k * 23) % 10) * 1.3, k * 13 + 4, a * 2.4);
    }
    ctx.globalAlpha = 1;
    ctx.restore();
});
