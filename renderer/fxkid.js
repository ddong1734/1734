...
function scrapChunk(ctx, x, y, r, seed, rot) {
    ...
    g.addColorStop(0, SCRAP_L);
    g.addColorStop(0.5, SCRAP_M);
    g.addColorStop(1, SCRAP_D);
    ctx.fillStyle = g;
    ctx.fill();
...
export function drawKidStack(ctx, cx, cy, size, stack, held, mathNow) {
    if (!(stack > 0)) return;
    const R = size * 0.5;
    const N = Math.round(4 + stack * 12);       // 쌓일수록 조각이 늘어난다
    ...
    for (let k = 0; k < N; k++) {
        ...
        const dist = R * (1.9 - settle * 0.95);
        const px = cx + Math.cos(a) * dist;
        const py = cy + Math.sin(a) * dist * 0.9;
        const rr = R * (0.16 + ((k * 31) % 10) / 90);
...
export function drawGolemBody(ctx, cx, cy, grow, alpha, mathNow, ghost) {
    ...
    const W = 150 * S, H = 175 * S;
    const top = cy - H;

    ctx.save();
    ctx.globalAlpha = alpha * (ghost ? 0.9 : 1);

    // ── 몸통 (사다리꼴 고철 덩어리) ─────────────────────────────
    ctx.beginPath();
    ctx.moveTo(cx - W * 0.62, top + H * 0.30);
    ctx.lineTo(cx + W * 0.62, top + H * 0.30);
    ctx.lineTo(cx + W * 0.42, cy);
    ctx.lineTo(cx - W * 0.42, cy);
    ctx.closePath();
    const bg = ctx.createLinearGradient(cx - W, top, cx + W, cy);
    bg.addColorStop(0, SCRAP_L);
    bg.addColorStop(0.5, SCRAP_M);
    bg.addColorStop(1, SCRAP_D);
    ctx.fillStyle = bg; ctx.fill();
    ctx.strokeStyle = "#20242b"; ctx.lineWidth = 5 * S; ctx.stroke();

    // ── 어깨 (좌우로 크게 튀어나온다) ───────────────────────────
    for (const sd of [-1, 1]) {
        scrapChunk(ctx, cx + sd * W * 0.70, top + H * 0.32, W * 0.32, sd > 0 ? 5 : 9, sd * 0.4);
        // 팔 (굵게 아래로)
        ctx.save();
        ctx.translate(cx + sd * W * 0.74, top + H * 0.38);
        ctx.rotate(sd * (0.24 + Math.sin(mathNow / 700 + sd) * 0.06));
        ctx.fillStyle = SCRAP_M;
        ctx.beginPath();
        ctx.moveTo(-W * 0.19, 0);
        ctx.lineTo(W * 0.19, 0);
        ...
        ctx.closePath(); ctx.fill();
        ctx.strokeStyle = "#20242b"; ctx.lineWidth = 4 * S; ctx.stroke();
        // 주먹
        scrapChunk(ctx, -W * 0.03, H * 0.68, W * 0.24, sd > 0 ? 11 : 15, 0);
        ctx.restore();
    }

    // ── 머리 (작고 각지게) ──────────────────────────────────────
    scrapChunk(ctx, cx, top + H * 0.10, W * 0.27, 2, Math.sin(mathNow / 900) * 0.08);

    // ── 눈 (자홍색으로 빛난다) ──────────────────────────────────
    ctx.globalCompositeOperation = "screen";
    ctx.globalAlpha = alpha * (0.75 + Math.sin(mathNow / 220) * 0.25);
    for (const sd of [-1, 1]) {
        const ex = cx + sd * W * 0.10, ey = top + H * 0.10;
        const eg = ctx.createRadialGradient(ex, ey, 1, ex, ey, W * 0.13);
        eg.addColorStop(0, "rgba(255,255,255,1)");
        eg.addColorStop(0.4, MAG_MAIN + "0.95)");
        eg.addColorStop(1, MAG_FADE);
        ctx.fillStyle = eg;
        ctx.beginPath(); ctx.arc(ex, ey, W * 0.13, 0, Math.PI * 2); ctx.fill();
    }

    // ── 이음새를 흐르는 자기력 ──────────────────────────────────
    ctx.globalAlpha = alpha * 0.6;
    for (let k = 0; k < 3; k++) {
        const yy = top + H * (0.38 + k * 0.20);
        magBolt(ctx, cx - W * 0.5, yy, cx + W * 0.5, yy, 0.7, 2.2 * S,
                k + Math.floor(mathNow / 160), k === 1);
    }
    ctx.globalCompositeOperation = "source-over";
    ctx.globalAlpha = 1;
    ctx.restore();
