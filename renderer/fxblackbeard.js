...
// ============================================================================
// 🟪 지저스 바제스 — 등장 낙하 (하늘에서 빠르게 떨어진다)
// ============================================================================
registerVisualFX('burgess_fall', (ctx, fx, alpha, state) => {
    const now = state.mathNow;
    const bg = state.burgess;
    // 실제 낙하 중인 바제스를 따라간다
    let cx = fx.x, cy = fx.y;
    if (bg && bg.hp > 0 && bg.state === 'falling') { cx = bg.x; cy = bg.y; }
    ...
    // 낙하 궤적 (긴 보랏빛 꼬리)
    ctx.globalCompositeOperation = "screen";
    let tg = ctx.createLinearGradient(cx, cy - 900, cx, cy);
    tg.addColorStop(0, "rgba(120, 30, 220, 0)");
    tg.addColorStop(0.6, `rgba(160, 60, 255, ${0.35 * alpha})`);
    tg.addColorStop(1, `rgba(220, 160, 255, ${0.75 * alpha})`);
    ctx.fillStyle = tg;
    ctx.beginPath();
    ctx.moveTo(cx - R * 0.35, cy - 900);
    ctx.lineTo(cx + R * 0.35, cy - 900);
    ...
    g.addColorStop(0, `rgba(240, 210, 255, ${0.85 * alpha})`);
    g.addColorStop(0.45, `rgba(150, 55, 245, ${0.45 * alpha})`);
    ...
    // 흩날리는 보랏빛 파편
    for (let s = 0; s < 10; s++) {
        let sp = ((now / 260) + s * 0.1) % 1;
        let sa = s * 2.2;
        ctx.globalAlpha = (1 - sp) * alpha;
        ctx.fillStyle = "rgba(190, 110, 255, 0.9)";
        ctx.beginPath();
        ctx.arc(cx + Math.cos(sa) * R * 1.2, cy - sp * 260 + Math.sin(sa) * 30, 7 - sp * 4, 0, Math.PI * 2);
        ctx.fill();
    }
    ctx.globalAlpha = 1;
    ctx.restore();
...
registerVisualFX('burgess_blast', (ctx, fx, alpha, state) => {
    ...
    ctx.globalCompositeOperation = "screen";
    ctx.globalAlpha = alpha;
    let g = ctx.createRadialGradient(0, 0, R * 0.1, 0, 0, R * (0.6 + prog * 0.9));
    g.addColorStop(0, "rgba(245, 225, 255, 0.95)");
    g.addColorStop(0.4, `rgba(178, 90, 255, ${0.7})`);
    g.addColorStop(1, "rgba(90, 10, 170, 0)");
    ctx.fillStyle = g;
    ctx.beginPath(); ctx.arc(0, 0, R * (0.6 + prog * 0.9), 0, Math.PI * 2); ctx.fill();

    // 실제 판정 범위를 알려주는 링
    ctx.strokeStyle = `rgba(230, 190, 255, ${alpha})`;
    ctx.lineWidth = 14 * alpha + 3;
    ctx.beginPath(); ctx.arc(0, 0, R * (0.45 + prog * 0.55), 0, Math.PI * 2); ctx.stroke();
    ctx.strokeStyle = `rgba(150, 55, 245, ${alpha * 0.85})`;
    ...
    ctx.beginPath(); ctx.ellipse(0, 30, R * (0.7 + prog * 0.9), 46 * (0.5 + prog), 0, 0, Math.PI * 2); ctx.stroke();
    ctx.globalCompositeOperation = "source-over";

    // 튀어오르는 파편
    for (let s = 0; s < 12; s++) {
        let sa = (Math.PI * 2 / 12) * s;
        let sr = R * (0.35 + prog * 0.85);
        ctx.globalAlpha = alpha * 0.9;
        ctx.fillStyle = (s % 2 === 0) ? "rgba(200, 140, 255, 0.9)" : "rgba(18, 0, 34, 0.85)";
        ctx.beginPath();
        ctx.arc(Math.cos(sa) * sr, Math.sin(sa) * sr * 0.55 - prog * 60, 11 - prog * 6, 0, Math.PI * 2);
        ctx.fill();
    }
    ctx.globalAlpha = 1;
    ctx.restore();
...
// ============================================================================
// 🔴 지저스 바제스 — 점프 착지 예정 지점 (0.7초 빨간 표시)
// ============================================================================
registerVisualFX('burgess_telegraph', (ctx, fx, alpha, state) => {
    const now = state.mathNow;
    const R = fx.radius || 113;
    const blink = 0.55 + Math.abs(Math.sin(now / 85)) * 0.45;
    const prog = 1 - alpha;   // 0 → 1 (점점 차오른다)
    ...
    ctx.globalCompositeOperation = "screen";
    ctx.globalAlpha = alpha * blink;
    let g = ctx.createRadialGradient(0, 0, R * 0.15, 0, 0, R);
    g.addColorStop(0, "rgba(255, 90, 90, 0.55)");
    g.addColorStop(0.6, "rgba(255, 30, 30, 0.32)");
    ...
    ctx.lineWidth = 9;
    ctx.beginPath(); ctx.ellipse(0, 0, R * prog, R * 0.45 * prog, 0, 0, Math.PI * 2); ctx.stroke();

    // 중앙 십자 조준
    ctx.strokeStyle = `rgba(255, 70, 70, ${alpha * blink})`;
    ...
    let ay = -220 - Math.sin(now / 180) * 40;
    ctx.beginPath();
    ctx.moveTo(0, ay + 90);
    ctx.lineTo(-34, ay);
    ctx.lineTo(-13, ay);
...
// 🟪 지저스 바제스 — 도약 궤적 잔상
// ============================================================================
registerVisualFX('burgess_trail', (ctx, fx, alpha) => {
    RenderUtils.withContext(ctx, fx.x, fx.y, () => {
        ctx.globalAlpha = alpha * 0.8;
        ctx.fillStyle = "rgba(96, 20, 180, 0.6)";
        ctx.beginPath(); ctx.ellipse(0, 0, 52, 58, 0, 0, Math.PI * 2); ctx.fill();
        ctx.globalCompositeOperation = "screen";
