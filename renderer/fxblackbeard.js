...
//  ✅ [수정] dark_floor 를 빠르고 자연스러운 '블랙홀' 형태로 재작업
// ============================================================================
...
// ============================================================================
// 🌊 블랙홀 — 지면을 뒤덮으며 빠르고 자연스럽게 일렁이는 암흑물질
//    ✅ [재작업] 느리고 뻣뻣하던 파도를 걷어내고,
//       ① 소용돌이치듯 빠르게 흐르는 다층 파형
//       ② 지면을 따라 회전하는 블랙홀 코어 여러 개
//       ③ 안쪽으로 빨려드는 입자
//       세 요소를 겹쳐 '살아 움직이는 심연' 느낌을 준다.
// ============================================================================
registerVisualFX('dark_floor', (ctx, fx, alpha, state) => {
    ...
    // ── ③ 지면을 따라 회전하는 블랙홀 코어 (5개) ─────────────────────────
    const CORES = 5;
    for (let c = 0; c < CORES; c++) {
        let drift = Math.sin(c * 2.1 + now / 1600) * (w / (CORES * 2.2));
        let cxp = A.minX + ((c + 0.5) / CORES) * w + drift;
        let cyp = G - 120 - Math.abs(Math.sin(c * 1.7 + now / 900)) * 90;
        let pulse = 1 + Math.sin(now / 200 + c * 1.3) * 0.22;
        let CR = (130 + c % 2 * 40) * pulse;

        // 심연 (검은 중심)
        ctx.fillStyle = `rgba(2, 0, 6, ${0.86 * hold})`;
        ctx.beginPath(); ctx.ellipse(cxp, cyp, CR * 0.72, CR * 0.5, 0, 0, Math.PI * 2); ctx.fill();

        // 회전하는 소용돌이 팔
        ctx.lineCap = "round";
        for (let a = 0; a < 4; a++) {
            let baseAng = now / 260 * (c % 2 === 0 ? 1 : -1) + a * (Math.PI / 2) + c;
            ctx.strokeStyle = `rgba(168, 68, 255, ${(0.5 + Math.sin(now / 170 + a + c) * 0.25) * hold})`;
            ctx.lineWidth = 9;
            ctx.beginPath();
            for (let s = 0; s <= 1.0001; s += 0.09) {
                let ang = baseAng + s * Math.PI * 1.9;
                let rr = s * CR;
                let xx = cxp + Math.cos(ang) * rr;
                let yy = cyp + Math.sin(ang) * rr * 0.62;
        ...
        // 코어 발광
        ctx.globalCompositeOperation = "screen";
        let cg = ctx.createRadialGradient(cxp, cyp, 4, cxp, cyp, CR * 1.15);
        cg.addColorStop(0, `rgba(238, 208, 255, ${0.72 * hold})`);
        cg.addColorStop(0.32, `rgba(160, 60, 250, ${0.42 * hold})`);
        cg.addColorStop(0.7, `rgba(80, 10, 170, ${0.2 * hold})`);
        cg.addColorStop(1, "rgba(24, 0, 56, 0)");
        ctx.fillStyle = cg;
        ctx.beginPath(); ctx.ellipse(cxp, cyp, CR * 1.15, CR * 0.78, 0, 0, Math.PI * 2); ctx.fill();
        ctx.globalCompositeOperation = "source-over";

        // 안쪽으로 빨려드는 입자
        ctx.globalCompositeOperation = "screen";
        for (let s = 0; s < 9; s++) {
            let sp = ((now / 620) + s * 0.111 + c * 0.3) % 1;
            let sa = s * 2.4 + now / 300 * (c % 2 === 0 ? 1 : -1);
            let sr = CR * 1.9 * (1 - sp);
            ctx.globalAlpha = sp * 0.9 * hold;
            ctx.fillStyle = "rgba(224, 178, 255, 0.95)";
            ctx.beginPath();
            ctx.arc(cxp + Math.cos(sa) * sr, cyp + Math.sin(sa) * sr * 0.62, 3 + sp * 5, 0, Math.PI * 2);
            ctx.fill();
        }
        ctx.globalAlpha = 1;
        ctx.globalCompositeOperation = "source-over";
