...
registerVisualFX('domain_energy_gather', (ctx, fx, alpha, state) => {
    ...
    // 🎨 빛 = 노란빛 흰색 · 어둠 = 보랏빛 검정
    const HOT  = isDark ? "rgba(214, 186, 255, "  : "rgba(255, 255, 255, ";
    const MID  = isDark ? "rgba(138,  74, 255, "  : "rgba(255, 236, 150, ";
    const EDGE = isDark ? "rgba( 46,  10, 100, 0)" : "rgba(255, 198,  70, 0)";
    const LINE = isDark ? "rgba(196, 156, 255, "  : "rgba(255, 250, 205, ";
    ...
    for (let i = 0; i < slabs.length; i++) {
        const s = slabs[i];
        // 판자마다 시차를 둬서 하나씩 차례로 변한다
        const lag = (i % 9) / 13;
        const tt = Math.max(0, Math.min(1, (t - lag) / Math.max(0.001, 1 - lag)));
        if (tt <= 0) continue;

        // ── ① 판자가 부르르 떨리며 달아오른다 ──────────────────────
        const shake = (1 - tt) * 7;
        const jx = Math.sin(state.mathNow / 28 + i * 2.1) * shake;
        const jy = Math.cos(state.mathNow / 24 + i * 1.7) * shake;

        ctx.save();
        ctx.translate(s.x + jx, s.y + jy);
        ctx.rotate(s.rot + Math.sin(state.mathNow / 90 + i) * 0.05 * (1 - tt));

        // 판자 본체가 점점 달아올라 사라진다
        ctx.globalAlpha = (1 - tt) * 0.9;
        let heat = ctx.createLinearGradient(-s.w / 2, 0, s.w / 2, 0);
        heat.addColorStop(0, MID + (0.25 + tt * 0.6) + ")");
        heat.addColorStop(0.5, HOT + (0.4 + tt * 0.6) + ")");
        heat.addColorStop(1, MID + (0.25 + tt * 0.6) + ")");
        ctx.fillStyle = heat;
        ctx.fillRect(-s.w / 2, -s.h / 2, s.w, s.h);
        ...
        ctx.lineWidth = 2 + tt * 4;
        for (let c = 0; c < 3; c++) {
            let ry = (-0.3 + c * 0.3) * s.h;
            ctx.beginPath();
            ctx.moveTo(-s.w / 2, ry);
            ctx.lineTo(-s.w * 0.1, ry + s.h * 0.22 * (c - 1));
            ctx.lineTo(s.w / 2, ry);
            ctx.stroke();
        }
        ctx.restore();
        ...
        // ── ② 조각이 튀며 중심으로 빨려든다 ────────────────────────
        ctx.globalAlpha = Math.min(1, tt * 1.6) * (1 - tt * 0.4);
        for (let k = 0; k < 6; k++) {
            let a = (k / 6) * Math.PI * 2 + i * 0.9 + state.mathNow / 420;
            let d = (1 - tt) * 78 + 12;
            let px = s.x + Math.cos(a) * d;
            let py = s.y + Math.sin(a) * d * 0.75;
            let sz = 5 + (1 - tt) * 7;
            ctx.save();
            ctx.translate(px, py);
            ctx.rotate(a + state.mathNow / 260);
            ctx.fillStyle = HOT + "0.9)";
            ctx.fillRect(-sz / 2, -sz * 0.22, sz, sz * 0.44);
            ctx.restore();
        }

        // ── ③ 에너지 구슬이 뭉친다 (맥동) ──────────────────────────
        let pulse = 1 + Math.sin(state.mathNow / 110 + i) * 0.12 * tt;
        let rr = (12 + tt * 52) * pulse;
        ctx.globalAlpha = 0.45 + tt * 0.55;
        let g = ctx.createRadialGradient(s.x, s.y, 2, s.x, s.y, rr);
        g.addColorStop(0, HOT + "1)");
        g.addColorStop(0.4, MID + "0.9)");
        g.addColorStop(1, EDGE);
        ctx.fillStyle = g;
        ctx.beginPath(); ctx.arc(s.x, s.y, rr, 0, Math.PI * 2); ctx.fill();

        // 구슬을 감싸는 고리
        ctx.globalAlpha = tt * 0.8;
        ctx.strokeStyle = LINE + "0.9)";
        ctx.lineWidth = 2.5;
        ctx.beginPath(); ctx.arc(s.x, s.y, rr * 1.35, 0, Math.PI * 2); ctx.stroke();

        // ── ④ 완성 직전 번쩍임 ─────────────────────────────────────
        if (tt > 0.85) {
            let fl = (tt - 0.85) / 0.15;
            ctx.globalAlpha = fl * 0.9;
            let fg = ctx.createRadialGradient(s.x, s.y, 2, s.x, s.y, rr * 2.2);
            fg.addColorStop(0, HOT + "1)");
            fg.addColorStop(1, EDGE);
            ctx.fillStyle = fg;
...
registerVisualFX('domain_bolt', (ctx, fx, alpha, state) => {
    ...
    const MID  = isDark ? "rgba(138,  74, 255, "  : "rgba(255, 236, 150, ";
    const EDGE = isDark ? "rgba( 46,  10, 100, 0)" : "rgba(255, 198,  70, 0)";
    // 어둠은 보랏빛 '검정' 이라 잔상 끝이 어둡게 깔린다
    const TAIL = isDark ? "rgba( 24,   6,  52, 0)" : "rgba(255, 214,  90, 0)";
    ...
    for (let i = 0; i < pts.length; i++) {
        ...
        // ── ① 날아간 자리에 남는 잔상 ─────────────────────────────
        //    출발점 → 착탄점 전체에 남고, 시간이 지나며 옅어진다.
        const trailFade = Math.max(0, 1 - Math.max(0, t - 0.25) / 0.75);
        if (trailFade > 0.01) {
            const hx = F.x + (P.x - F.x) * fly;
            const hy = F.y + (P.y - F.y) * fly;

            // 넓게 번지는 띠
            ctx.globalAlpha = trailFade * 0.5;
            let tg = ctx.createLinearGradient(F.x, F.y, hx, hy);
            tg.addColorStop(0, TAIL);
            tg.addColorStop(0.55, MID + "0.45)");
            tg.addColorStop(1, HOT + "0.85)");
            ctx.strokeStyle = tg;
            ctx.lineWidth = 26; ctx.lineCap = "round";
            ctx.beginPath(); ctx.moveTo(F.x, F.y); ctx.lineTo(hx, hy); ctx.stroke();

            // 가운데 밝은 심
            ctx.globalAlpha = trailFade * 0.85;
            ctx.strokeStyle = HOT + "0.95)";
            ctx.lineWidth = 6;
            ctx.beginPath(); ctx.moveTo(F.x, F.y); ctx.lineTo(hx, hy); ctx.stroke();

            // 경로에 흩어져 남는 잔광 알갱이
            ctx.globalAlpha = trailFade * 0.7;
            for (let k = 1; k <= 6; k++) {
                let ft = k / 7;
                let gx = F.x + (hx - F.x) * ft;
                let gy = F.y + (hy - F.y) * ft;
                let gr = 20 * (1 - ft * 0.5);
                let gg = ctx.createRadialGradient(gx, gy, 1, gx, gy, gr);
                gg.addColorStop(0, HOT + "0.9)");
        ...
        if (fly < 1) {
            ...
            let hg = ctx.createRadialGradient(cx, cy, 2, cx, cy, 40);
            hg.addColorStop(0, HOT + "1)");
            hg.addColorStop(0.5, MID + "0.85)");
            hg.addColorStop(1, EDGE);
            ctx.fillStyle = hg;
        ...
        // ── ③ 착탄 폭발 ───────────────────────────────────────────
        if (boom > 0) {
            // 🌑 [수정] 어둠은 5발이 흩어져 꽂히므로 폭발 반경을 조금만 넓혔다
            const rr = (isDark ? 150 : 105) * (1 - Math.pow(1 - Math.min(1, boom / 0.5), 2.2));
            ctx.globalAlpha = Math.max(0, 1 - Math.max(0, boom - 0.5) / 0.5);
            let g = ctx.createRadialGradient(P.x, P.y, 2, P.x, P.y, rr);
            g.addColorStop(0, HOT + "1)");
            g.addColorStop(0.4, MID + "0.85)");
            g.addColorStop(1, EDGE);
            ctx.fillStyle = g;
            ctx.beginPath(); ctx.arc(P.x, P.y, rr, 0, Math.PI * 2); ctx.fill();

            ctx.strokeStyle = HOT + "0.9)";
            ctx.lineWidth = 3;
            ctx.beginPath(); ctx.arc(P.x, P.y, rr * 0.9, 0, Math.PI * 2); ctx.stroke();
            ...
            for (let k = 0; k < 7; k++) {
                let a = (k / 7) * Math.PI * 2 + i;
                ctx.beginPath();
                ctx.moveTo(P.x + Math.cos(a) * rr * 0.5, P.y + Math.sin(a) * rr * 0.5);
                ctx.lineTo(P.x + Math.cos(a) * rr * 1.3, P.y + Math.sin(a) * rr * 1.3);
                ctx.stroke();
            }
        }
    }
