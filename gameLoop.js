...
module.exports = {
    update: (ctx) => {
        ...
        const { killHinbeom, aggroHinbeom, isInHinbeomArea, getMinion, killMinion, spawnHinbeomMinions, despawnHinbeomMinions,
                recordHinbeomDamage, PORTAL_COOLDOWN,
        ...
        // ====================================================================
        // 🌀 박힌범 처치 포탈 — 포탈에 닿은 플레이어를 자기 팀 기지로 순간이동
        // ====================================================================
        if (State.hinbeomPortal) {
            ...
            if (now >= pt.expireAt) {
                ...
                for (let pid in players) {
                    ...
                    if (now - (p.lastPortalUse || 0) < (PORTAL_COOLDOWN || 1500)) continue;
                    if (Math.hypot(p.x - pt.x, p.y - pt.y) > (pt.radius + 45)) continue;
