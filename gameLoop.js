...
module.exports = {
    update: (ctx) => {
        ...
        const { killHinbeom, aggroHinbeom, isInHinbeomArea, getMinion, killMinion, spawnHinbeomMinions, despawnHinbeomMinions,
                ...
                DESCENT_RISE, DESCENT_ASCEND_MS } = ctx;
        ...
        // ====================================================================
        // 🟣 암흑 왕좌 포탈 — 3초간 머무르면 검은수염의 공간으로 진입
        // ====================================================================
        if (State.darkPortal) {
        ...
        // ====================================================================
        // ⚫ 검은수염 AI — 블랙홀(암흑물질) / 크로우즈+파공아 / 공중 강림
        // ====================================================================
        ...
        if (bb.hp > 0 && bb.state !== 'dead') {
            // 1초마다 체력 회복
            if (now - (bb.lastRegenTick || 0) >= 1000) {
                bb.lastRegenTick = now;
                if (bb.hp < bb.maxHp) bb.hp = Math.min(bb.maxHp, bb.hp + BB_REGEN);
            }

            // 넉백 (공중 강림/상승 중에는 밀리지 않는다)
            ...
            if (bb.telegraphUntil && now >= bb.telegraphUntil) {
                bb.telegraphUntil = 0;
                let tgt = bb.crowsPendingTarget ? players[bb.crowsPendingTarget] : null;
                if (tgt && !tgt.isDead && isInDarkArea(tgt)) {
                    // 검은수염 바로 앞으로 끌어온다
                    ...
                    bb.crowsActiveTarget = bb.crowsPendingTarget;
                    bb.crowsHitAt = now + CROWS_PULL_MS;

                    io.emit('crowsStart', {
                        id: bb.crowsPendingTarget,
                        x: bb.x, y: bb.y,
                        x2: tgt.x, y2: tgt.y,
            ...
            if (!busyNow && anyPlayerInDark && now >= bb.frozenUntil) {
                ...
                if (now >= bb.crowsNextCast) {
                    ...
                    if (cands.length > 0) {
                        ...
                        bb.crowsNextCast = now + 1000;   // 대상이 없으면 1초 뒤 재시도
                    }
                }
                ...
                // 🌑 공중 강림 : 1초마다 3% (어둠 소용돌이로 2초간 상승 후 5초 시전)
                if (!busyNow) {
