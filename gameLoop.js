...
module.exports = {
    update: (ctx) => {
        ...
        const { killHinbeom, aggroHinbeom, isInHinbeomArea, getMinion, killMinion, spawnHinbeomMinions, despawnHinbeomMinions,
                ...
                BG_JUMP_INTERVAL, BG_JUMP_TELEGRAPH, BG_JUMP_TRAVEL, BG_JUMP_DAMAGE, BG_JUMP_MULT, BG_JUMP_ARC, BG_GRAVITY } = ctx;
        ...
        // ⛓️ 어둠어둠열매 크로우즈 속박 진행 (0.5초마다 25 피해 · 2초 경직)
        if (typeof ctx.processYamiBinds === 'function') ctx.processYamiBinds(now);
        // 💥 흔들흔들열매 파공아 예약 처리 (0.5초 경직 후 폭발 / 시너지 강화 폭발)
        if (typeof ctx.processGuraCharges === 'function') ctx.processGuraCharges(now);
        ...
        // ====================================================================
        // 🌀 포탈 대기 처리 공용 함수
        //    ✅ [수정] 박힌범 포탈과 검은수염 포탈은 같은 대기 필드(portalDwellUntil)를
        //       공유하므로, 따로따로 순회하면 한쪽 루프가 다른 쪽 포탈에서 쌓이던
        //       대기 시간을 매 프레임 0으로 지워버려 카운트다운이 끝나지 않았다.
        //       → 같은 필드를 쓰는 포탈들을 '한 번의 순회'로 묶어서 처리한다.
        // ====================================================================
        ...
        /** 같은 대기 필드를 공유하는 포탈 여러 개를 한 번에 처리한다 */
        ...
        // ── 만료된 포탈 먼저 정리 ────────────────────────────────────────
        if (State.hinbeomPortal && now >= State.hinbeomPortal.expireAt) {
            State.hinbeomPortal = null;
            io.emit('syncHinbeomPortal', null);
        }
        if (State.blackbeardPortal && now >= State.blackbeardPortal.expireAt) {
            State.blackbeardPortal = null;
            io.emit('syncBlackbeardPortal', null);
        }
        if (State.darkPortal && now >= State.darkPortal.expireAt) {
            State.darkPortal = null;
            io.emit('syncDarkPortal', null);
        }

        // ── 🌀 기지 귀환 포탈(박힌범 · 검은수염)은 대기 필드를 공유하므로 함께 처리
        let homePortals = [];
        ...
        if (bb.hp > 0 && bb.state !== 'dead') {
            ...
            // ⛓️ 크로우즈 : 경직(1초) 이후 발동 — 조준 표시 안에 있는 대상만 적중
            ...
            if (bb.crowsHitAt && now >= bb.crowsHitAt) {
                ...
                let gx = gTgt ? gTgt.x : bb.x;
                let gy = gTgt ? gTgt.y : bb.y;
        ...
        // ====================================================================
        // 🟪 지저스 바제스 AI
        //    ✅ [수정] 점프 간격 BG_JUMP_INTERVAL 이 3000 → 2000 으로 단축됨
        // ====================================================================
        if (burgessAlive()) {
            ...
            // ── 🦘 도약 중 (포물선 이동 · 공중 목표 Y까지 도달) ─────────────
                ...
                bg.y = baseY - Math.sin(t * Math.PI) * BG_JUMP_ARC;
            }
            // ── 도약 종료 → 착지 풍압 (공중이면 이후 자연 낙하) ─────────────
            else if (bg.jumpingUntil && now >= bg.jumpingUntil) {
                bg.jumpingUntil = 0;
                bg.x = bg.jumpTargetX;
                bg.y = bg.jumpTargetY;
                // 점프 착지 풍압 (바제스 크기의 4.5배 범위 · 300 피해)
                burgessShockwave(bg.x, bg.y, bg.radius * BG_JUMP_MULT, BG_JUMP_DAMAGE);

                if (bg.y < BG_GROUND_Y - 4) {
                    bg.airborne = true;
                    bg.vy = 0;
                    bg.state = 'airborne';
                } else {
            ...
            // ── 예고 종료 → 실제 도약 시작 ────────────────────────────────
                ...
                io.emit('burgessJump', {
                    ...
                    duration: BG_JUMP_TRAVEL, radius: bg.radius, arc: BG_JUMP_ARC
                });
            }
            // ── 평소 : 지면에 붙어 대기 / 2초마다 점프 준비 ────────────────
            else {
                bg.y = BG_GROUND_Y;
                bg.state = 'idle';
                ...
                if (now >= bg.frozenUntil) {
                    ...
                    if (now >= bg.jumpNextCast) {
                        ...
                        if (found) {
                            ...
                            let ty = found.y;
                            let minY = DARK_AREA.minY + bg.radius;
                            ty = Math.max(minY, Math.min(BG_GROUND_Y, ty));
