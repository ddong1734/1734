...
//
// ⚡ 카시모 전하(電荷) 스택 감쇠 — 8초마다 1칸씩
// ⚡🔮 환수호박 — 전기 잔상 만료 정리 · 전자파 연쇄 폭발 처리
//    · 환수호박 발동 중에는 '모든 회복 기능이 무효'가 된다.
//      (아이템 초당 회복 · 자담치킨 등 체력 재생을 전부 차단한다)
// ============================================================================
...
module.exports = {
    update: (ctx) => {
        ...
        safe('regen', () => {
            for (let pid in players) {
                let p = players[pid];
                // 🔮 환수호박 : 회복 완전 차단 (재생 타이머만 갱신해 해제 직후 폭발 회복을 막는다)
