...
module.exports = {
    update: (ctx) => {
        ...
        // ====================================================================
        // 🟪 지저스 바제스 AI
        //    ✅ [수정] 공중에 있는 상대 위치로도 점프한다. 도착 후에는 중력으로 자연 낙하.
        // ====================================================================
        if (burgessAlive()) {
            const BG_GROUND_Y = DARK_GROUND - bg.radius;

            // 넉백 (낙하 / 점프 / 공중 상태에서는 밀리지 않는다)
            ...
            // ── 평소 : 지면에 붙어 대기 / 3초마다 점프 준비 ────────────────
                ...
                if (now >= bg.frozenUntil) {
                    ...
                    if (now >= bg.jumpNextCast) {
                        // 가장 가까운 플레이어를 찾는다 (공중에 있어도 대상이 된다)
                        let found = null, closest = Infinity;
                        for (let pid in players) {
                            let p = players[pid];
                            if (p.isDead || !isInDarkArea(p)) continue;
                        ...
                        if (found) {
                            ...
                            // ✅ 공중 목표 Y — 상대 위치에 맞춘다 (너무 높으면 제한)
                            let ty = found.y;
                            let minY = DARK_AREA.minY + bg.radius;   // 천장 제한
                            ...
                            // 🔴 착지 예정 지점을 빨간색으로 미리 표시 (공중이면 그 높이에)
