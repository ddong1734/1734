...
let _minionWasActive = false;      // 🐗 소환체 동기화 추적
...
module.exports = {
    update: (ctx) => {
        ...
        const { killHinbeom, aggroHinbeom, isInHinbeomArea, getMinion, killMinion, spawnHinbeomMinions, despawnHinbeomMinions,
                ...
                killBlackbeard, aggroBlackbeard, isInDarkArea, clearBlackbeardSkills,
                DARK_AREA, DARK_GROUND, DARK_ENTRY_X, DARK_ENTRY_Y, BB_REGEN, BB_RADIUS,
                ...
                DESCENT_CHANCE, DESCENT_ROLL_MS, DESCENT_DURATION, DESCENT_TICK_MS, DESCENT_TICK_DMG, DESCENT_RISE } = ctx;
        ...
        for (let i = maguBombs.length - 1; i >= 0; i--) {
            ...
            if (now >= b.explodeAt) {
                ...
                } else if (b.targetType === 'blackbeard' && blackbeard.hp > 0) {
                    target = blackbeard; tx = blackbeard.x; ty = blackbeard.y; blackbeard.maguBombUntil = 0;
        ...
        for (let i = justiceBombs.length - 1; i >= 0; i--) {
            ...
            if (now >= b.explodeAt) {
                ...
                if (b.targetType === 'player' && players[b.targetId] && !players[b.targetId].isDead) {
                    ...
                    target = blackbeard; tx = blackbeard.x; ty = blackbeard.y; blackbeard.justiceBombUntil = 0;
                } else if (b.targetType === 'minion') {
                    target = getMinion(b.targetId);
        ...
        for (let i = giantPartisanQueue.length - 1; i >= 0; i--) {
            ...
            if (now >= q.spawnTime) {
                ...
                } else if (q.targetType === 'blackbeard' && blackbeard.hp > 0) {
                    tx = blackbeard.x; ty = blackbeard.y;
        ...
        for (let i = projectiles.length - 1; i >= 0; i--) {
            ...
            if (p.homing) {
                ...
                if (!target && blackbeard.hp > 0 && blackbeard.state !== 'dead') { let d = Math.hypot(blackbeard.x - p.x, blackbeard.y - p.y); if (d < minDist) { minDist = d; target = blackbeard; } }
            ...
            if ((!hit || p.piercing) && hinbeom.hp > 0 && Math.hypot(p.x - hinbeom.x, p.y - hinbeom.y) < hinbeom.radius + eR) {
                if (!(p.piercing && p.hitIds && p.hitIds.includes('hinbeom'))) {
                    ...
                    if (!isInvincible) {
                        ...
                        if (typeof recordHinbeomDamage === 'function') recordHinbeomDamage(p.ownerId, p.damage);   // 🏆 피해량 기록
                        if (p.freeze) {
            ...
            // ⚫ 검은수염 투사체 판정
            if ((!hit || p.piercing) && blackbeard.hp > 0 && blackbeard.state !== 'dead' && Math.hypot(p.x - blackbeard.x, p.y - blackbeard.y) < blackbeard.radius + eR) {
                if (!(p.piercing && p.hitIds && p.hitIds.includes('blackbeard'))) {
                    blackbeard.hp -= p.damage; emitDamageText(blackbeard.x, blackbeard.y, p.damage);
                    if (p.freeze) {
                        blackbeard.frozenUntil = Math.max(blackbeard.frozenUntil || 0, Date.now() + p.freeze);
                        if (p.hasJusticeCoat && p.type === 'partisan') {
                            let hitKey = 'partisanHits_' + p.ownerId;
                            blackbeard[hitKey] = (blackbeard[hitKey] || 0) + 1;
                            if (blackbeard[hitKey] >= 3) {
                                blackbeard[hitKey] = 0;
                                giantPartisanQueue.push({ targetId: 'blackbeard', targetType: 'blackbeard', spawnTime: Date.now() + 500, ownerId: p.ownerId, team: p.team });
                            }
                        }
                    }
                    if (p.fire) addBurn('blackbeard', blackbeard, p.fire.dps, p.fire.dur, p.ownerId);

                    if (p.hasMagu && p.type === 'meigou') {
                        blackbeard.maguBombUntil = Math.max(blackbeard.maguBombUntil || 0, Date.now() + 3000);
                        maguBombs.push({ targetId: 'blackbeard', targetType: 'blackbeard', explodeAt: Date.now() + 3000, ownerId: p.ownerId, team: p.team });
                    }
                    if (p.hasJusticeCoat && p.type === 'dai_funka') {
                        blackbeard.justiceBombUntil = Math.max(blackbeard.justiceBombUntil || 0, Date.now() + 3000);
                    ...
                    if (blackbeard.hp <= 0) killBlackbeard(p.ownerId);
        ...
        for (let i = mantleBolts.length - 1; i >= 0; i--) {
            ...
            if (hinbeom.hp > 0 && !mb.hitIds.includes('hinbeom') && sweptFallHit(mb.x, mbPrevY, mb.y, hinbeom.x, hinbeom.y, mb.radius + hinbeom.radius)) {
                ...
                if (!isInvincible) {
                    ...
                    if (typeof recordHinbeomDamage === 'function') recordHinbeomDamage(mb.ownerId, mb.damage);   // 🏆 피해량 기록
            ...
            // ⚫ 검은수염 낙뢰 판정
            if (blackbeard.hp > 0 && blackbeard.state !== 'dead' && !mb.hitIds.includes('blackbeard') && sweptFallHit(mb.x, mbPrevY, mb.y, blackbeard.x, blackbeard.y, mb.radius + blackbeard.radius)) {
                mb.hitIds.push('blackbeard'); blackbeard.hp -= mb.damage; emitDamageText(blackbeard.x, blackbeard.y, mb.damage);
                blackbeard.electrocutedUntil = Math.max(blackbeard.electrocutedUntil || 0, now + 100);
                blackbeard.frozenUntil = Math.max(blackbeard.frozenUntil || 0, now + 100);
                blackbeard.airFreezeUntil = Math.max(blackbeard.airFreezeUntil || 0, now + 100);
                if (mbOwner) aggroBlackbeard(mb.ownerId);
                if (blackbeard.hp <= 0) killBlackbeard(mb.ownerId);
            }
            // 🐗 소환된 할배새끼 낙뢰 판정
            for (let mi = minions.length - 1; mi >= 0; mi--) {
                let mn = minions[mi];
        ...
        for (let i = magmas.length - 1; i >= 0; i--) {
            ...
            if (hinbeom.hp > 0 && !m.hitIds.includes('hinbeom') && sweptFallHit(m.x, mPrevY, m.y, hinbeom.x, hinbeom.y, m.radius + hinbeom.radius)) {
                ...
                if (!isInvincible) {
                    ...
                    if (typeof recordHinbeomDamage === 'function') recordHinbeomDamage(m.ownerId, m.damage);   // 🏆 피해량 기록
            ...
            // ⚫ 검은수염 마그마 판정
            if (blackbeard.hp > 0 && blackbeard.state !== 'dead' && !m.hitIds.includes('blackbeard') && sweptFallHit(m.x, mPrevY, m.y, blackbeard.x, blackbeard.y, m.radius + blackbeard.radius)) {
                m.hitIds.push('blackbeard'); blackbeard.hp -= m.damage; emitDamageText(blackbeard.x, blackbeard.y, m.damage);
                addBurn('blackbeard', blackbeard, m.fire.dps, m.fire.dur, m.ownerId);
                if (owner) aggroBlackbeard(m.ownerId);
                if (blackbeard.hp <= 0) killBlackbeard(m.ownerId);
            }
            // 🐗 소환된 할배새끼 마그마 판정
            for (let mi = minions.length - 1; mi >= 0; mi--) {
                let mn = minions[mi];
        ...
        // ====================================================================
        // 🌀 박힌범 처치 포탈 — 3초간 머무르면 자기 팀 기지로 순간이동
        //    · 포탈 범위를 벗어나면 대기 시간이 즉시 초기화된다
        //    · 대기 중인 플레이어에게는 머리 위 카운트다운(portalDwell)이 표시된다
        // ====================================================================
        if (State.hinbeomPortal) {
            ...
            if (now >= pt.expireAt) {
                ...
                const DWELL = PORTAL_DWELL_MS || 3000;
                for (let pid in players) {
                    let p = players[pid];
                    if (!p) continue;

                    // 사망했거나 쿨다운 중이면 대기 상태를 초기화한다
                    if (p.isDead || (now - (p.lastPortalUse || 0) < (PORTAL_COOLDOWN || 1500))) {
                        if (p.portalDwellUntil) { p.portalDwellUntil = 0; p.portalDwellStart = 0; io.emit('portalDwell', { id: pid, until: 0 }); }
                        continue;
                    }

                    let inPortal = Math.hypot(p.x - pt.x, p.y - pt.y) <= (pt.radius + 45);

                    if (!inPortal) {
                        // ✅ 포탈 근처를 벗어나면 3초가 초기화된다
                        if (p.portalDwellUntil) { p.portalDwellUntil = 0; p.portalDwellStart = 0; io.emit('portalDwell', { id: pid, until: 0 }); }
                        continue;
                    }

                    // 포탈 안에 새로 진입한 순간 → 카운트다운 시작
                    if (!p.portalDwellUntil) {
                        p.portalDwellStart = now;
                        p.portalDwellUntil = now + DWELL;
                        io.emit('portalDwell', { id: pid, until: p.portalDwellUntil });
                        continue;
                    }

                    // 3초를 다 채웠으면 기지로 순간이동
                    if (now >= p.portalDwellUntil) {
                        ...
                        p.knockbackForce = 0;
                        p.lastPortalUse = now;
                        p.portalDwellUntil = 0;
                        p.portalDwellStart = 0;

                        io.emit('portalDwell', { id: pid, until: 0 });
        ...
        // ====================================================================
        // 🟣 암흑 왕좌 포탈 — 3초간 머무르면 검은수염의 공간으로 이동
        // ====================================================================
        if (State.darkPortal) {
            ...
            if (now >= dp.expireAt) {
                ...
                for (let pid in players) {
                    ...
                    if (p.isDead || (now - (p.lastDarkPortalUse || 0) < (PORTAL_COOLDOWN || 1500))) {
                        if (p.darkDwellUntil) { p.darkDwellUntil = 0; p.darkDwellStart = 0; io.emit('darkDwell', { id: pid, until: 0 }); }
                        continue;
                    }

                    let inPortal = Math.hypot(p.x - dp.x, p.y - dp.y) <= (dp.radius + 45);
                    ...
                    if (!p.darkDwellUntil) {
                        p.darkDwellStart = now;
                        p.darkDwellUntil = now + DWELL;
                        io.emit('darkDwell', { id: pid, until: p.darkDwellUntil });
                        continue;
                    }

                    if (now >= p.darkDwellUntil) {
                        // ⚫ 암흑 왕좌로 진입
                        p.x = DARK_ENTRY_X + (Math.random() * 400 - 200);
                        p.y = DARK_ENTRY_Y;
                        p.vy = 0;
                        p.knockbackForce = 0;
                        p.lastDarkPortalUse = now;
                        p.darkDwellUntil = 0;
                        p.darkDwellStart = 0;

                        io.emit('darkDwell', { id: pid, until: 0 });
                        io.to(pid).emit('teleport', { x: p.x, y: p.y });
                        io.emit('syncPlayerFull', p);
                    }
                }
        ...
        // ====================================================================
        // 🌀 검은수염 처치 포탈 — 3초간 머무르면 자기 팀 기지로 순간이동
        // ====================================================================
        ...
        // ====================================================================
        // ⚫ 검은수염 AI + 암흑물질 장판 + 크로우즈/파공아 + 공중 강림
        //    ✅ 스킬 시전 중(castingUntil)에는 다른 스킬을 중복으로 쓸 수 없다
        // ====================================================================
        if (blackbeard.hp > 0 && blackbeard.state !== 'dead') {
            // 1초마다 체력 회복
            if (now - (blackbeard.lastRegenTick || 0) >= 1000) {
                blackbeard.lastRegenTick = now;
                if (blackbeard.hp < blackbeard.maxHp) blackbeard.hp = Math.min(blackbeard.maxHp, blackbeard.hp + BB_REGEN);
            }

            // 넉백
            if (Math.abs(blackbeard.knockbackForce) > 0) {
                blackbeard.x += blackbeard.knockbackForce;
                blackbeard.knockbackForce *= 0.85;
                if (Math.abs(blackbeard.knockbackForce) < 1) blackbeard.knockbackForce = 0;
            }

            const isCasting = (blackbeard.castingUntil && now < blackbeard.castingUntil);
            const isTelegraph = (blackbeard.telegraphUntil && now < blackbeard.telegraphUntil);
            const isDescending = blackbeard.descentActive;

            // ── 이동 AI (경직/공중 강림 중에는 움직이지 않는다) ──────────────
            if (now >= blackbeard.frozenUntil && !isTelegraph && !isDescending) {
                let bSpeed = blackbeard.speed;

                if (blackbeard.state === 'chase') {
                    let t = players[blackbeard.targetId];
                    if (!t || t.isDead || !isInDarkArea(t)) { blackbeard.targetId = null; blackbeard.state = 'return'; }
                }
                ...
                if (blackbeard.state !== 'chase') {
                    ...
                    for (let pid in players) {
                        ...
                        let d = Math.abs(p.x - blackbeard.x);
                        if (d < closest) { closest = d; found = pid; }
                    }
                    if (found) { blackbeard.targetId = found; blackbeard.state = 'chase'; }
                }

                if (blackbeard.state === 'chase' && players[blackbeard.targetId]) {
                    let t = players[blackbeard.targetId];
                    if (blackbeard.x < t.x - 60) blackbeard.x += bSpeed;
                    else if (blackbeard.x > t.x + 60) blackbeard.x -= bSpeed;
                } else if (blackbeard.state === 'return') {
                    if (Math.abs(blackbeard.x - blackbeard.homeX) < 10) {
                        blackbeard.x = blackbeard.homeX; blackbeard.state = 'idle';
                    } else {
                        blackbeard.x += (blackbeard.homeX > blackbeard.x ? bSpeed : -bSpeed);
                    }
                }
            }

            // 암흑 왕좌 밖으로는 나갈 수 없다
            let bMinX = DARK_AREA.minX + blackbeard.radius;
            let bMaxX = DARK_AREA.maxX - blackbeard.radius;
            if (blackbeard.x < bMinX) { blackbeard.x = bMinX; blackbeard.knockbackForce = 0; }
            if (blackbeard.x > bMaxX) { blackbeard.x = bMaxX; blackbeard.knockbackForce = 0; }
            // 공중 강림 중이 아니면 바닥에 붙어 있는다
            if (!isDescending) blackbeard.y = blackbeard.homeY;
            ...
            // ================================================================
            // 🌑 [스킬 3] 공중 강림 — 1초마다 3% (5초 지속)
            // ================================================================
            if (anyPlayerInDark && !isCasting && !isTelegraph && !isDescending) {
                if (now >= blackbeard.descentNextRoll) {
                    blackbeard.descentNextRoll = now + DESCENT_ROLL_MS;
                    if (Math.random() < DESCENT_CHANCE) {
                        blackbeard.descentActive = true;
                        blackbeard.descentUntil = now + DESCENT_DURATION;
                        blackbeard.descentNextTick = now;                    // 즉시 첫 틱
                        blackbeard.castingUntil = now + DESCENT_DURATION;    // 🔒 다른 스킬 중복 금지
                        blackbeard.y = blackbeard.homeY - DESCENT_RISE;      // 공중으로 높게 띄어오른다

                        io.emit('descentStart', {
                            x: blackbeard.x, y: blackbeard.y,
            ...
            if (blackbeard.descentActive) {
                // 5초 동안 1초마다 150 피해 + 기절
                while (now >= blackbeard.descentNextTick && now < blackbeard.descentUntil + 1) {
                    blackbeard.descentNextTick += DESCENT_TICK_MS;
                    for (let pid in players) {
                        let t = players[pid];
                        if (t.isDead || !isInDarkArea(t)) continue;
                        let actual = DESCENT_TICK_DMG;      // 방어력 무시
                        t.hp -= actual;
                        emitDamageText(t.x, t.y, actual);
                        t.frozenUntil = Math.max(t.frozenUntil || 0, blackbeard.descentUntil);
                        t.electrocutedUntil = Math.max(t.electrocutedUntil || 0, blackbeard.descentUntil);
                        emitStatus(io, t);
                        if (t.hp <= 0) checkPlayerDeath(t, null); else io.to(pid).emit('takeDamage', actual);
                    }
                }
                if (now >= blackbeard.descentUntil) {
                    blackbeard.descentActive = false;
                    blackbeard.descentUntil = 0;
                    blackbeard.y = blackbeard.homeY;                 // 착지
                    io.emit('descentEnd');
                }
            }

            // ================================================================
            // 🌊 [스킬 1] 암흑물질 장판 — 1초마다 7% (3초 지속)
            // ================================================================
            const isCasting2 = (blackbeard.castingUntil && now < blackbeard.castingUntil);
            if (anyPlayerInDark && !isCasting2 && !blackbeard.descentActive) {
                if (now >= blackbeard.darkFloorNextRoll) {
                    blackbeard.darkFloorNextRoll = now + DARKFLOOR_ROLL_MS;
                    if (Math.random() < DARKFLOOR_CHANCE) {
                        blackbeard.darkFloorUntil = now + DARKFLOOR_DURATION;
                        blackbeard.darkFloorNextTick = now + DARKFLOOR_TICK_MS;
                        blackbeard.castingUntil = now + DARKFLOOR_DURATION;   // 🔒 다른 스킬 중복 금지
            ...
            if (blackbeard.darkFloorUntil && now < blackbeard.darkFloorUntil) {
                // 0.5초마다 40 피해 (점프력 감소는 클라이언트 물리에서 처리)
                while (now >= blackbeard.darkFloorNextTick && blackbeard.darkFloorNextTick <= blackbeard.darkFloorUntil) {
                    blackbeard.darkFloorNextTick += DARKFLOOR_TICK_MS;
                    for (let pid in players) {
                        let t = players[pid];
                        if (t.isDead || !isInDarkArea(t)) continue;
                        let actual = DARKFLOOR_TICK_DMG;      // 방어력 무시
                        t.hp -= actual;
                        emitDamageText(t.x, t.y, actual);
                        if (t.hp <= 0) checkPlayerDeath(t, null); else io.to(pid).emit('takeDamage', actual);
                    }
                }
            } else if (blackbeard.darkFloorUntil && now >= blackbeard.darkFloorUntil) {
                blackbeard.darkFloorUntil = 0;
                blackbeard.darkFloorNextTick = 0;
                io.emit('darkFloorEnd');
            }

            // ================================================================
            // ⛓️ [스킬 2] 크로우즈 (7초마다) + 파공아
            //    · 시전 1초 전 경직 + 빨간 조준 표시
            //    · 범위/두께는 각성 엘 토르와 동일
            // ================================================================
            const isCasting3 = (blackbeard.castingUntil && now < blackbeard.castingUntil);
            if (anyPlayerInDark && !isCasting3 && !blackbeard.descentActive && !blackbeard.crowsActiveTarget && !blackbeard.crowsPendingTarget) {
                if (!blackbeard.crowsNextCast) blackbeard.crowsNextCast = now + CROWS_INTERVAL;
                if (now >= blackbeard.crowsNextCast) {
                    // 무작위 플레이어 선정
                    let candidates = [];
                    for (let pid in players) {
                        let p = players[pid];
                    ...
                    if (candidates.length > 0) {
                        ...
                        let t = players[pick];
                        blackbeard.crowsPendingTarget = pick;
                        blackbeard.telegraphUntil = now + CROWS_TELEGRAPH;   // 1초 경직
                        blackbeard.castingUntil = now + CROWS_TELEGRAPH + CROWS_PULL_MS;  // 🔒 중복 금지

                        // 조준 방향 (사거리는 각성 엘 토르와 동일)
                        let dx = t.x - blackbeard.x, dy = t.y - blackbeard.y;
                        let dist = Math.hypot(dx, dy);
                        if (dist === 0) { dx = 1; dy = 0; dist = 1; }
                        blackbeard.crowsAimX = blackbeard.x + (dx / dist) * Math.min(CROWS_RANGE, dist);
                        blackbeard.crowsAimY = blackbeard.y + (dy / dist) * Math.min(CROWS_RANGE, dist);

                        io.emit('crowsTelegraph', {
                            x: blackbeard.x, y: blackbeard.y,
                            x2: blackbeard.crowsAimX, y2: blackbeard.crowsAimY,
                            thickness: CROWS_THICKNESS,
                            duration: CROWS_TELEGRAPH
                        });
                    } else {
                        blackbeard.crowsNextCast = now + CROWS_INTERVAL;
                    }
                }
            }

            // 경직이 끝나면 실제 크로우즈 발사
            if (blackbeard.crowsPendingTarget && now >= blackbeard.telegraphUntil) {
                let pick = blackbeard.crowsPendingTarget;
                blackbeard.crowsPendingTarget = null;
                let t = players[pick];

                if (t && !t.isDead && isInDarkArea(t)) {
                    // 사거리 / 두께 판정 (각성 엘 토르와 동일)
                    let ux = blackbeard.crowsAimX - blackbeard.x, uy = blackbeard.crowsAimY - blackbeard.y;
                    let ulen = Math.hypot(ux, uy); if (ulen === 0) { ux = 1; uy = 0; ulen = 1; }
                    ux /= ulen; uy /= ulen;
                    let rx = t.x - blackbeard.x, ry = t.y - blackbeard.y;
                    let s = rx * ux + ry * uy;
                    let d = Math.abs(rx * (-uy) + ry * ux);

                    if (s >= 0 && s <= CROWS_RANGE && d <= CROWS_THICKNESS / 2) {
                        blackbeard.crowsActiveTarget = pick;
                        blackbeard.crowsHitAt = now + CROWS_PULL_MS;

                        // 끌려오는 목적지 = 검은수염 바로 앞
                        let landX = blackbeard.x + (t.x >= blackbeard.x ? 1 : -1) * (blackbeard.radius + 60);
                        let landY = blackbeard.y;

                        t.crowsPullUntil = now + CROWS_PULL_MS;
                        t.crowsTargetX = landX;
                        t.crowsTargetY = landY;
                        t.isCasting = false;
                        t.skill1Dashing = false;
                        t.yataActive = false; t.yataPath = null;
                        t.elThorActive = false; t.mantleActive = false; t.raigoActive = false; t.raigoDropped = false;

                        io.emit('crowsStart', {
                            id: pick,
                            x: blackbeard.x, y: blackbeard.y,
                            x2: t.x, y2: t.y,
                            targetX: landX, targetY: landY,
                            until: t.crowsPullUntil,
                        ...
                        // 빗나감
                        blackbeard.crowsNextCast = now + CROWS_INTERVAL;
                        blackbeard.castingUntil = 0;
                        io.emit('crowsEnd', { id: pick });
                    }
                } else {
                    blackbeard.crowsNextCast = now + CROWS_INTERVAL;
                    blackbeard.castingUntil = 0;
            ...
            // 끌려온 플레이어에게 파공아
            if (blackbeard.crowsActiveTarget && blackbeard.crowsHitAt && now >= blackbeard.crowsHitAt) {
                let pick = blackbeard.crowsActiveTarget;
                blackbeard.crowsActiveTarget = null;
                blackbeard.crowsHitAt = 0;
                blackbeard.crowsNextCast = now + CROWS_INTERVAL;

                let guraR = blackbeard.radius * GURA_RADIUS_MULT;   // ✅ 검은수염 크기의 1.2배

                io.emit('crowsEnd', { id: pick });
                io.emit('guraImpact', { x: blackbeard.x, y: blackbeard.y, radius: guraR });

                for (let pid in players) {
                    let t = players[pid];
                    if (t.isDead) continue;
                    if (Math.hypot(t.x - blackbeard.x, t.y - blackbeard.y) > guraR + 45) continue;
                    let actual = GURA_DAMAGE;    // 방어력 무시
                    t.hp -= actual;
                    emitDamageText(t.x, t.y, actual);
                    if (t.crowsPullUntil) { t.crowsPullUntil = 0; }
                    if (t.hp <= 0) checkPlayerDeath(t, null); else io.to(pid).emit('takeDamage', actual);
                }

                // 끌려온 대상의 포박 해제
                let tp = players[pick];
                if (tp) { tp.crowsPullUntil = 0; io.emit('syncPlayerFull', tp); }
                blackbeard.castingUntil = 0;
            }

            // 크로우즈 포박 시간이 끝났는데 아직 파공아 전이라면 유지 (물리는 클라이언트가 처리)
            for (let pid in players) {
                let p = players[pid];
                if (p.crowsPullUntil && now >= p.crowsPullUntil && blackbeard.crowsActiveTarget !== pid) {
                    p.crowsPullUntil = 0;
                }
            }
        }

        let bDelta = compressors.blackbeardDelta.getDelta('blackbeard', blackbeard);
        if (bDelta) io.emit('blackbeardUpdate', bDelta);

        // ====================================================================
        // 🐗 소환된 할배새끼 AI (바구니 안에서만 활동)
        // ====================================================================
        // ✅ 바구니에 플레이어가 아무도 없으면 소환체는 전부 사라진다
        ...
        for (let i = shockwaves.length - 1; i >= 0; i--) {
            ...
            for (let entity of nearEntities) {
                if (entity.entityType === 'player') {
                    ...
                    if (Math.abs(hinbeom.x - sw.x) < hitRadiusX && Math.abs(hinbeom.y - sw.y) < hitRadiusY) {
                        if (!sw.hitIds.includes('hinbeom')) {
                            ...
                            if (!isInvincible) {
                                ...
                                if (typeof recordHinbeomDamage === 'function') recordHinbeomDamage(sw.ownerId, sw.damage);   // 🏆 피해량 기록
                    ...
                    // ⚫ 검은수염 충격파 판정
                    if (Math.abs(blackbeard.x - sw.x) < hitRadiusX && Math.abs(blackbeard.y - sw.y) < hitRadiusY) {
                        if (!sw.hitIds.includes('blackbeard')) {
                            sw.hitIds.push('blackbeard');
                            blackbeard.hp -= sw.damage; blackbeard.knockbackForce += sw.kb * 0.2;
                            if (sw.freeze) blackbeard.frozenUntil = Math.max(blackbeard.frozenUntil || 0, Date.now() + sw.freeze);
                            emitDamageText(blackbeard.x, blackbeard.y, sw.damage);
                            aggroBlackbeard(sw.ownerId);
                            if (blackbeard.hp <= 0) killBlackbeard(sw.ownerId);
                        }
                    }
