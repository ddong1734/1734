...
// 🎯 고속 낙하체가 프레임 사이를 건너뛰며 대상을 통과해 버리는 것을 막는 스윕(경로) 판정.
...
module.exports = {
    update: (ctx) => {
        ...
        // ====================================================================
        // 🌀 포탈 대기 처리 공용 함수
        //    ✅ [수정] 쿨다운 검사를 '새로 진입할 때'로만 제한해,
        //       이미 카운트다운이 진행 중이면 절대 초기화되지 않도록 했다.
        //       (기존에는 매 프레임 쿨다운을 검사해 카운트다운이 멈춘 것처럼 보였다)
        //    dwellKey  : 'portalDwellUntil' | 'darkDwellUntil'
        //    startKey  : 'portalDwellStart' | 'darkDwellStart'
        //    useKey    : 'lastPortalUse'    | 'lastDarkPortalUse'
        //    eventName : 'portalDwell'      | 'darkDwell'
        // ====================================================================
        ...
        function processPortal(pt, dwellKey, startKey, useKey, eventName, onComplete) {
            for (let pid in players) {
                let p = players[pid];
                if (!p) continue;
                ...
                // ✅ 포탈 밖으로 나가면 3초가 초기화된다
                if (!inPortal) { clearDwell(p, pid, dwellKey, startKey, eventName); continue; }

                // 포탈 안에 새로 진입한 순간 → 쿨다운을 이때만 검사한다
                if (!p[dwellKey]) {
                    if (now - (p[useKey] || 0) < COOLDOWN) continue;   // 방금 사용했으면 잠시 대기
                    p[startKey] = now;
                ...
                // ✅ 카운트다운이 진행 중이면 그대로 유지 (중간 초기화 없음)
                if (now >= p[dwellKey]) {
                    p[dwellKey] = 0; p[startKey] = 0;
                    p[useKey] = now;
                    io.emit(eventName, { id: pid, until: 0 });
        ...
        if (State.hinbeomPortal) {
            ...
            if (now >= pt.expireAt) {
                ...
                processPortal(pt, 'portalDwellUntil', 'portalDwellStart', 'lastPortalUse', 'portalDwell', goHome);
            }
        }
        ...
        if (State.darkPortal) {
            ...
            if (now >= dp.expireAt) {
                ...
                processPortal(dp, 'darkDwellUntil', 'darkDwellStart', 'lastDarkPortalUse', 'darkDwell', goDark);
            }
        }

        // ====================================================================
        // ⚫ 검은수염 AI — 블랙홀 / 크로우즈+파공아 / 공중 강림
        // ====================================================================
        ...
        if (bb.hp > 0 && bb.state !== 'dead') {
            ...
            // ── ⛓️ 크로우즈 : 경직(1초) 이후 발동 ───────────────────────────
            //    ✅ [수정] 빨간 조준선(band) 안에 들어와 있는 대상에게만 적중한다.
            if (bb.telegraphUntil && now >= bb.telegraphUntil) {
                bb.telegraphUntil = 0;

                // 조준선 안에 남아있는 대상들 중에서 고른다
                //  1) 원래 지목했던 대상이 아직 조준선 안이면 그대로
                //  2) 벗어났다면, 조준선 안의 다른 플레이어 중 가장 가까운 대상
                ...
                if (tgt) {
                    ...
                    // ✅ 조준선 안에 아무도 없으면 스킬이 그대로 불발된다 (파공아도 없음)
            ...
            if (!busyNow && anyPlayerInDark && now >= bb.frozenUntil) {
                ...
                if (now >= bb.crowsNextCast) {
                    ...
                    if (cands.length > 0) {
                        ...
                        // ✅ 시전 시점의 조준 방향을 고정 저장 (빨간 표시 = 실제 판정)
                        ...
                        // 빨간 조준 표시는 '사거리 끝'까지 그린다 (실제 판정 범위와 동일)
