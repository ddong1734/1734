...
module.exports = {
    update: (ctx) => {
        ...
        let now = Date.now(); io.emit('syncBases', bases);
        ...
        for (let i = maguBombs.length - 1; i >= 0; i--) {
            ...
            if (now >= b.explodeAt) {
                ...
                if (b.targetType === 'player' && players[b.targetId] && !players[b.targetId].isDead) {
                    target = players[b.targetId]; tx = target.x; ty = target.y; target.maguBombUntil = 0; io.emit('syncPlayerFull', target);
        ...
        for (let i = justiceBombs.length - 1; i >= 0; i--) {
            ...
            if (now >= b.explodeAt) {
                ...
                let target, tx, ty;
                if (b.targetType === 'player' && players[b.targetId] && !players[b.targetId].isDead) {
                    target = players[b.targetId]; tx = target.x; ty = target.y; target.justiceBombUntil = 0; io.emit('syncPlayerFull', target);
                } else if (b.targetType === 'monster' && monster.hp > 0) {
        ...
        if (detUp || now % 1000 < 20) io.emit('syncDetectors', detectors);
        ...
        for (let pid in players) { if (!players[pid].isDead) grid.insert({...players[pid], entityType: 'player', refId: pid}); }
        if (monster.hp > 0) grid.insert({...monster, entityType: 'monster', refId: 'monster'});
        okras.forEach(ok => { if (ok.hp > 0) grid.insert({...ok, entityType: 'okra', refId: ok.id}); });
                ...
                if (entity.entityType === 'player') {
                    ...
                    if (Math.abs(p.x - sw.x) < hitRadiusX && Math.abs(p.y - sw.y) < hitRadiusY) { 
                        ...
                        if (canHit && !sw.hitIds.includes(entity.refId)) { 
                            ...
                            if (sw.freeze) { p.frozenUntil = Math.max(p.frozenUntil || 0, Date.now() + sw.freeze); io.emit('syncPlayerFull', p); }
