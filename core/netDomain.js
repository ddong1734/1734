...
window.registerNetModule('domain', function (socket, U) {
    ...
    // ── 🌑💥 영역 전용 평타 : 빛 폭발 + 연결고리 ────────────────────
    socket.on('domainLightBurst', (d) => {
        if (!d) return;
        window.visualFX.push({
            id: d.ownerId, type: 'domain_light_burst',
            x: d.x, y: d.y, radius: d.radius || 260,
            links: d.links || [], blasts: d.blasts || [],
            durationMs: 620, life: 38, maxLife: 38
        });
    });

    // ── 🌑☀️🌑 영역 [빛]/[어둠] : 판자가 에너지로 변한다 ────────────
    socket.on('domainLightGather', (d) => {
        if (!d) return;
        let ms = d.gatherMs || 1000;
        // 이 영역의 에너지 출발 지점(변환된 판자)을 기억해 둔다.
        // 이후 폭격 이펙트가 여기서 날아가는 것처럼 보이게 한다.
        window._domEnergy = window._domEnergy || {};
        window._domEnergy[d.ownerId] = { seed: d.seed || 1, perTarget: d.perTarget || 1, kind: d.kind || 'light' };

        window.visualFX.push({
            id: d.ownerId, type: 'domain_energy_gather',
            x: 0, y: 0,
            kind: d.kind || 'light',
            seed: d.seed || 1, perTarget: d.perTarget || 1,
            points: d.points || [],
            durationMs: ms,
    ...
    /** 🎯 폭격이 날아올 출발 지점을 만든다 (변환된 판자 근처에서 온 것처럼) */
    function boltOrigins(ownerId, pts) {
        const dm = (window.serverDomains || []).find(x => x && x.ownerId === ownerId);
        const R = dm ? dm.radius : 675;
        const out = [];
        for (let i = 0; i < pts.length; i++) {
            const a = (i * 2.399) + (Date.now() / 900);   // 매번 다른 방향에서
            out.push({ x: pts[i].x + Math.cos(a) * R * 0.75, y: pts[i].y + Math.sin(a) * R * 0.55 });
        }
        return out;
    }

    // ── 🌑☀️ 영역 [빛] : 0.3초마다 1발씩 폭격 ───────────────────────
    socket.on('domainLightTick', (d) => {
        if (!d) return;
        const pts = d.points || [];
        window.visualFX.push({
            id: d.ownerId, type: 'domain_bolt', kind: 'light',
            x: 0, y: 0, points: pts, from: boltOrigins(d.ownerId, pts),
            durationMs: 300, life: 18, maxLife: 18
        });
    });

    // ── 🌑🌑 영역 [어둠] : 한 번에 꽂히는 폭격 ──────────────────────
    socket.on('domainDarkStrike', (d) => {
        if (!d) return;
        const pts = d.points || [];
        window.visualFX.push({
            id: d.ownerId, type: 'domain_bolt', kind: 'dark',
            x: 0, y: 0, points: pts, from: boltOrigins(d.ownerId, pts),
    ...
    // ── 💫 영역 [아광속 발차기] : 별 궤도 ───────────────────────────
    socket.on('domainKickCast', (d) => {
        if (!d) return;
        let total = (d.chargeMs || 2000) + (d.flyMs || 5000);
        window.visualFX.push({
            id: d.ownerId, type: 'domain_kick_path',
            x: 0, y: 0, path: d.path || [],
            durationMs: total,
            life: Math.round(total / (1000 / 60)), maxLife: Math.round(total / (1000 / 60))
        });
    });

    socket.on('domainKickBlast', (d) => {
        if (!d) return;
        window.visualFX.push({
            id: d.ownerId, type: 'domain_kick_blast',
            x: d.x, y: d.y, radius: d.radius || 300,
