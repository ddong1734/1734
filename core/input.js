...
//            ⚡🔮 환수호박 중이면 조이스틱 방향으로 전격 순간이동이 나간다.
//   · 1번  : 번개 (경직 없음) / 전하 4스택 시 서버가 '대기를 가르는 번개'로 전환
//   · 2번  : 주력 방출 — 시전 중 이동 · 스킬 · 평타가 모두 봉인된다
...
window.initControls = (socket) => {
    ...
    const isFullyLocked = () => isCrowsPulled() || isFruitLocked() || isSonicCharging() || isSurgeLocked() || isNpcTalking();
    ...
    const triggerAttack = () => {
        ...
        // ⚡🔮 [신규] 환수호박 중이면 평타 대신 전격 순간이동이 나간다.
        //    조이스틱 방향(dirX/dirY)을 함께 보내야 서버가 그 방향으로 이동시킨다.
        if (isAmber()) {
            const KS3 = window.GameData.Skills.KASHIMO_S3 || {};
            let blinkCd = KS3.blinkCooldown || 400;
            if (now < (window.myPlayer.blinkCdEnd || 0)) return;
            window.myPlayer.blinkCdEnd = now + blinkCd;

            let dirX = window.joyX || 0;
            let dirY = window.joyY || 0;
            // 조이스틱이 중립이면 바라보는 방향으로
