...
window.initControls = (socket) => {
    ...
    // 🔥 마르코 : 봉리력 응축 · 불사 엉겅퀴 유지 중에는 조작이 봉인된다
    const isMarcoLocked = () => {
        const n = Date.now();
        if (window.myPlayer.marcoCastEnd && n < window.myPlayer.marcoCastEnd) return true;
        if (window.myPlayer.marcoShieldEnd && n < window.myPlayer.marcoShieldEnd) return true;
        return false;
    };

    // 🧲 키드 : 차징 · 발사 · 변신 · 고철 고정 중에는 조작이 봉인된다
    const isKidLocked = () => {
        const n = Date.now();
        const p = window.myPlayer;
        if (p.kidLaserCastEnd && n < p.kidLaserCastEnd) return true;
        if (p.kidLaserFireEnd && n < p.kidLaserFireEnd) return true;
        if (p.kidGolemCastEnd && n < p.kidGolemCastEnd) return true;
        if (p.kidHoldUntil && n < p.kidHoldUntil) return true;
        return false;
    };
    ...
    zone.addEventListener('pointermove', (e) => {
        ...
        // 🕹️ [수정] 차징·시전 중이라고 조이스틱 입력 자체를 막으면 안 된다.
        //    · 손잡이가 그 자리에 굳어 보이고
        //    · 댐드 펑크처럼 '조준' 에 입력이 필요한 스킬이 아예 동작하지 않는다.
        //    입력은 항상 받되, '이동' 만 막는다 (moveX/moveY 를 0 으로).
        const lockedNow = isFullyLocked()
            || (window.myPlayer.isCasting && !isSkill3Aiming())
            || window.myPlayer.skill1Dashing;

        moveKnob(e, lockedNow);
    });

    const moveKnob = (e, aimOnly) => { 
        ...
        // 🕹️ aimOnly : 손잡이와 joyX/joyY 는 갱신하되 실제 이동은 하지 않는다
        if (aimOnly) { window.myPlayer.moveX = 0; window.myPlayer.moveY = 0; return; }
    ...
    document.getElementById('btn-jump').addEventListener('pointerdown', (e) => { 
        ...
        if (window.myPlayer.jumpCount > 0) { 
            ...
            // 🗿 [키드] 고철 골렘 상태에서는 점프 높이 1.5배
            if (window.myPlayer.kidGolemEnd && Date.now() < window.myPlayer.kidGolemEnd) jumpV *= 1.5;
            // 🧲 어사인의 고철이 쌓이면 점프가 낮아진다 (자기자기열매)
            if (window.myPlayer.kidJumpCut !== undefined && window.myPlayer.kidJumpCut < 1)
                jumpV *= Math.max(0.15, window.myPlayer.kidJumpCut);
            // 🦾 자기장 안에서는 점프 높이 절반
            if (window.myPlayer.kidFieldUntil && Date.now() < window.myPlayer.kidFieldUntil) jumpV *= 0.5;
    ...
    const triggerAttack = () => {
        ...
        if (now - window.lastAttackTime >= cool) { 
            ...
            let effectType = charType === 'BORSALINO' ? 'ama_no_murakumo'
                           ...
                           : (charType === 'KURUSU' ? 'kurusu_strike'
                           : (charType === 'MARCO' ? 'marco_strike'
                           : (charType === 'KID'
    ...
    document.getElementById('btn-skill2').addEventListener('pointerdown', (e) => {
        ...
        if (charType === '__REMOVED_PARK__') {
            ...
            //    🎯 시전 순간의 이동키 방향으로 포신을 잡는다. (위·아래·대각선 모두 가능)
            let lx = window.myPlayer.moveX || window.joyX || 0;
            let ly = window.myPlayer.moveY || window.joyY || 0;
            if (Math.hypot(lx, ly) < 0.05) { lx = window.myPlayer.lastFacing || 1; ly = 0; }
            window.myPlayer.kidLaserCastEnd = now + 3000;
            window.myPlayer.moveX = 0; window.myPlayer.moveY = 0;
            clearInterval(window.autoAttackInterval);
            socket.emit('useSkill', {
                type: 2, dir: window.myPlayer.lastFacing,
                dirX: lx, dirY: ly,
                x: window.myPlayer.x, y: window.myPlayer.y
            });
        } else if (charType === 'MARCO') {
            // 🔥 [봉리력] 1초 응축 후 광역 불길 — 응축 중 완전 고정
            window.myPlayer.marcoCastEnd = now + 1000;
            window.myPlayer.moveX = 0; window.myPlayer.moveY = 0;
            clearInterval(window.autoAttackInterval);
            socket.emit('useSkill', { type: 2, dir: window.myPlayer.lastFacing, x: window.myPlayer.x, y: window.myPlayer.y });
        } else if (charType === 'KURUSU') {
            ...
            // 🕹️ 조이스틱을 기울였으면 그 방향으로 돌진한다 (안 기울였으면 바라보는 쪽)
            let rdx = window.myPlayer.moveX || window.joyX || 0;
            let rdy = window.myPlayer.moveY || window.joyY || 0;
            if (Math.hypot(rdx, rdy) < 0.05) { rdx = window.myPlayer.lastFacing || 1; rdy = 0; }
            socket.emit('useSkill', {
                type: 2, dir: window.myPlayer.lastFacing,
                dirX: rdx, dirY: rdy,
                x: window.myPlayer.x, y: window.myPlayer.y
    ...
    document.getElementById('btn-skill3').addEventListener('pointerdown', (e) => {
        ...
        if (charType === 'MARCO') {
            // 🔥 [불사 엉겅퀴] 조이스틱 방향에 2초 보호막 — 그동안 위치 고정
            // 🕹️ [수정] 조이스틱은 버튼을 누르는 순간 이미 0 일 수 있다.
            //    실제 이동 입력(moveX/moveY)을 먼저 보고, 그다음 조이스틱을 본다.
            let sx = window.myPlayer.moveX || window.joyX || 0;
            let sy = window.myPlayer.moveY || window.joyY || 0;
            if (Math.hypot(sx, sy) < 0.05) { sx = window.myPlayer.lastFacing || 1; sy = 0; }
            window.myPlayer.marcoShieldEnd = now + 3500;
            window.myPlayer.moveX = 0; window.myPlayer.moveY = 0;
            clearInterval(window.autoAttackInterval);
            socket.emit('useSkill', { type: 3, dir: window.myPlayer.lastFacing, dirX: sx, dirY: sy, x: window.myPlayer.x, y: window.myPlayer.y });
            return;
        }
