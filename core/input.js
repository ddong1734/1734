...
window.initControls = (socket) => {
    ...
    document.getElementById('btn-skill2').addEventListener('pointerdown', (e) => {
        ...
        if (charType === 'PARK') {
            ...
            window.joyX = 0; window.joyY = 0;
            clearInterval(window.autoAttackInterval);
            let kbK = document.getElementById('knob'); if (kbK) kbK.style.transform = 'translate(0px, 0px)';
            socket.emit('useSkill', { type: 2, dir: window.myPlayer.lastFacing, x: window.myPlayer.x, y: window.myPlayer.y });
        } else if (charType === 'MARCO') {
            ...
            window.joyX = 0; window.joyY = 0;
            clearInterval(window.autoAttackInterval);
            let kb2 = document.getElementById('knob'); if (kb2) kb2.style.transform = 'translate(0px, 0px)';
            socket.emit('useSkill', { type: 2, dir: window.myPlayer.lastFacing, x: window.myPlayer.x, y: window.myPlayer.y });
        } else if (charType === 'KURUSU') {
            ...
            let rdx = window.joyX || 0, rdy = window.joyY || 0;
            if (rdx === 0 && rdy === 0) { rdx = window.myPlayer.lastFacing || 1; rdy = 0; }
    ...
    document.getElementById('btn-skill3').addEventListener('pointerdown', (e) => {
        ...
        if (charType === 'KID') {
            ...
            window.joyX = 0; window.joyY = 0;
            clearInterval(window.autoAttackInterval);
            let kbG = document.getElementById('knob'); if (kbG) kbG.style.transform = 'translate(0px, 0px)';
            socket.emit('useSkill', { type: 3, dir: window.myPlayer.lastFacing, x: window.myPlayer.x, y: window.myPlayer.y });
            return;
        }
        ...
        if (charType === 'MARCO') {
            ...
            let sx = window.joyX || 0, sy = window.joyY || 0;
            if (sx === 0 && sy === 0) { sx = window.myPlayer.lastFacing || 1; sy = 0; }
            window.myPlayer.marcoShieldEnd = now + 3500;
            window.myPlayer.moveX = 0; window.myPlayer.moveY = 0;
            window.joyX = 0; window.joyY = 0;
            clearInterval(window.autoAttackInterval);
            let kb3 = document.getElementById('knob'); if (kb3) kb3.style.transform = 'translate(0px, 0px)';
