...
//   · 2번  : 주력 방출 — 시전 중 이동 · 스킬 · 평타가 모두 봉인된다 (3초)
//   · 3번  : 환수호박 — 죽을 때까지 해제 불가
//
// ⚡🔮 [환수호박 각성 상태]
//   · 평타  : 조이스틱 방향으로 전격 돌진 (150 피해 · 0.5초 기절)
...
window.initControls = (socket) => {
    ...
    // ⚡🔮 전격 돌진 중인가 (돌진 중에는 조이스틱 입력을 무시한다)
    ...
    const triggerAttack = () => {
        ...
        // ⚡🔮 [수정] 환수호박 중이면 평타 대신 '전격 돌진'이 나간다.
        //    조이스틱 방향(dirX/dirY)을 함께 보내야 서버가 그 방향으로 돌진시킨다.
        if (isAmber()) {
            ...
            // 🛟 로컬에서도 즉시 돌진을 시작한다 (서버 응답을 기다리지 않는다)
