...
window.initControls = (socket) => {
    ...
    zone.addEventListener('pointerdown', (e) => { e.preventDefault(); if(window.myPlayer.isDead) return; if (isElThorLocked()) return; if (isCrowsPulled()) return; activeId = e.pointerId; zone.setPointerCapture(e.pointerId); moveKnob(e); });
    zone.addEventListener('pointermove', (e) => { if(activeId === e.pointerId && !window.myPlayer.isDead) { e.preventDefault(); if (isElThorLocked()) return; if (isCrowsPulled()) return; if ((window.myPlayer.isCasting && !isSkill3Aiming()) || window.myPlayer.skill1Dashing) return; moveKnob(e); }});
