...
window.initControls = (socket) => {
    ...
    const triggerAttack = () => {
        ...
        if (now - window.lastAttackTime >= cool) { 
            ...
            let effectType = charType === 'BORSALINO' ? 'ama_no_murakumo'
                           ...
                           : (charType === 'DABURA' ? 'dabura_strike' : 'punch')))));
    ...
    document.getElementById('btn-skill4').addEventListener('pointerdown', (e) => {
        ...
        if (!window.myPlayer.hasWorldCleave) return;      // 🚫 아이템 미장착
        if (window.myPlayer.isDead) return;
        if (window.myPlayer.cleaveCasting) return;        // 이미 시전 중
        if (now < (window.myPlayer.cd4 || 0)) return;     // ⏳ 쿨타임
        ...
        // ⏳ 쿨타임 130초 · 경직 0.5초 (server/config.js 의 CLEAVE_CD / CLEAVE_CAST_MS 와 동일)
        let sk4Cd = 130000;
        let castMs = 500;

        // 🔒 0.5초 경직 — 이동 · 조작을 즉시 봉인한다
        ...
        socket.emit('useWorldCleave', { dir: window.myPlayer.lastFacing, dirX: cDirX, dirY: cDirY, x: window.myPlayer.x, y: window.myPlayer.y });
