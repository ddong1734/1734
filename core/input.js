...
//   · 2번  : 주력 방출 — ✅ [수정] 시전 중 이동 · 스킬 · 평타가 모두 봉인된다
...
window.initControls = (socket) => {
    ...
    // ⚡🌋 [신규] 주력 방출 중에는 이동 · 스킬 · 평타가 모두 봉인된다
    ...
    const isFullyLocked = () => isCrowsPulled() || isFruitLocked() || isSonicCharging() || isSurgeLocked();
    ...
    document.getElementById('btn-jump').addEventListener('pointerdown', (e) => { 
        ...
        if (isFullyLocked()) return;              // ⚫🍈⚡ 모든 봉인 상태
    ...
    const triggerAttack = () => {
        ...
        if (isFullyLocked()) return;              // ⚫🍈⚡ 모든 봉인 상태 (⚡🌋 주력 방출 포함)
    ...
    document.getElementById('btn-skill1').addEventListener('pointerdown', (e) => {
        ...
        if (charType === 'PARK') {
            ...
            // ⚡ 엘 토르: 시전 순간 조이스틱 방향 고정, 2초간 완전 고정 경직
