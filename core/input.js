...
//
// ⚡ [카시모 하지메]
//   · 평타 : 보랏빛 전격 타격(kashimo_strike). 서버가 전하 스택과 반격을 처리한다.
//   · 1번  : 번개 — 경직 없이 즉시 발사. 전하 4스택 대상이 화면 안에 있으면
//            서버가 자동으로 '대기를 가르는 번개'로 전환한다.
//   · 2번  : ✅ [신규] 주력 방출 — 4초간 보랏빛 에너지를 방출한다.
//            시전 중에도 자유롭게 이동할 수 있다 (경직 없음).
//   · 3번  : 아직 구현하지 않았다 (입력 자체를 막는다).
...
window.initControls = (socket) => {
    ...
    document.getElementById('btn-skill1').addEventListener('pointerdown', (e) => {
        ...
        if (charType === 'PARK') {
            ...
            // ⚡ 번개 : 경직 없이 즉시 발사한다.
            //    전하 4스택 대상이 화면 안에 있으면 서버가 '대기를 가르는 번개'로 전환한다.
    ...
    document.getElementById('btn-skill2').addEventListener('pointerdown', (e) => {
        ...
        if (charType === 'PARK') {
            ...
            //    지속 시간 동안 스킬 버튼에 남은 시간을 표시하기 위해 로컬 상태를 세운다.
    ...
    document.getElementById('btn-skill3').addEventListener('pointerdown', (e) => {
        ...
        // 🚧 카시모의 3번 스킬은 아직 구현하지 않았다
        if (charTypePre === 'KASHIMO') return;
