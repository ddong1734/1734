...
// ⚡ 카시모 하지메 전용 상태(전하 스택 · 번개 쿨타임 · 주력 방출)를 추가했다.
...
window.visualFX.push = function(data) {
    for (let i = 0; i < FX_POOL_SIZE; i++) {
        if (!this[i].active) {
            ...
            if (data.arc !== undefined) fx.arc = data.arc;   // 🦘 도약 궤적 높이 / ⚡ 주력 방출 높이
...
window.applySkillNames = () => {
    ...
    setLabel('btn-skill2', isBors ? '야타의<br>거울' : (isKuzan ? '파르티잔' : (isSaka ? '대분화' : (isEnel ? '만뢰' : (isKashimo ? '주력<br>방출' : '50m')))));
