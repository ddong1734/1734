...
// 🔥 저주의 왕 (헤이안 스쿠나) 격리 구역 — server/config.js 의 CURSE_ZONE_MIN/MAX 와 동일해야 한다
window.CURSE_ZONE_X = { min: 42900, max: 49100 };
...
// 🔥 저주의 왕 (헤이안 스쿠나)
window.serverCursePortal = null;
window.serverSukuna = null;
window.serverSukunaPortal = null;
window.serverSukunaFires = [];
window.serverOkras = []; 
...
window.myPlayer = { 
    ...
    portalDwellUntil: 0, darkDwellUntil: 0, curseDwellUntil: 0,
    darkBanned: false,
...
window.applySkillNames = () => {
    ...
    let isDaido = charType === 'DAIDO';   // ⚔️ 다이도 하가네
    ...
    let s2 = isBors ? '야타의<br>거울'
           ...
           : (isSaka ? '대분화'
           : (isEnel ? '만뢰'
           : (isKurusu ? (holyFull ? '축복<br><span style="color:#ffd23c">최대</span>' : '축복')
           : (isDaido ? '질풍참'
           : (isDabura ? '어둠'
