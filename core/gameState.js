...
// ⚡ [신규] 카시모 하지메 전용 상태(전하 스택 · 번개 쿨타임)를 추가했다.
...
// ⚡ [신규] 카시모 전하 스택 상수 (server/config.js 와 동일해야 함)
window.KASHIMO_CHARGE_MAX = 4;
window.KASHIMO_CHARGE_DECAY_MS = 8000;
...
window.myPlayer = { 
    ...
    // ⚡ [신규] 카시모 하지메 — 내가 맞은 전하 스택 / 내 번개 쿨타임
    kashimoCharge: 0,
...
// ============================================================================
// 스킬 이름 동적 UI 업데이트
//   ⚡ [신규] 카시모의 1번 스킬은 전하 4스택 대상이 화면 안에 있으면
...
window.applySkillNames = () => {
    ...
    let s1 = isBors ? '광선'
           ...
           : (isSaka ? '명구'
           : (isEnel ? '엘 토르'
           : (isKashimo ? (window.kashimoSkyReady ? '대기를<br>가르는<br>번개' : '번개')
           : '멀리뛰기'))));

    setLabel('btn-skill1', s1);
    setLabel('btn-skill2', isBors ? '야타의<br>거울' : (isKuzan ? '파르티잔' : (isSaka ? '대분화' : (isEnel ? '만뢰' : (isKashimo ? '???' : '50m')))));
    setLabel('btn-skill3', isBors ? '팔척경<br>곡옥' : (isKuzan ? '아이스<br>에이지' : (isSaka ? '유성<br>화산' : (isEnel ? '뇌영' : (isKashimo ? '???' : '디트로이트')))));

    // ⚡ 카시모 1번 스킬 버튼은 상태에 따라 색이 바뀐다
    if (typeof window.refreshKashimoSkillLabel === 'function') window.refreshKashimoSkillLabel();
};

// ============================================================================
// ⚡ [신규] 카시모 1번 스킬 라벨 · 색상 갱신
//    renderEngine 이 window.kashimoSkyReady 를 바꿀 때마다 호출한다.
//    라벨 텍스트만 바꾸고 .cd-overlay 는 그대로 유지해야 쿨타임 표시가 깨지지 않는다.
// ============================================================================
window._kashimoLabelState = null;
window.refreshKashimoSkillLabel = () => {
    let p = window.myPlayer;
    if (!p) return;

    let btn = document.getElementById('btn-skill1');
    if (!btn) return;

    if (p.characterType !== 'KASHIMO') {
        if (window._kashimoLabelState !== null) {
            window._kashimoLabelState = null;
    ...
    let ready = !!window.kashimoSkyReady;
    if (window._kashimoLabelState === ready) return;
    window._kashimoLabelState = ready;

    // .cd-overlay 를 보존한 채 텍스트만 교체한다
    let cd = btn.querySelector('.cd-overlay');
    btn.innerHTML = ready ? '대기를<br>가르는<br>번개' : '번개';
    if (cd) btn.appendChild(cd);
    else { let d = document.createElement('div'); d.className = 'cd-overlay'; btn.appendChild(d); }

    if (ready) {
        // 🟣 발동 준비 상태 — 진한 보라 + 빛나는 테두리
        btn.style.background = 'rgba(126, 34, 206, 0.95)';
        btn.style.borderColor = '#e9d5ff';
        btn.style.boxShadow = '0 0 18px 4px rgba(168, 85, 247, 0.9)';
