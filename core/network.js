...
window.initNetwork = (socket) => {
    ...
    // 💥 흔들흔들 시전 경직 (0.5초) — 이 동안 이동·평타·스킬 모두 불가
    ...
    // ============================================================
    // ⛓️ [신규] 어둠어둠열매 — 전방으로 뻗는 어둠 촉수 / 빗나갔을 때 잔상
    // ============================================================
    ...
    // 🕳️ [신규] 어둠 흡수 — 끌어온 대상을 2초간 어둠으로 빨아들인다
    socket.on('yamiAbsorb', (data) => {
        ...
        window.visualFX.push({
            type: 'yami_absorb',
            x: data.x, y: data.y,
            targetKind: data.targetKind || 'player',
    ...
    // ⛓️ 어둠어둠 — 시전자 경직 (2초 동안 이동·평타·스킬 모두 불가)
    ...
    socket.on('bossHit', (data) => { 
        ...
        if (Date.now() < (window.myPlayer.elThorLockUntil || 0) || Date.now() < (window.myPlayer.raigoPullUntil || 0) || Date.now() < (window.myPlayer.crowsPullUntil || 0) || Date.now() < (window.myPlayer.yamiBindUntil || 0) || Date.now() < (window.myPlayer.yamiLockUntil || 0) || Date.now() < (window.myPlayer.guraChargeUntil || 0)) {
            if (typeof window.flashDamageVignette === 'function') window.flashDamageVignette();
            return;
        }
