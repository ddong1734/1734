// 파일명: core/network.js
//
// 🛟 [게임 멈춤 방지] 잠금 해제 이벤트가 유실돼도 복구되도록
//     ① 잠금을 세팅하는 모든 핸들러가 '만료 시각'을 상한선과 함께 기록
//     ② 연결이 끊기거나 재연결되면 모든 잠금을 즉시 초기화
//     ③ 서버가 준 값이 비정상적으로 크면 잘라낸다
//
// ⚡ 카시모 하지메 이벤트
//   · kashimoCharge       : 전하 스택 갱신
//   · kashimoCounter      : 반격 전류 (플레이어 · 오크라)
//   · kashimoBoltCast     : 1번 스킬 시전 섬광
//   · kashimoBoltHit      : 번개 적중 전격
//   · kashimoSkyBolt      : 대기를 가르는 번개
//   · kashimoSurgeStart/End : 주력 방출
//   · kashimoAmberStart/End : ✅ [신규] 환수호박
//   · kashimoAmberTrail   : ✅ [신규] 전기 잔상
//   · kashimoWaveCast/Blast : ✅ [신규] 전자파 연쇄 폭발
//   · kashimoSonicCharge/Fire : ✅ [신규] 음파
...
window.initNetwork = (socket) => {
    ...
    const releaseAllLocks = (reason) => {
        ...
        p.lightDashUntil = 0;
        p.sonicChargeUntil = 0;                      // ⚡🔮 음파 경직도 해제
        p._cliStuckSince = 0;
    ...
    /** ⚡🔮 환수호박 오라 이펙트를 끈다 */
    const clearAmberFX = (ownerId) => {
        for (let i = 0; i < window.visualFX.length; i++) {
            let t = window.visualFX[i];
            if (t.active && t.type === 'kashimo_amber_aura' && t.ownerId === ownerId) t.active = false;
        }
    };

    /** ⚡🔮 환수호박 오라를 켠다 (중복 방지) */
    const startAmberFX = (ownerId) => {
    ...
    socket.on('reconnectSuccess', (data) => {
        ...
        clearAmberFX(window.myId);
        if (window.myPlayer.amberActive) startAmberFX(window.myId);
    ...
    socket.on('kashimoSkyBolt', (d) => {
        ...
        window.visualFX.push({
            ...
            x: d.x, y: d.y,
            x2: (d.originX !== undefined) ? d.originX : d.x,
    ...
    socket.on('kashimoAmberStart', (d) => {
        ...
        if (d.id === window.myId) {
            ...
            // 🔮 1·2번 스킬이 전용 스킬로 바뀌고 3번은 사라진다
            if (typeof window.applySkillNames === 'function') window.applySkillNames();
        }
        if (window.players[d.id]) window.players[d.id].amberActive = true;

        startAmberFX(d.id);
        if (typeof window.triggerScreenShake === 'function') window.triggerScreenShake(700, 16, false);
    });
    ...
    socket.on('kashimoAmberEnd', (d) => {
        ...
        for (let i = 0; i < window.visualFX.length; i++) {
            let t = window.visualFX[i];
            if (t.active && t.type === 'kashimo_amber_trail' && t.ownerId === d.id) t.active = false;
        }
    });
    ...
    socket.on('kashimoWaveCast', (d) => {
        ...
        if (typeof window.triggerScreenShake === 'function') window.triggerScreenShake(300, 12, false);
    });
    ...
    socket.on('kashimoSonicCharge', (d) => {
        if (!d) return;
        let dur = capUntil(Date.now() + (d.duration || 500));

        if (d.id === window.myId) {
            window.myPlayer.sonicChargeUntil = dur;
            window.myPlayer.moveX = 0; window.myPlayer.moveY = 0;
            window.joyX = 0; window.joyY = 0;
        }
        if (window.players[d.id]) window.players[d.id].sonicChargeUntil = dur;

        window.visualFX.push({
            type: 'kashimo_sonic_charge',
            x: d.x, y: d.y, dir: d.dir || 1,
            ownerId: d.id,
            durationMs: d.duration || 500,
            life: Math.round((d.duration || 500) / (1000 / 60)),
            maxLife: Math.round((d.duration || 500) / (1000 / 60))
        });
    });
    ...
    socket.on('kashimoSonicFire', (d) => {
        ...
        if (typeof window.triggerScreenShake === 'function') window.triggerScreenShake(420, 18, false);
    });
    ...
    socket.on('player_died', (deadId) => {
        ...
        for (let i = 0; i < window.visualFX.length; i++) {
            let t = window.visualFX[i];
            if (t.active && t.type === 'kashimo_amber_trail' && t.ownerId === deadId) t.active = false;
        }
        ...
        if (deadId === window.myId) {
            ...
            window.myPlayer.amberActive = false;
            window.myPlayer.amberCdEnd = 0;
            window.myPlayer.cd1 = 0; window.myPlayer.cd2 = 0; window.myPlayer.cd3 = 0;
            if (typeof window.applySkillNames === 'function') window.applySkillNames();
    ...
    socket.on('player_respawned', (pData) => {
        ...
        if (pData.id === window.myId) { 
            ...
            window.myPlayer.amberActive = false; window.myPlayer.amberCdEnd = 0;
            window.myPlayer.cd3 = 0;
            if (typeof window.applySkillNames === 'function') window.applySkillNames();
    ...
    socket.on('syncPlayerFull', (data) => {
        if (data.id === window.myId) {
            let prevAmber = !!window.myPlayer.amberActive;
            ...
            let localState = {
                ...
                kashimoBoltCdEnd: window.myPlayer.kashimoBoltCdEnd,
                surgeCdEnd: window.myPlayer.surgeCdEnd,
                waveCdEnd: window.myPlayer.waveCdEnd,          // ⚡🔮 로컬 쿨타임 유지
                sonicCdEnd: window.myPlayer.sonicCdEnd,
                amberCdEnd: window.myPlayer.amberCdEnd,
                cd1: window.myPlayer.cd1, cd2: window.myPlayer.cd2, cd3: window.myPlayer.cd3,
            ...
            // ⚡🔮 환수호박 상태가 바뀌었다면 스킬 버튼을 갱신한다
            if (prevAmber !== !!window.myPlayer.amberActive) {
                if (window.myPlayer.amberActive) startAmberFX(window.myId); else clearAmberFX(window.myId);
                if (typeof window.applySkillNames === 'function') window.applySkillNames();
            }
            ...
            if (t) {
                ...
                if (prevAmber !== !!t.amberActive) {
                    if (t.amberActive) startAmberFX(data.id); else clearAmberFX(data.id);
                }
                ...
                if (data.amberActive) startAmberFX(data.id);
            }
        }
    });
    ...
    socket.on('enemyUpdate', (delta) => { 
        ...
        if (!t) { window.players[delta.id] = initNet(delta); if (delta.amberActive) startAmberFX(delta.id); return; }
        ...
        // ⚡🔮 환수호박 오라 동기화
        if (prevAmber !== !!t.amberActive) {
            if (t.amberActive) startAmberFX(delta.id); else clearAmberFX(delta.id);
        }
    ...
    socket.on('playerLeft', (id) => { clearSurgeFX(id); clearAmberFX(id); delete window.players[id]; });
    ...
    socket.on('bossHit', (data) => { 
        ...
        if (now < (window.myPlayer.elThorLockUntil || 0) || now < (window.myPlayer.raigoPullUntil || 0)
            ...
            || now < (window.myPlayer.sonicChargeUntil || 0)) {
            if (typeof window.flashDamageVignette === 'function') window.flashDamageVignette();
            return;
        }
    ...
    socket.on('heal', (amt) => {
        ...
        if (window.myPlayer.amberActive) return;
        window.myPlayer.hp = Math.min(window.myPlayer.maxHp, window.myPlayer.hp + amt);
        window.visualFX.push({ x: window.myPlayer.x, y: window.myPlayer.y - 50, life: 30, maxLife: 30, type: 'heal', val: Math.round(amt) });
    });
    ...
    socket.on('actionEffect', (data) => {
        ...
        else if(data.type === 'kashimo_strike') { data.life = data.life || 13; data.maxLife = data.maxLife || 13; }
