...
window.initNetwork = (socket) => {
    ...
    socket.on('reconnectSuccess', (data) => {
        ...
        window.serverDarkPortal = data.darkPortal || null;                  // 🟣 암흑 왕좌 포탈
        window.serverBlackbeard = data.blackbeard || null;                  // ⚫ 검은수염
        window.serverBlackbeardPortal = data.blackbeardPortal || null;      // 🌀 검은수염 귀환 포탈
    ...
    // 🟣 암흑 왕좌 포탈 동기화 (박힌범 처치 시 25% 확률로 생성)
    ...
    // 🌀 포탈 대기 카운트다운 동기화
    //    until 이 0 이면 포탈에서 벗어난 것 → 카운트다운 즉시 제거(3초 초기화)
    ...
    socket.on('blackbeardUpdate', (delta) => {
        ...
        if (delta.risingUntil !== undefined) b.risingUntil = delta.risingUntil;      // ✅ 어둠 소용돌이 상승 구간
        if (delta.descentUntil !== undefined) b.descentUntil = delta.descentUntil;   // ✅ 공중 강림 구간
    ...
    // 🌊 블랙홀(암흑물질 장판) 시작 — 맵 바닥 전체를 뒤덮는다 (4초)
    socket.on('darkFloorStart', (data) => {
        if (!data) return;
        // 이미 표시 중인 장판은 새 것으로 교체
    ...
    // 🔴 크로우즈 예고 — 시전 1초 전 빨간색 조준 표시 (검은수염 경직)
    socket.on('crowsTelegraph', (data) => {
        ...
        window.visualFX.push({
            ...
            radius: data.thickness ? data.thickness / 2 : 135,
            durationMs: dur,
            life: Math.round(dur / (1000 / 60)),
            maxLife: Math.round(dur / (1000 / 60))
        });
    ...
    // ⛓️ 크로우즈 발동 — 암흑물질이 길게 뻗어 대상을 빠르게 끌어온다 (스킬 봉인)
    ...
    socket.on('guraImpact', (data) => {
        ...
        window.visualFX.push({
            type: 'gura_impact',
            x: data.x, y: data.y,
            radius: data.radius || 283,     // ✅ 판정 상향(3.0배)에 맞춘 기본값
            life: 34, maxLife: 34
        });
        if (typeof window.triggerScreenShake === 'function') window.triggerScreenShake(520, 24);
    });
    ...
    socket.on('darkRise', (data) => {
        ...
        window.visualFX.push({
            ...
            y: data.fromY,
            y2: data.fromY,          // 출발 Y
            radius: data.toY,        // 도착 Y (radius 필드를 재사용)
            durationMs: dur,
            life: Math.round(dur / (1000 / 60)),
            maxLife: Math.round(dur / (1000 / 60))
        });
        if (typeof window.triggerScreenShake === 'function') window.triggerScreenShake(dur, 8);
    });

    // 🌑 공중 강림 — 5초간 공중에서 강림 이펙트를 쏟아낸다
    socket.on('descentStart', (data) => {
        ...
        // ✅ 흔들림을 20 → 11 로 완화 (웅장하되 어지럽지 않게)
        if (typeof window.triggerScreenShake === 'function') window.triggerScreenShake(dur, 11);
    });
    ...
    socket.on('player_died', (deadId) => {
        if (deadId === window.myId) {
            ...
            window.myPlayer.yataActive = false; 
            window.myPlayer.yataPath = null;
            window.myPlayer.portalDwellUntil = 0;                                  // 🌀 사망 시 카운트다운 제거
            window.myPlayer.darkDwellUntil = 0;                                    // 🟣 사망 시 카운트다운 제거
            window.myPlayer.crowsPullUntil = 0;                                    // ⚫ 크로우즈 해제
            ...
            window.players[deadId].portalDwellUntil = 0;                           // 🌀 사망 시 카운트다운 제거
            window.players[deadId].darkDwellUntil = 0;                             // 🟣 사망 시 카운트다운 제거
            window.players[deadId].crowsPullUntil = 0;                             // ⚫ 크로우즈 해제
    ...
    socket.on('player_respawned', (pData) => {
        if (pData.id === window.myId) { 
            ...
            window.myPlayer.electrocutedUntil = 0; 
            window.myPlayer.lightDashUntil = 0; 
            window.myPlayer.portalDwellUntil = 0;                                  // 🌀 부활 시 카운트다운 제거
            window.myPlayer.darkDwellUntil = 0;                                    // 🟣 부활 시 카운트다운 제거
            window.myPlayer.crowsPullUntil = 0;                                    // ⚫ 크로우즈 해제
        ...
        if (window.players[pData.id]) { 
            ...
            window.players[pData.id].portalDwellUntil = 0;                         // 🌀 부활 시 카운트다운 제거
            window.players[pData.id].darkDwellUntil = 0;                           // 🟣 부활 시 카운트다운 제거
            window.players[pData.id].crowsPullUntil = 0;                           // ⚫ 크로우즈 해제
    ...
    socket.on('teleport', (pos) => {
        ...
        window.myPlayer.portalDwellUntil = 0;                                      // 🌀 이동 완료 → 카운트다운 제거
        window.myPlayer.darkDwellUntil = 0;                                        // 🟣 이동 완료 → 카운트다운 제거
    ...
    socket.on('bossHit', (data) => { 
        ...
        if (Date.now() < (window.myPlayer.elThorLockUntil || 0) || Date.now() < (window.myPlayer.raigoPullUntil || 0) || Date.now() < (window.myPlayer.crowsPullUntil || 0)) {
            if (typeof window.flashDamageVignette === 'function') window.flashDamageVignette();
            return;
        }
