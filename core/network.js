...
window.initNetwork = (socket) => {
    ...
    socket.on('player_respawned', (pData) => {
        ...
        if (window.players[pData.id]) { 
            ...
            window.players[pData.id].x = pData.x; window.players[pData.id].y = pData.y;
    ...
    socket.on('teleport', (pos) => { window.myPlayer.x = pos.x; window.myPlayer.y = pos.y; });
    ...
    socket.on('yataEnd', (data) => {
        if (data.id === window.myId) {
            ...
            window.players[data.id].x = data.x; window.players[data.id].y = data.y;
        }
    ...
    socket.on('enemyUpdate', (delta) => { 
        if (window.players[delta.id]) { 
            ...
            if (Date.now() - lastHit > 800) {
                ...
                if (delta.x !== undefined) window.players[delta.id].x = delta.x;
                if (delta.y !== undefined) window.players[delta.id].y = delta.y;
                if (delta.lastFacing !== undefined) window.players[delta.id].lastFacing = delta.lastFacing;
            }
        } else { 
            window.players[delta.id] = delta; 
        } 
    });
