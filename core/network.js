...
window.initNetwork = (socket) => {
    ...
    // ⛓️ 크로우즈 발동 — 암흑물질이 길게 뻗어 대상을 끌어온다 (스킬 봉인)
    socket.on('crowsStart', (data) => {
        if (!data) return;
        let dur = data.duration || 900;
    ...
    socket.on('guraImpact', (data) => {
        ...
        window.visualFX.push({
            type: 'gura_impact',
            x: data.x, y: data.y,
            radius: data.radius || 113,
            life: 34, maxLife: 34
        });
        if (typeof window.triggerScreenShake === 'function') window.triggerScreenShake(520, 26);
    });
    ...
    socket.on('descentStart', (data) => {
        ...
        // 5초 내내 화면이 웅장하게 흔들린다
        if (typeof window.triggerScreenShake === 'function') window.triggerScreenShake(dur, 20);
    });
