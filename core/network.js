...
window.initNetwork = (socket) => {
    ...
    socket.on('burgessSpawn', (data) => {
        ...
        if (typeof window.triggerScreenShake === 'function') window.triggerScreenShake(700, 10, true);
    });
    ...
    socket.on('burgessDespawn', () => {
        ...
        for (let i = 0; i < window.visualFX.length; i++) {
            ...
            if (t.active && (t.type === 'burgess_telegraph' || t.type === 'burgess_blast' || t.type === 'burgess_spawn')) t.active = false;
        }
    });
    ...
    socket.on('burgessJump', (data) => {
        ...
        window.visualFX.push({
            type: 'burgess_jump',
            x: data.fromX, y: data.y,
            x2: data.toX, y2: data.y,
            radius: data.radius || 76,
    ...
    socket.on('burgessBlast', (data) => {
        ...
        if (typeof window.triggerScreenShake === 'function') window.triggerScreenShake(380, 16, true);
    });
