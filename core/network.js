...
window.initNetwork = (socket) => {
    ...
    //    검은수염 파공아와 동일한 이펙트를 쓰되, 암흑 왕좌 밖에서도 흔들린다.
    // ============================================================
    ...
    // ============================================================
    // ⛓️ [신규] 어둠어둠열매 — 플레이어 평타 크로우즈
    // ============================================================
    socket.on('playerYamiBeam', (data) => {
        if (!data) return;
        let dur = data.duration || 420;
        window.visualFX.push({
            type: 'crows_beam',
    ...
    socket.on('player_died', (deadId) => {
        if (deadId === window.myId) {
            ...
            window.myPlayer.yamiLockUntil = 0; window.myPlayer.yamiBindUntil = 0;
            ...
            window.players[deadId].yamiLockUntil = 0; window.players[deadId].yamiBindUntil = 0;
    ...
    socket.on('player_respawned', (pData) => {
        ...
        if (window.players[pData.id]) { 
            ...
            window.players[pData.id].yamiLockUntil = 0; window.players[pData.id].yamiBindUntil = 0;
