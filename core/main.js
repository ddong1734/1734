...
setInterval(() => {
    ...
    safeTick('physics', () => {
        if (!pObj.isDead) { 
            ...
            // ✨ [쿠루스 하나] 지나간 자리에 1.5초짜리 황금빛 잔상을 남긴다.
            //    너무 촘촘하면 화면이 지저분해지므로 최소 이동 거리와 간격을 둔다.
            if (pObj.characterType === 'KURUSU' && !pObj.isDead) {
                // ⚠️ 첫 기준점을 '현재 좌표' 로 잡으면 이동량이 늘 0이라
                //    잔상이 영원히 안 생긴다. 좌표가 아직 없을 때만 초기화한다.
                if (window._kurusuTrailX === undefined) {
                    window._kurusuTrailX = pObj.x;
                    window._kurusuTrailY = pObj.y;
                    window._kurusuTrailAt = 0;
                }
                const moved = Math.hypot(pObj.x - window._kurusuTrailX,
                                         pObj.y - window._kurusuTrailY);
                if (loopNow - (window._kurusuTrailAt || 0) >= 60 && moved > 8) {
                    window._kurusuTrailAt = loopNow;
                    window._kurusuTrailX = pObj.x;
                    window._kurusuTrailY = pObj.y;
                    window.visualFX.push({
                        type: 'kurusu_trail', x: pObj.x, y: pObj.y,
