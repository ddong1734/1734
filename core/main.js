    ...
    safeTick('physics', () => {
        if (!pObj.isDead) { 
            ...
            if (pObj.characterType === 'KURUSU' && !pObj.isDead) {
                if (!window._kurusuTrailAt) window._kurusuTrailAt = 0;
                const moved = Math.hypot(pObj.x - (window._kurusuTrailX || pObj.x),
                                         pObj.y - (window._kurusuTrailY || pObj.y));
                if (loopNow - window._kurusuTrailAt >= 70 && moved > 16) {
