...
setInterval(() => {
    ...
    safeTick('cdUI', () => {
        if (typeof window.updateCDUI === 'function') {
            ...
            // 🗡️ 세계를 가르는 참격 — 아이템을 장착했을 때만 갱신한다
            if (pObj.hasWorldCleave) {
        ...
        if (!pObj.isDead && !pObj.npcTalking) {
            ...
            for (let i = 0; i < list.length; i++) {
                ...
                if (n.kind === 'mahera') {
                    ...
                    if (st >= 2) continue;                     // ✅ 완료
                    // 🗡️ 진행 중이면 법진 + 퇴마의 검을 모두 갖고 있어야 한다
                    if (st === 1 && !(inv.some(i2 => i2.id === 'beopjin') && inv.some(i2 => i2.id === 'toema_sword'))) continue;
                } else {
