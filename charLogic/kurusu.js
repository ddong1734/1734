...
function useGather(p, ctx) {
    ...
    if (typeof forEach === 'function') {
        ...
        forEach(p, hitTest, (t) => {
            const o = t.obj;
            if (!o || o.hp === undefined || o.hp <= 0 || o.state === 'dead') return;
            // 😈 타천 : 대상마다 30% 확률로 2를 얻는다
            count += (p.hasTacheon && Math.random() < 0.30) ? 2 : 1;
...
function useLadder(p, ctx) {
    ...
    p.frozenUntil = Math.max(p.frozenUntil || 0, now + S3.castTime + beamMs);
    ...
    for (let pid in State.players) {
        ...
        if (typeof forEach === 'function') {
            ...
            forEach(p, hitTest, (t) => {
                ...
                if (t.obj) t.obj.frozenUntil = Math.max(t.obj.frozenUntil || 0, p.ladderBeamEnd);
                if (t.kind === 'player') {
                    shakeIds.push(t.id);
