...
// ⚫ [추가] 엘 토르 / 뇌영 데미지가 검은수염(암흑 왕좌 보스)에게도 들어가도록 추가
...
function elThorTick(p, now, ctx) {
    ...
    let mobDmg = (typeof ctx.toemaDmg === 'function') ? ctx.toemaDmg(p, dmg) : dmg;
    ...
    if (monster.hp > 0 && inBeam(monster.x, monster.y)) {
        monster.hp -= mobDmg; monster.targetId = p.id; monster.state = 'chase';
        ...
        emitDamageText(monster.x, monster.y, mobDmg);
    ...
    if (hinbeom && hinbeom.hp > 0 && hinbeom.state !== 'dead' && inBeam(hinbeom.x, hinbeom.y)) {
        if (!hinbeomInvincible) {
            hinbeom.hp -= mobDmg;
            if (typeof ctx.recordHinbeomDamage === 'function') ctx.recordHinbeomDamage(p.id, mobDmg);   // 🏆 피해량 기록
            ...
            emitDamageText(hinbeom.x, hinbeom.y, mobDmg);
    ...
    if (bb && bb.hp > 0 && bb.state !== 'dead' && inBeam(bb.x, bb.y)) {
        bb.hp -= mobDmg;
        bb.frozenUntil = Math.max(bb.frozenUntil || 0, p.elThorEnd);
        bb.electrocutedUntil = Math.max(bb.electrocutedUntil || 0, p.elThorEnd);
        bb.airFreezeUntil = Math.max(bb.airFreezeUntil || 0, p.elThorEnd);
        emitDamageText(bb.x, bb.y, mobDmg);
    ...
    for (let mi = minions.length - 1; mi >= 0; mi--) {
        ...
        if (inBeam(mn.x, mn.y)) {
            mn.hp -= mobDmg; mn.targetId = p.id; mn.state = 'chase';
            ...
            emitDamageText(mn.x, mn.y, mobDmg);
    ...
    okras.forEach(ok => {
        if (ok.hp > 0 && inBeam(ok.x, ok.y)) {
            ok.hp -= mobDmg; ok.targetId = p.id; ok.state = 'chase';
            ...
            emitDamageText(ok.x, ok.y, mobDmg);
...
function raigoTick(p, now, ctx) {
    ...
    // ⚫ [추가] 검은수염 (암흑 왕좌 보스)
    let bb = (typeof ctx.getBlackbeard === 'function') ? ctx.getBlackbeard() : null;
    if (bb && bb.hp > 0 && bb.state !== 'dead' && bb.x >= minX && bb.x <= maxX) {
        bb.hp -= mobDmg;
        bb.frozenUntil = Math.max(bb.frozenUntil || 0, p.raigoCastEnd);
        bb.electrocutedUntil = Math.max(bb.electrocutedUntil || 0, p.raigoCastEnd);
        emitDamageText(bb.x, bb.y, mobDmg);
        if (typeof ctx.aggroBlackbeard === 'function') ctx.aggroBlackbeard(p.id);
        if (bb.hp <= 0 && typeof ctx.killBlackbeard === 'function') ctx.killBlackbeard(p.id);
    }
