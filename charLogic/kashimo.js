...
//   · 2번 [주력 방출]         : 4초간 방출 · 시전 중 완전 고정
//                               🏵️ 여의 : 좌우 범위 증가
//   · 3번 [환수호박]          : 이동속도 1.3배 · 죽을 때까지 해제 불가
//
//   [환수호박 각성]
//   · 평타 : ✅ [신규] 조이스틱 방향으로 전격 순간이동 (바제스 지름만큼)
//            가로벽은 통과, 세로벽(solid)은 통과 불가.
//            출발↔도착 사이 전류 잔상 + 도착지 전기폭발 (150 · 기절 1초)
//   · 1번 [전자파] : 연쇄 전기폭발
//                    🌩️ 뇌신 : 범위·거리 증가 + 0.3초 뒤 전체 자리 동시 재폭발
//   · 2번 [음파]   : 0.5초 경직 후 부채꼴
//                    🌩️ 뇌신 : 음파 범위로 번개 7발 동시 발사
// ============================================================================
 ...
 */
function spawnBolt(p, dirX, dirY, ctx) {
...
// ============================================================================
// ⚡🔮 [신규] 환수호박 평타 = 전격 순간이동 (블링크)
//    · 조이스틱 방향으로 '지저스 바제스의 지름'만큼 이동한다.
//    · 가로벽(발판)은 통과할 수 있지만, 세로벽(solid)은 통과할 수 없다.
//    · 출발지 ↔ 도착지 사이에 전류 잔상이 남는다.
//    · 도착지에 전기 폭발 (150 피해 · 1초 기절)
// ============================================================================
 ...
 */
function resolveBlinkPath(fromX, fromY, ux, uy, dist, ctx) {
    const State = ctx.State;
...
/** ⚡🔮 환수호박 평타 → 전격 순간이동 */
function doBlink(p, data, ctx) {
    const { io, Skills, State } = ctx;
    const KS3 = Skills.KASHIMO_S3;
    let now = Date.now();

    if (now < (p.blinkCdEnd || 0)) return;
    p.blinkCdEnd = now + ((KS3 && KS3.blinkCooldown) ? KS3.blinkCooldown : C.K_BLINK_COOLDOWN);
    ...
    const dist = (KS3 && KS3.blinkDist) ? KS3.blinkDist : C.K_BLINK_DIST;

    let fromX = p.x, fromY = p.y;
    // 🧱 세로벽은 통과 불가 — 경로를 따라가며 막히면 멈춘다
    let dest = resolveBlinkPath(fromX, fromY, ux, uy, dist, ctx);
    ...
    // ⚡ 출발지 ↔ 도착지 사이 전류 잔상
    io.emit('kashimoBlink', {
        ownerId: p.id,
        fromX: fromX, fromY: fromY,
        toX: dest.x, toY: dest.y,
        duration: C.K_BLINK_FX_MS
    });

    // 💥 도착지 전기 폭발 (150 · 기절 1초)
    const bDmg = (KS3 && KS3.blastDamage) ? KS3.blastDamage : C.K_BLINK_BLAST_DMG;
    const bStun = (KS3 && KS3.blastStun) ? KS3.blastStun : C.K_BLINK_BLAST_STUN;
    const bR = (KS3 && KS3.blastRadius) ? KS3.blastRadius : C.K_BLINK_BLAST_RADIUS;

    io.emit('kashimoBlinkBlast', {
        ownerId: p.id, x: dest.x, y: dest.y,
        radius: bR, duration: C.K_BLINK_BLAST_FX_MS
    });

    blastAt(p, dest.x, dest.y, bR, bDmg, bStun, ctx, {});

    // 📡 순간이동은 서버가 좌표를 강제로 옮겼으므로 본인에게도 알린다
    io.to(p.id).emit('teleport', { x: dest.x, y: dest.y });
    io.emit('syncPlayerFull', p);
}
 ...
 *    ⚡🔮 환수호박 중이라면 평타 대신 전격 순간이동이 나간다.
 *    @returns true 를 반환하면 일반 평타 처리를 건너뛴다
 */
function tryAmberBlink(p, data, ctx) {
