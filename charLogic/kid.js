...
function useAssign(p, ctx) {
    ...
    if (typeof forEach === 'function') {
        ...
        forEach(p, inR, (t) => {
            ...
            // 🛟 [안전장치] 고철 표식 상한 — 같은 대상 중복도 함께 정리한다
            if (ctx.State.kidMarks.length >= 120) ctx.State.kidMarks.shift();
            // 같은 시전자가 같은 대상에 이미 붙여 뒀다면 새로 갈아 끼운다
            for (let q = ctx.State.kidMarks.length - 1; q >= 0; q--) {
                const om = ctx.State.kidMarks[q];
                if (om.ownerId === p.id && om.targetId === t.id) {
                    clearMark(om.obj);
                    ctx.State.kidMarks.splice(q, 1);
                }
            }
...
function processMarks(now, ctx) {
    ...
    for (let i = State.kidMarks.length - 1; i >= 0; i--) {
        ...
        if (now >= m.holdEnd) {
            ...
            // 💥 공격력 반영 (폭발이라 계수 1.0)
            hitOne(owner, { obj: o, kind: m.kind, id: m.targetId }, S1.blastDamage + Math.round((owner.bonusDamage || 0) * 1.0), ctx);
            clearMark(o);
            State.kidMarks.splice(i, 1);
            continue;
        }
        ...
        // 쌓일수록 느려진다 (100% → 25%)
        //   🧲 자기자기열매 : 감속이 더 강해지고(→8%) 점프 높이도 낮아진다
        const minSpd = owner.hasMagnetFruit ? 0.08 : S1.slowMin;
        o.kidSlow = 1 - (1 - minSpd) * o.kidStack;
        if (owner.hasMagnetFruit) o.kidJumpCut = 1 - 0.6 * o.kidStack;
        else o.kidJumpCut = 1;

        if (now >= m.nextTick) {
            m.nextTick = now + S1.tickInterval;
            // 💥 공격력 반영 (연타형이라 계수 0.2)
            hitOne(owner, { obj: o, kind: m.kind, id: m.targetId }, S1.tickDamage + Math.round((owner.bonusDamage || 0) * 0.2), ctx);
        }
        if (m.kind === 'player') ctx.io.emit('syncPlayerFull', o);
    }
}
...
function useLaser(p, data, ctx) {
    ...
    // 🧲 자기자기열매 : 차징 시간이 절반으로 줄어든다
    const castMs = p.hasMagnetFruit ? Math.round(S2.castTime / 2) : S2.castTime;
    p.kidLaserCastEnd = now + castMs;
    p.kidLaserFireEnd = 0;
    p.kidLaserNextTick = 0;

    // 🎯 [수정] 시전 순간의 이동키 방향을 그대로 각도로 삼는다.
    //    예전에는 좌우(0 / π) 만 잡아 위·아래로 쏘면 엉뚱한 데로 나갔다.
    let ax = Number(data && data.dirX), ay = Number(data && data.dirY);
    if (!Number.isFinite(ax) || !Number.isFinite(ay) || (ax === 0 && ay === 0)) {
        ax = dir; ay = 0;
    }
    p.kidLaserAngle = Math.atan2(ay, ax);
    p.kidLaserX = p.x; p.kidLaserY = p.y;

    ctx.io.emit('kidLaserCast', {
        id: p.id, x: p.x, y: p.y,
        castMs: castMs, angle: p.kidLaserAngle,
...
function processLaser(now, ctx) {
    ...
    for (let pid in State.players) {
        ...
        if (typeof forEach === 'function') {
            ...
            // 💥 공격력 반영 (연타형이라 계수 0.2)
            const lDmg = S2.tickDamage + Math.round((p.bonusDamage || 0) * 0.2);
            forEach(p, hitTest, (t) => hitOne(p, t, lDmg, ctx));
        }
...
function processGolem(now, ctx) {
    ...
    for (let pid in State.players) {
        ...
        if (p.kidGolemCastEnd) {
            ...
            // 🗿 골렘은 방어력이 50% 늘어난다 (원래 값을 기억해 두고 복구한다)
            p._kidDefBackup = p.defense || 0;
            p.defense = Math.min(0.9, (p.defense || 0) + 0.5);
        ...
        if (p.isDead || now >= p.kidGolemEnd) {
            p.kidGolemEnd = 0;
            // 방어력 원복
            if (p._kidDefBackup !== undefined) { p.defense = p._kidDefBackup; p._kidDefBackup = undefined; }
...
const FIELD_RADIUS = 460;      // 🦾 기계 의수 자기장 반경 (보통보다 조금 작게)

/** 🦾 [기계 의수] 골렘 주변 자기장 — 감속 · 점프 저하 · 약한 끌어당김 */
function processMagField(now, ctx) {
    const State = ctx.State;
    for (let pid in State.players) {
        const g = State.players[pid];
        if (!g || !g.hasMechArm) continue;
        if (!g.kidGolemEnd || now >= g.kidGolemEnd) continue;

        for (let tid in State.players) {
            const o = State.players[tid];
            if (!o || o.isDead || o.team === g.team) continue;
            const dx = g.x - o.x, dy = g.y - o.y;
            const d = Math.hypot(dx, dy);
            if (d > FIELD_RADIUS || d < 1) continue;

            // 이동속도 · 점프 높이 절반
            o.kidFieldUntil = now + 200;      // 잠깐만 유지 → 벗어나면 바로 풀린다
            // 약하게 끌려온다
            const pull = 1.6;
            o.x += (dx / d) * pull;
            o.y += (dy / d) * pull * 0.5;
