...
/**
 * 🔥 매 프레임 : 받은 피해를 게이지에 쌓는다.
 *
 *   예전에는 io.emit('syncPlayerFull') 을 가로채 집계했는데,
 *   그 방송이 안 나가는 피해 경로에서는 게이지가 늦게 차올랐다.
 *   이제 매 프레임 체력을 직접 비교해 '줄어든 만큼' 즉시 쌓는다.
 *   (회복은 무시한다)
 */
function processGauge(now, ctx) {
    const State = ctx.State;
    for (let pid in State.players) {
        const p = State.players[pid];
        if (!p || p.characterType !== 'MARCO') continue;

        if (p._marcoLastHp === undefined) { p._marcoLastHp = p.hp; continue; }

        // 부활·최대체력 변화로 체력이 늘면 기준만 갱신한다
        if (p.hp >= p._marcoLastHp) { p._marcoLastHp = p.hp; continue; }

        const lost = p._marcoLastHp - p.hp;
        p._marcoLastHp = p.hp;
        if (lost <= 0) continue;

        // 불꽃이 켜져 있는 동안에는 쌓이지 않는다
        if (p.marcoRegenUntil && now < p.marcoRegenUntil) continue;

        p.marcoGauge = (p.marcoGauge || 0) + lost;
        if (p.marcoGauge >= GAUGE_MAX) {
            p.marcoGauge = 0;
            p.marcoRegenUntil = now + REGEN_MS;
            p.marcoRegenNext = now;
...
function useBall(p, data, ctx) {
    ...
    // 🛟 [안전장치] 덩어리 상한
    if (ctx.State.marcoBalls.length >= 24) ctx.State.marcoBalls.shift();
...
function processBalls(now, ctx) {
    ...
    for (let i = State.marcoBalls.length - 1; i >= 0; i--) {
        ...
        if (now >= b.endAt) {
            ...
            // 💥 공격력 반영 (폭발이라 계수 0.5)
            sweep(owner, b.x, b.y, S1.blastRadius, S1.blastDamage + Math.round((owner.bonusDamage || 0) * 0.5), ctx);
        ...
        if (typeof forEach === 'function') {
            ...
            forEach(owner, near, (t) => {
                ...
                if (gd < S1.radius && b.hitIds.indexOf(t.id) === -1) {
                    b.hitIds.push(t.id);
                    // 💥 공격력 반영 (단발 접촉이라 계수 1.0)
                    hitOne(owner, t, S1.contactDamage + Math.round((owner.bonusDamage || 0) * 1.0), ctx);
                    // 🟡 황금 벨트 : 닿은 대상에게 화상
...
function processField(now, ctx) {
    ...
    for (let pid in State.players) {
        ...
        if (p.marcoCastEnd) {
            ...
            // 🛟 [안전장치] 비정상 상황에서 장판이 무한히 쌓이면 서버가 느려진다.
            //    가장 오래된 것부터 버려 상한을 지킨다.
            if (State.marcoFields.length >= 24) State.marcoFields.shift();
    ...
    for (let i = State.marcoFields.length - 1; i >= 0; i--) {
        ...
        // 💥 공격력 반영 (연타형이라 계수 0.2)
        sweep(owner, f.x, f.y, FR, S2.tickDamage + Math.round((owner.bonusDamage || 0) * 0.2), ctx);
    ...
    for (let pid in State.players) {
        ...
        if (p.hasPhoenixFruit && typeof ctx.forEachTarget === 'function') {
            ...
            ctx.forEachTarget(p, inR, (t) => {
                hitOne(p, t, S3.blastDamage + Math.round((p.bonusDamage || 0) * 1.0), ctx);
                applyBurn(p, t, ctx);
            });
        } else {
            // 💥 공격력 반영 (단발 대형기라 계수 1.0)
            sweep(p, p.marcoShieldX, p.marcoShieldY, S3.blastRadius, S3.blastDamage + Math.round((p.bonusDamage || 0) * 1.0), ctx);
        }
        ctx.io.emit('syncPlayerFull', p);
    }
}
