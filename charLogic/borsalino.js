...
//
// 🥊 야타의 거울 '빔 대시' 통과 데미지가 박힌범 · 소환체 · 검은수염 · 바제스에게 적용
// 🏆 박힌범 피해량 기록 → '박힌범 오크라' 드롭 판정(2500 이상)에 반영
// ⚫ [수정] 암흑 왕좌 안에서 야타를 쓰면 원래 맵으로 튕겨 나가던 문제 해결
//    · 시전 위치가 암흑 왕좌 구역이면 경로 전체를 그 구역 안으로 클램프
//    · 암흑 왕좌 바닥(y=2000) 아래로는 절대 내려가지 않도록 하한 고정

/** ⚫ 암흑 왕좌 구역 판정용 상수 (index.js / gameState.js 와 동일해야 함) */
const DARK_ZONE_MIN = 35400;
const DARK_ZONE_MAX = 41600;
const DARK_AREA_MIN_X = 36000;
const DARK_AREA_MAX_X = 41000;
const DARK_GROUND_Y   = 2000;
const DARK_CEIL_Y     = 600;      // 암흑 왕좌 천장(= DARK_AREA.minY)
...
function buildYataPath(sx, sy, dirX, dirY, range) {
    ...
    // ⚫ [수정] 시전 시작 지점이 암흑 왕좌 구역이면, 경로 전체를 그 구역 안으로만 제한한다.
    //    (기존에는 32000 기준으로 클램프해 원래 맵으로 순간이동되는 버그가 있었다)
    const inDarkZone = (sx >= DARK_ZONE_MIN && sx <= DARK_ZONE_MAX);

    if (inDarkZone) {
        const HALF_W = 45;
        const minX = DARK_AREA_MIN_X + HALF_W + 5;
        const maxX = DARK_AREA_MAX_X - HALF_W - 5;
        const minY = DARK_CEIL_Y + 45;              // 천장 아래
        const maxY = DARK_GROUND_Y - 45;            // ✅ 바닥 아래로 내려가지 않는다
        pts.forEach(pt => {
            pt.x = Math.max(minX, Math.min(maxX, pt.x));
            pt.y = Math.max(minY, Math.min(maxY, pt.y));
        });
    } else {
        // 🌍 일반 맵 : 기존 동작 유지 (단, 암흑 왕좌 구역으로는 넘어가지 못하게 상한을 둔다)
        const worldMaxX = DARK_ZONE_MIN - 50;
        pts.forEach(pt => {
            pt.x = Math.max(50, Math.min(worldMaxX, pt.x));
            // 🌌 중앙 정글 상층부(음수 Y)에서 시전해도 지상으로 강제 하강하지 않도록 상한 확장
...
function updateLoop(p, now, ctx) {
    ...
    if (p.yataActive) {
        ...
        if (pos) {
            // ⚫🔥 [버그 수정] 별세계(암흑 왕좌 · 저주의 왕) 안에서는 경로를 따라가더라도
            //    구역 밖으로 튕겨 나가면 안 된다. 서버가 좌표를 직접 밀어 넣는 자리이므로
            //    여기서 막지 않으면 클라이언트 물리로도 되돌릴 수 없다.
            let nx = pos.x, ny = pos.y;
            if (typeof ctx.clampSpecialArea === 'function') {
                let c = ctx.clampSpecialArea(p, nx, ny);
                nx = c.x; ny = c.y;
            }
            p.x = nx; p.y = ny;
        }
        ...
        if (now - (p.yataLastHitScan || 0) >= 60) {
            ...
            // ⚔️ 퇴마의 검 : 몬스터에게만 30% 추가 피해 (적 플레이어에는 미적용)
            let mobBeamDmg = (typeof ctx.toemaDmg === 'function') ? ctx.toemaDmg(p, beamDmg) : beamDmg;
            ...
            if (monster.hp > 0 && Math.hypot(monster.x - p.x, monster.y - p.y) < monster.radius + 60 && !p.yataHitIds.includes('monster')) {
                p.yataHitIds.push('monster'); monster.hp -= mobBeamDmg; monster.targetId = p.id; monster.state = 'chase';
                emitDamageText(monster.x, monster.y, mobBeamDmg);
                if (monster.hp <= 0) { p.gold += 2500; io.to(p.id).emit('updateGold', p.gold); gainXp(p, 200); if (typeof ctx.addBeopjinKill === 'function') ctx.addBeopjinKill(p.id); monster.targetId=null; monster.state='dead'; setTimeout(()=>{ monster.hp=monster.maxHp; monster.x=monster.homeX; monster.y=837; monster.state='idle'; },30000); }
            }

            // 🥊 박힌범 (할배새끼가 살아있는 동안은 무적)
            ...
            if (hinbeom && hinbeom.hp > 0 && hinbeom.state !== 'dead' && !hinbeomInvincible
                ...
                hinbeom.hp -= mobBeamDmg;
                if (typeof ctx.recordHinbeomDamage === 'function') ctx.recordHinbeomDamage(p.id, mobBeamDmg);
                emitDamageText(hinbeom.x, hinbeom.y, mobBeamDmg);
            ...
            if (bb && bb.hp > 0 && bb.state !== 'dead'
                ...
                bb.hp -= mobBeamDmg;
                emitDamageText(bb.x, bb.y, mobBeamDmg);
            ...
            // 🟪 지저스 바제스
            let bg = (typeof ctx.getBurgess === 'function') ? ctx.getBurgess() : null;
            let bgAlive = (typeof ctx.burgessAlive === 'function') ? ctx.burgessAlive() : false;
            if (bg && bgAlive
                && Math.hypot(bg.x - p.x, bg.y - p.y) < bg.radius + 60
                && !p.yataHitIds.includes('burgess')) {
                p.yataHitIds.push('burgess');
                bg.hp -= mobBeamDmg;
                emitDamageText(bg.x, bg.y, mobBeamDmg);
                if (typeof ctx.aggroBurgess === 'function') ctx.aggroBurgess(p.id);
                if (bg.hp <= 0 && typeof ctx.killBurgess === 'function') ctx.killBurgess(p.id);
            }
            ...
            for (let mi = minions.length - 1; mi >= 0; mi--) {
                ...
                if (Math.hypot(mn.x - p.x, mn.y - p.y) < mn.radius + 60 && !p.yataHitIds.includes('minion_' + mn.id)) {
                    ...
                    mn.hp -= mobBeamDmg; mn.targetId = p.id; mn.state = 'chase';
                    emitDamageText(mn.x, mn.y, mobBeamDmg);
            ...
            okras.forEach(ok => {
                if (ok.hp > 0 && Math.hypot(ok.x - p.x, ok.y - p.y) < ok.radius + 60 && !p.yataHitIds.includes('okra_'+ok.id)) {
                    p.yataHitIds.push('okra_'+ok.id); ok.hp -= mobBeamDmg; ok.targetId = p.id; ok.state='chase';
                    emitDamageText(ok.x, ok.y, mobBeamDmg);
                    if (ok.hp <= 0) { 
                        p.gold += 500; io.to(p.id).emit('updateGold', p.gold); gainXp(p, 50); 
                        if (typeof ctx.addBeopjinKill === 'function') ctx.addBeopjinKill(p.id); 
