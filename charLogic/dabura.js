...
function useSkill(p, data, ctx) {
    ...
    if (data.type === 3) {
        ...
        // 💫 [영역 전용] 자기 영역 안이라면 별 궤도 비행으로 바뀐다.
        //    2초 경직 후 5초 동안 영역 안을 튕겨 다니며 폭발한다 (1회당 150).
        if (typeof ctx.tryDomainKickSkill === 'function' && ctx.tryDomainKickSkill(p)) {
            p.dKickCdEnd = now + (S3.cd || 0);
            if (ctx.io) ctx.io.emit('syncPlayerFull', p);
            return;
        }
