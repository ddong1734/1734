...
window.GLOBAL_ITEM_DB = {
    ...
    'tacheon': { name: '타천(墮天)', desc: '🕊️[집회] 대상마다 50% 확률로 신성력을 1 대신 2 획득', color: '#e74c3c', rarity: '전설', value: 12000, sellPrice: 6000 },
    ...
    'haeru': { name: '해루석', desc: '장착 효과 없음 · 판매 시 10,000 G · 용골/석혼도 제작 재료', color: '#4fd8d0', rarity: '전설', value: 20000, sellPrice: 10000 },
    // 🔥 마르코 전용 계열
    ...
    'magnet_awake': { name: '자기자기열매(각성)', desc: '자기자기열매 + 기계 의수의 모든 능력 (🧲1·2·3번 스킬 강화)', color: '#9b59b6', rarity: '신화', value: 50000, sellPrice: 25000 },
...
window.getBorderColor = (rarity, defaultColor) => {
    ...
    // ⚠️ '고유' 등급은 신화로 통합되었다. 옛 데이터가 남아 있어도 보라색으로 보이게 한다.
    if (rarity.includes('고유')) return '#9b59b6'; 
    return defaultColor; 
};
...
window.renderSmithUI = () => {
    ...
    // 🔥 불사조 마르코 (신화) — 새새 열매 + 황금 벨트 + 5000 G
    let hasPF = inv.some(i => i.id === 'phoenix_fruit');
    let hasGB = inv.some(i => i.id === 'golden_belt');
    let canCraftPM = hasPF && hasGB && (window.myPlayer.gold || 0) >= 5000;
    let boxPM = document.createElement('div'); boxPM.className = 'shop-item-box'; boxPM.style.border = "3px solid #5fe8e0";
    boxPM.innerHTML = `<div style="margin-bottom:8px;"><h3 style="color:#5fe8e0; margin:0 0 5px 0;">불사조 마르코</h3><p style="color:#bdc3c7; font-size:12px;">비용: 5,000 G<br>재료: 새새 열매 모델 불사조+황금 벨트<br><span style="color:#b9fff8;">마르코 1·2·3번 전부 강화</span></p></div><button class="btn-main" style="padding:8px; font-size:15px; width:100%; background:${canCraftPM?'#2fbcd0':'#7f8c8d'}; margin-top:auto;" onclick="window.socket.emit('craftItem', 'phoenix_marco')">${canCraftPM?'5000 G 합성':'재료 부족'}</button>`;
    grid.appendChild(boxPM);

    // 🧲 자기자기열매(각성) (신화) — 자기자기열매 + 기계 의수 + 5000 G
    let hasMF = inv.some(i => i.id === 'magnet_fruit');
    let hasMA = inv.some(i => i.id === 'mech_arm');
    let canCraftAwake = hasMF && hasMA && (window.myPlayer.gold || 0) >= 5000;
