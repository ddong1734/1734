...
(function () {
    ...
    const ORDER = ['PARK', 'BORSALINO', 'KUZAN', 'SAKAZUKI', 'ENEL', 'KASHIMO', 'DABURA', 'DAIDO', 'KURUSU', 'MARCO'];
    let picked = 'PARK';
    ...
    function build() {
        ...
        ORDER.forEach(id => {
            ...
            wrap.id = 'cc_' + id;
            wrap.style.cssText = 'position:relative; background:#1a1e29; border:3px solid #39424f; border-radius:12px;'
                               + 'padding:10px 12px 10px 12px; min-width:132px; cursor:pointer; transition:0.15s; touch-action:manipulation;';
            wrap.innerHTML =
                '<input type="radio" name="charSelect" id="cr_' + id + '" value="' + id + '" style="display:none;">'
              + '<div style="color:' + info.color + '; font-size:16px; font-weight:bold; margin-bottom:2px; padding-right:22px;">' + info.name + '</div>'
              + '<div style="color:#8fa3b8; font-size:11px;">' + info.tag + '</div>'
              + '<div style="position:absolute; top:6px; right:6px; width:24px; height:24px; border-radius:50%;'
              + 'background:#2c3e50; color:#dfe6ec; font-size:14px; font-weight:bold; line-height:24px; text-align:center;">i</div>';

            wrap.addEventListener('click', function (e) {
                const r = wrap.getBoundingClientRect();
                // 오른쪽 위 ⓘ 영역을 누르면 상세보기
                if (e.clientX > r.right - 34 && e.clientY < r.top + 34) window.openCharInfo(id);
