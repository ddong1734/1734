...
// ============================================================================
// ⚡ [신규] 카시모 하지메 — 미구현 스킬 버튼 비활성 표시
//    2번 / 3번 스킬은 아직 구현하지 않았으므로 흐리게 처리해
//    눌러도 아무 일이 없다는 것을 시각적으로 알린다.
// ============================================================================
...
window.updateKashimoSkillDim = () => {
    ...
    if (window._kashimoDimState === isKashimo) return;
    window._kashimoDimState = isKashimo;

    ['btn-skill2', 'btn-skill3'].forEach(id => {
        let btn = document.getElementById(id);
        if (!btn) return;
        if (isKashimo) {
            btn.style.opacity = '0.35';
            btn.style.filter = 'grayscale(90%)';
        } else {
            btn.style.opacity = '';
            btn.style.filter = '';
        }
...
// 전투 중 0.2초마다 카시모 스킬 버튼 상태를 점검한다
setInterval(() => {
