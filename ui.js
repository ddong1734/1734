...
// ============================================================================
// ⚡ 카시모 하지메 — 미구현 스킬 버튼 비활성 표시
//    ✅ [수정] 2번 [주력 방출] 이 구현되었으므로 3번 스킬만 흐리게 처리한다.
//    ⚡🌋 방출 중에는 2번 버튼이 보랏빛으로 빛난다.
// ============================================================================
...
window.updateKashimoSkillDim = () => {
    ...
    if (window._kashimoDimState !== isKashimo) {
        ...
        if (btn3) {
            if (isKashimo) {
                btn3.style.opacity = '0.35';
                btn3.style.filter = 'grayscale(90%)';
            } else {
                btn3.style.opacity = '';
                btn3.style.filter = '';
            }
        }
        // 다른 캐릭터로 바뀌면 2번 버튼 스타일도 원복한다
        if (!isKashimo) {
