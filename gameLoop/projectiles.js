...
function updateFallers(ctx, now, list, opts) {
    ...
    for (let i = list.length - 1; i >= 0; i--) {
        ...
        forEachFallTarget(ctx, f, prevY, 0, (t) => {
            ...
            t.obj.hp -= f.damage;
            emitDamageText(t.obj.x, t.obj.y, f.damage);
            if (t.kind === 'hinbeom' && typeof ctx.recordHinbeomDamage === 'function') ctx.recordHinbeomDamage(f.ownerId, f.damage);
            if (opts.shock) {
                t.obj.electrocutedUntil = Math.max(t.obj.electrocutedUntil || 0, now + 100);
                t.obj.frozenUntil = Math.max(t.obj.frozenUntil || 0, now + 100);
                t.obj.airFreezeUntil = Math.max(t.obj.airFreezeUntil || 0, now + 100);
            ...
            if (!hit || p.piercing) {
                ...
                for (let t of targets) {
                    ...
                    if (t.kind === 'player') {
                        ...
                        t.obj.hp -= p.damage;
                        emitDamageText(t.obj.x, t.obj.y, p.damage);
                        if (t.kind === 'hinbeom' && typeof ctx.recordHinbeomDamage === 'function') ctx.recordHinbeomDamage(p.ownerId, p.damage);
                        applyProjectileEffects(ctx, p, t);
    ...
    updateShockwaves: (ctx, now) => {
        ...
        for (let i = shockwaves.length - 1; i >= 0; i--) {
            ...
            for (let entity of nearEntities) {
                ...
                obj.hp -= sw.damage;
                emitDamageText(obj.x, obj.y, sw.damage);
                ...
                if (kind === 'hinbeom' && typeof ctx.recordHinbeomDamage === 'function') ctx.recordHinbeomDamage(sw.ownerId, sw.damage);
