...
//
// ⚡ [신규] 오크라(황금오크라 포함)가 카시모 하지메를 근접 공격하면
//    카시모의 전기 반격(50 피해 + 보라색 전류)이 발동한다.
//    ※ 할배새끼 · 소환체 · 보스의 공격에는 반격이 발동하지 않는다.
// ============================================================================

const { emitStatus } = require('./shared.js');
const S = require('../server/state.js');   // 🛡️ 보호막 판정
...
function decayKnockback(e) {
    ...
    { const _kp = e.x; e.x += e.knockbackForce; blockedMove(e, _kp); }
...
/** 좌우 이동 + 영역 클램프 공통 */
/** 🛡️ [마르코] 보호막에 막혀 그 자리에서 멈춘다 (몬스터·보스 이동 차단) */
function blockedMove(e, prevX) {
    if (!S.shieldAt) return false;
    if (S.shieldAt(e.x, e.y)) { e.x = prevX; e.knockbackForce = 0; return true; }
    return false;
}
    ...
    update: (ctx, now) => {
        ...
        if (monster.hp > 0 && now >= monster.frozenUntil) {
            ...
            if (monster.state === 'chase' && t && !t.isDead) {
                ...
                } else { const _p = monster.x; monster.x += (monster.homeX > monster.x ? sp : -sp); blockedMove(monster, _p); }
            }
        }
        ...
        if (hinbeom.hp > 0 && hinbeom.state !== 'dead') {
            ...
            if (now >= hinbeom.frozenUntil) {
                ...
                // 🧍 [수정] 박힌범은 이제 자리를 뜨지 않는다.
                //    어그로와 공격(패왕색 패기 등)은 그대로 두고, 이동만 막는다.
                //    x 를 매 프레임 제자리로 되돌려 넉백으로도 밀려나지 않게 한다.
                hinbeom.x = hinbeom.homeX;
                hinbeom.knockbackForce = 0;
            } else { hinbeom.x = hinbeom.homeX; hinbeom.knockbackForce = 0; }
            ...
            for (let hi = hinbeom.hakiBursts.length - 1; hi >= 0; hi--) {
                ...
                while (now >= burst.nextTick && burst.ticksLeft > 0) {
                    ...
                    for (let pid in players) {
                        let t = players[pid];
                        if (t.isDead || !isInHinbeomArea(t)) continue;
                        if (!(S.shieldAt && S.shieldAt(t.x, t.y, undefined))) t.hp -= ctx.HAKI_TICK_DMG;
        ...
        if (bb.hp > 0 && bb.state !== 'dead') {
            ...
            if (!busy && now >= bb.frozenUntil) {
                ...
                if (bb.state === 'chase' && players[bb.targetId]) chaseAndClamp(bb, players[bb.targetId].x, ctx.DARK_AREA, bb.speed);
                    ...
                    else { const _p = bb.x; bb.x += (bb.homeX > bb.x ? bb.speed : -bb.speed); blockedMove(bb, _p); }
            ...
            if (bb.descentActive) {
                while (now >= bb.descentNextTick && now < bb.descentUntil) {
                    ...
                    for (let pid in players) {
                        ...
                        if (!(S.shieldAt && S.shieldAt(t.x, t.y, undefined))) t.hp -= ctx.DESCENT_TICK_DMG;
            ...
            if (bb.darkFloorUntil && now < bb.darkFloorUntil) {
                while (now >= bb.darkFloorNextTick && now < bb.darkFloorUntil) {
                    ...
                    for (let pid in players) {
                        ...
                        if (!(S.shieldAt && S.shieldAt(t.x, t.y, undefined))) t.hp -= actual;
        ...
        if (minions.length > 0 || _minionWasActive) {
            ...
            io.emit('syncMinions', minions.map(mn => ({
                ...
                kashimoCharge: mn.kashimoCharge, kashimoChargeUntil: mn.kashimoChargeUntil
            })));
        }

        // ════════════════════════════════════════════════════════════════
        // 🥬 오크라
        //    ⚡ [신규] 오크라(황금오크라 포함)가 카시모를 때리면
        //       카시모의 전기 반격이 발동한다.
        // ════════════════════════════════════════════════════════════════
        okras.forEach(ok => {
            decayKnockback(ok);
                ...
                if (!p || p.isDead || isInDarkZone(p) || Math.hypot(p.x - ok.x, p.y - ok.y) > 1500) { ok.state = 'return'; ok.targetId = null; }
                    ...
                    if (d <= 40 + ok.radius) {
                        if (now - ok.lastAttack >= 1000) {
                            ...
                            if (!(S.shieldAt && S.shieldAt(p.x, p.y, undefined))) p.hp -= actual;
                            emitDamageText(p.x, p.y, actual);
                            if (p.hp <= 0) checkPlayerDeath(p); else io.to(p.id).emit('takeDamage', actual);

                            // ⚡ 카시모 반격 : 오크라가 카시모를 때렸다면 전류로 되받는다
                            if (!p.isDead && p.characterType === 'KASHIMO'
                                && typeof ctx.kashimoCounterMob === 'function') {
                                ctx.kashimoCounterMob(p, ok, 'okra', ok.id);
                            }
                        }
